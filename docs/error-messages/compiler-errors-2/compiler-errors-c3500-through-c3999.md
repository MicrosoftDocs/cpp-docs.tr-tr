---
title: Derleyici hataları C3500 ile c3999 arasındaki | Microsoft Docs
ms.date: 11/17/2017
ms.technology:
- cpp-diagnostics
...
|Derleyici Hatası C3502|Program kitaplık kimliği alınamıyor '*ProgID*'|
|Derleyici Hatası C3503|karakter ' 0 x*değer*' ham dize sabit değeri izin verilmiyor|
|Derleyici Hatası C3504|dizesinden GUID oluşturulamıyor '*dize*'|
|[Derleyici Hatası C3505](compiler-error-c3505.md)|tür kitaplığı yüklenemiyor '*Kitaplığı*'|
|[Derleyici Hatası C3506](compiler-error-c3506.md)|Kitaplık kimliği için kayıtlı bir tür kitaplığı yok '*Kitaplığı*'|
|[Derleyici Hatası C3507](compiler-error-c3507.md)|bir program kimliği en fazla 39 karakter olabilir*ProgID*'; ya da bir noktalama işareti içeriyor. '.'; ya da rakamla başlayamaz|
|[Derleyici Hatası C3508](compiler-error-c3508.md)|'*türü*': geçerli bir Otomasyon türü değil|
|[Derleyici Hatası C3509](compiler-error-c3509.md)|'*türü*': Geçersiz Otomasyon dönüş türü; bir parametre 'retval' olarak işaretlendiğinde, dönüş türü 'void', 'HRESULT' veya 'SCODE' olmalıdır|
|[Derleyici Hatası C3510](compiler-error-c3510.md)|bağımlı tür kitaplığı bulunamıyor *kitaplığı*|
|Derleyici Hatası C3511|'*tanımlayıcı*': bir temsilci oluşturucusuna bir çağrı tek üye Başlatıcısı olmalıdır|
|Derleyici Hatası C3512|Ham dize sabit değeri için sınırlayıcı karakter sırası en fazla 16 karakter olmalıdır|
|Derleyici Hatası C3513|'*dize*': Desteklenmeyen ham dize sabit değeri sınırlayıcı karakteri|
|Derleyici Hatası C3514|'*karakter*' (*değer*): Desteklenmeyen ham dize sabit değeri sınırlayıcı karakteri|
|Derleyici Hatası C3515|bir sınıf şablonu kısmi özelleştirmesi için bir bağımsız değişken bir paket genişletmesi ise son bağımsız değişken olmalıdır|
|Derleyici Hatası C3516|Beklenmeyen bitiş-ham dize sabit değeri işlenirken dosya bulundu; sınırlayıcı sırası '*dize*' eşleşmedi|
|Derleyici Hatası C3517|'*tanımlayıcı*' bir diğer ad bildirimi 'auto' içeren bir tür olamaz|
|Derleyici Hatası C3518|'*tanımlayıcı*': bir doğrudan liste başlatma bağlamında türü için '*türü*' yalnızca bir tek bir başlatıcı ifadeden|
|[Derleyici Hatası C3519](compiler-error-c3519.md)|'*parametre*': embedded_idl özniteliğine geçersiz parametre|
|Derleyici Hatası C3520|'*tanımlayıcı*': parametre paketi bu bağlamda genişletilmeli|
|Derleyici Hatası C3521|'*tanımlayıcı*' bir parametre paketi değil|
|Derleyici Hatası C3522|'*türü*': parametre paketi bu bağlamda genişletilemiyor|
|Derleyici Hatası C3523|'sizeof...' bağımsız değişkeni bir genişletilmemiş parametre paketi gerektirir|
|Derleyici Hatası C3524|'*tanımlayıcı*': 'sizeof' bir parametre paketine uygulanamaz. 'Sizeof...' kullanmayı mı amaçlamıştınız?|
|Derleyici Hatası C3525|'*parametre*': bir sınıf şablonunun şablon parametre listesinin sonunda görünmelidir bir şablon parametre paketine sahipse|
|Derleyici Hatası C3526|'...' 'this' uygulanamaz|
|Derleyici Hatası C3527|'*tanımlayıcı*' 'sizeof...' için geçerli bir işlenen değil. 'Sizeof' kullanmayı mı amaçlamıştınız?|
...
|Derleyici Hatası C3564|concurrency::parallel_for_each için çağrı grafı derlenirken başlatılmamış değer okunurken: '*işlevi*'|
...
|[Derleyici Hatası C3638](compiler-error-c3638.md)|'*işleci*': standart kutulama ve kutudan çıkarma dönüştürme işleçleri yeniden tanımlanamaz|
...
|[Derleyici Hatası C3675](compiler-error-c3675.md)|'*işlevi*': olduğundan ayrıldı '*özelliği*' tanımlanır|
...
|[Derleyici Hatası C3712](compiler-error-c3712.md)|'*event_handler*': olay işleyicisi yöntemi kaynak aynı türü döndürmelidir '*olay*'|
...
|Derleyici Hatası C3786|'std::make_integer_sequence' için ikinci şablon bağımsız değişkeni değerinden büyük veya sıfıra eşit bir tamsayı sabiti olmalıdır|
...
|[Derleyici Hatası C3860](compiler-error-c3860.md)|sınıf şablonu/genel adından şablonu/genel bağımsız değişken listesi parametreleri şablon/genel parametre listesinde kullanıldıkları sırada listelemelidir|
...
|Derleyici Hatası C3934|herhangi bir 'main' biçiminde olan bir işlev restrict(cpu) dışında kısıtlama belirticileri bulunamaz|
...
|Derleyici Hatası C3971|'*türü*': Kısmi tanım tam tanımdan sonra yer alamaz|
