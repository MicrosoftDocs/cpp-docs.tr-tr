...
|Hata|İleti|
|-----------|-------------|
|Derleyici Hatası C2900|'*bildirimcisi*': 'özel', 'iç' veya 'özel korumalı' WinRT sınıflardaki üye işlev şablonları olmalıdır|
|Derleyici Hatası C2901|'*tanımlayıcısı*': Genel arabirim veya temsilci ortak olamaz|
...
|[Derleyici Hatası C2903](compiler-error-c2903.md)|'*tanımlayıcısı*': simge olan sınıf şablon/genel ne şablon/genel işlevi|
|[Derleyici Hatası C2904](compiler-error-c2904.md)|'*tanımlayıcısı*': adı geçerli kapsamdaki bir şablon için zaten kullanılıyor|
|Derleyici Hatası C2905|Kullanımdan kalktı.|
|[Derleyici Hatası C2906](compiler-error-c2906.md)|'*şablonu*': açık uzmanlık 'şablon <>' gerektirir|
|Derleyici Hatası C2907|bağımsız değişken Kaydet '*numarası*' geçerli bir kayıt numarası belirtmiyor|
|[Derleyici Hatası C2908](compiler-error-c2908.md)|Açık uzmanlık; '*şablonu*' zaten başlatıldı|
|[Derleyici Hatası C2909](compiler-error-c2909.md)|'*tanımlayıcısı*': işlev şablonunun açık örnekleme dönüş türü gerektirir|
|[Derleyici Hatası C2910](compiler-error-c2910.md)|'*işlevi*': açıkça özel|
|[Derleyici Hatası C2911](compiler-error-c2911.md)|'*üye*': bildirilen veya geçerli kapsamda tanımlı|
|[Derleyici Hatası C2912](compiler-error-c2912.md)|Açık uzmanlık '*bildirimi*' işlevi şablon uzmanlaşması değil|
...
|[Derleyici Hatası C2920](compiler-error-c2920.md)|yeniden tanımlama: '*türü*': sınıf şablon/genel zaten bildirildikten olarak*bildirimi*'|
|[Derleyici Hatası C2921](compiler-error-c2921.md)|yeniden tanımlama: '*türü*': sınıf şablon/genel yeniden bildirilen olarak*bildirimi*'|
|Derleyici Hatası C2922|'*arabirimi*': A WinRT arabirim statik üyeler içeremez|
|[Derleyici Hatası C2923](compiler-error-c2923.md)|'*türü*': '*tanımlayıcısı*'parametresi için geçerli şablonu genel tür bağımsız değişkeni değil'*parametresi*'|
|Derleyici Hatası C2924|R2'de __declspec(interrupt) rutin bağımsız değişken|
|Derleyici Hatası C2925|__declspec(interrupt) yordamı kayan nokta kullanamazsınız|
|Derleyici Hatası C2926|'*tanımlayıcısı*': varsayılan üye başlatıcıdan UNION içinde anonim bir yapı üyesi için izin verilmiyor|
|[Derleyici Hatası C2927](compiler-error-c2927.md)|'*tanımlayıcısı*': işlev şablonunun en az bir bağımsız değişkeni ile çağrılmalıdır|
|[Derleyici Hatası C2928](compiler-error-c2928.md)|Açık örnekleme; '*tanımlayıcısı*'işlevi veya Şablon sınıfı statik veri üyesi değil'*sınıfı*'|
|[Derleyici Hatası C2929](compiler-error-c2929.md)|'*bildirimcisi*': açık örnekleme; olamaz açıkça zorlamak ve şablon sınıfının üye eşlemesinin bastırmak|
|[Derleyici Hatası C2930](compiler-error-c2930.md)|'*sınıfı*': şablon kimliği/genel kimliği bir numaralandırıcı olarak yeniden tanımlandı ' enum *tanımlayıcısı*'|
|[Derleyici Hatası C2931](compiler-error-c2931.md)|'*class1*': şablon kimliği/genel kimliği bir üye işlevini yeniden tanımlandı '*Ders2*'|
|[Derleyici Hatası C2932](compiler-error-c2932.md)|'*türü*': şablon kimliği/genel kimliği veri üyesi olarak yeniden tanımlandı '*tanımlayıcısı*'|
|[Derleyici Hatası C2933](compiler-error-c2933.md)|'*türü*': şablon kimliği/genel kimliği typedef üye olarak yeniden tanımlandı '*tanımlayıcısı*'|
|[Derleyici Hatası C2934](compiler-error-c2934.md)|'*türü*': şablon kimliği/genel kimliği yeniden tanımlandı iç içe '*öğesi*','*tanımlayıcısı*'|
|[Derleyici Hatası C2935](compiler-error-c2935.md)|'*türü*': şablon kimliği/genel kimliği genel bir işlevi olarak yeniden tanımlandı|
|[Derleyici Hatası C2936](compiler-error-c2936.md)|'*türü*': şablon kimliği/genel kimliği genel veri değişken olarak yeniden tanımlandı|
...
|[Derleyici Hatası C2973](compiler-error-c2973.md)|'*şablonu*': Geçersiz şablon bağımsız değişken '*numarası*'|
