---
title: Derleyici uyarılarını C4600 ile c4799 arasındaki | Microsoft Docs
ms.date: 07/03/2018
ms.technology:
- cpp-diagnostics
...
|Derleyici Uyarısı (düzey 1) C4602|#pragma pop_macro: 'makro adı' önceki hiçbir #pragma push_macro bu tanımlayıcısı|
|Derleyici Uyarısı (düzey 1) C4603|'*tanımlayıcı*': Makro tanımlı değil veya tanımı, önceden derlenmiş üstbilgi kullanımından farklıdır|
...
|Derleyici Uyarısı (düzey 1) C4613|'*sembol*': kesimin sınıfı değiştirilemez|
...
|Derleyici Uyarısı (düzey 1) C4620|hiçbir sonek biçimi ' operator ++' 'type' önek biçimi kullanılıyor, türü için bulunamadı|
...
|Derleyici Uyarısı (Düzey 3) C4622|hata ayıklama bilgileri üzerine oluşturulmuş nesne dosyasındaki Ön derlenmiş üstbilginin oluşturulması sırasında: 'dosya'|
|[Derleyici Uyarısı (düzey 4) C4623](../../error-messages/compiler-warnings/compiler-warning-level-4-c4623.md)|'derived class': varsayılan oluşturucu örtük bir şekilde bir taban sınıf varsayılan oluşturucusuna erişilemez veya silinmiş olduğundan silindi olarak tanımlandı|
|[Derleyici Uyarısı (düzey 1) C4624](../../error-messages/compiler-warnings/compiler-warning-level-1-c4624.md)|'derived class': yıkıcı örtük bir şekilde bir temel sınıf yok edicisini erişilemez veya silinmiş olduğundan silindi olarak tanımlandı|
|[Derleyici Uyarısı (düzey 4) C4625](../../error-messages/compiler-warnings/compiler-warning-level-4-c4625.md)|'derived class': kopya Oluşturucusu örtük bir şekilde bir taban sınıf kopya oluşturucusuna erişilemez veya silinmiş olduğundan silindi olarak tanımlandı|
|[Derleyici Uyarısı (düzey 4) C4626](../../error-messages/compiler-warnings/compiler-warning-level-4-c4626.md)|'derived class': atama işleci örtük bir şekilde bir taban sınıf atama işlecine erişilemez veya silinmiş olduğundan silindi olarak tanımlandı|
|[Derleyici Uyarısı (düzey 1) C4627](../../error-messages/compiler-warnings/compiler-warning-level-1-c4627.md)|'\<tanımlayıcısı >': Ön derlenmiş üstbilgi kullanımı aranırken atlandı|
|[Derleyici Uyarısı (düzey 1) C4628](../../error-messages/compiler-warnings/compiler-warning-level-1-c4628.md)|-Ze ile digraf kullanılması desteklenmez. '%S' için alternatif bir belirteç olarak yorumlanmadı'digraph ' karakter dizisi|
|Derleyici Uyarısı (düzey 4) C4629|Digraph kullanıldığında, belirteç char (Bu değilse, iki karakter arasına bir boşluk ekleyin)' olarak yorumlanıp'digraph ' karakter dizisi|
...
|Derleyici Uyarısı (düzey 4) C4634|XML belgesi yorum hedef: uygulanamaz: nedeni|
|Derleyici Uyarısı (Düzey 3) C4635|XML belgesi yorum hedef: hatalı oluşturulmuş XML: nedeni|
|Derleyici Uyarısı (Düzey 3) C4636|XML belgesi açıklaması oluşturmak için uygulanan: Etiket boş olmayan 'öznitelik' özniteliği gerektirir.|
|Derleyici Uyarısı (Düzey 3 ve düzey 4) C4637|XML belgesi yorum hedef: \<dahil > Etiket atıldı. Neden|
|Derleyici Uyarısı (Düzey 3) C4638|XML belgesi yorum hedef: Bilinmeyen Sembol 'symbol' başvurusu.|
...
|Derleyici Uyarısı (Düzey 3) C4645|__declspec(noreturn) ile bildirilen işlevde dönüş deyimi var|
|Derleyici Uyarısı (Düzey 3) C4646|__declspec(noreturn) ile bildirilen işlevde void olmayan dönüş türüne sahip.|
...
|Derleyici Uyarısı (düzey 1) C4655|'symbol': değişken türü son yapımdan beri yeni veya başka yerde farklı şekilde tanımlanmış|
...
|Derleyici Uyarısı (düzey 1) C4657|ifade en son yapımdan beri yeni olan bir veri türü içeriyor|
...
|Derleyici Uyarısı (düzey 1) C4662|Açık örnek oluşturma; Şablon sınıfı 'ıdentifier1', 'identifier2' değerini özelleştirecek tanım yok sahiptir.|
...
|Derleyici Uyarısı (düzey 4) C4670|'identifier': Bu taban sınıfına erişilemez|
|Derleyici Uyarısı (düzey 4) C4671|'identifier': kopya oluşturucusuna erişilemez|
|Derleyici Uyarısı (düzey 4) C4672|'ıdentifier1': belirsiz. İlk 'identifier2' görüldü|
...
|Derleyici Uyarısı (düzey 1) C4674|'method', 'static' olarak bildirilmeli ve tam olarak bir parametreye sahip|
...
|Derleyici Uyarısı (düzey 1) C4678|temel sınıf 'taban_türü' 'derived_type' daha az erişilebilir|
|[Derleyici Uyarısı (düzey 1) C4679](../../error-messages/compiler-warnings/compiler-warning-level-1-c4679.md)|'member': üye alınamadı|
...
|Derleyici Uyarısı (düzey 4) C4718|'işlev çağrısı': özyinelemeli çağrının silme hiçbir yan etkisi vardır|
...
|Derleyici Uyarısı (düzey 1) C4722|'function': yok edici hiç dönmüyor, olası bellek sızıntısı|
|[Derleyici Uyarısı (düzey 3) C4723](../../error-messages/compiler-warnings/compiler-warning-level-3-c4723.md)|olası 0 ile bölme|
|Derleyici Uyarısı (Düzey 3) C4724|olası 0 ile mod|
...
|Derleyici Uyarısı (düzey 1) C4739|'var' değişkeni başvuru depolama alanını aşıyor|
...
|Derleyici Uyarısı (düzey 4) C4754|%S(%d) konumundaki karşılaştırma içindeki aritmetik işlemler için dönüştürme kuralları bir dalın yürütülemez anlamına gelir. '%S', '%s' (veya %d baytlık benzer bir tür) dönüştürün.|
...
|Derleyici Uyarısı (düzey 4) C4764|Yakalama nesneleri 16 baytın hizalayabilirsiniz değil|
...
|Derleyici Uyarısı (düzey 1) C4794|Segment iş parçacığında yerel depolama değişken '% '%s' öğesinden '%s' s' değiştirildi|
