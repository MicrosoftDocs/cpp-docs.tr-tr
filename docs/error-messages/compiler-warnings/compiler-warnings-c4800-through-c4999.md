...
| Derleyici Uyarısı (düzey 4) C4845 | '\_\_declspec (hiçbir\_init\_All) ', komut satırında '/d1initall\[0\|1\|2\|3] ' belirtilmemişse yok sayılır |
| Derleyici Uyarısı (düzey 4) C4846 | '*Value*', '/d1initall ' için geçerli bir bağımsız değişken değil: komut satırı bayrağı yoksayıldı |
| Derleyici Uyarısı (düzey 4) C4847 | '\_\_declspec (hiçbir\_init\_All) ' yalnızca bir işleve, bir sınıf türüne veya yerel değişkene uygulanabilir: yoksayıldı |
| Derleyici Uyarısı (düzey 1) C4848 | C++ 17 ve önceki sürümlerde ' No\_UNIQUE\_Address ' standart özniteliği için destek bir satıcı uzantısıdır |
...
|Derleyici Uyarısı (düzey 2) C4872|Şu konumdaki concurrency::p arallel_for_each için çağrı grafı derlenirken, kayan nokta sıfıra bölme algılandı: '*Location*'|
...
|Derleyici Uyarısı (düzey 1) C4882|const olmayan çağrı işleçleri olan funları eşzamanlılık::p arallel_for_each olarak geçirme kullanım dışıdır|
...
|[Derleyici Uyarısı (düzey 1) C4910](compiler-warning-level-1-c4910.md)|'\<tanımlayıcı >: ' __declspec (dllexport) ' ve ' extern ' açık bir örnek oluşturmada uyumsuz|
...
|[Derleyici Uyarısı (düzey 1) C4920](compiler-warning-level-1-c4920.md)|Enum enum üyesi member_1 = value_1 member_2 = olarak Enum numaralandırmasında zaten görüldü value_2|
...
|Derleyici Uyarısı (düzey 1) C4934|' __delegate (çok noktaya yayın) ' kullanım dışı, bunun yerine '\__delegate ' kullanın|
...
|[Derleyici Uyarısı (düzey 4) C4938](compiler-warning-level-4-c4938.md)|'*var*': kayan nokta azaltma değişkeni,/fp: strict veya #pragma altında tutarsız sonuçlara neden olabilir fenv_access|
...
|Derleyici Uyarısı C4954|'*Function*': profil oluşturulmadı (__int64 switch ifadesi içeriyor)|
...
|Derleyici Uyarısı (düzey 1) C4971|Bağımsız değişken sırası: \<hedef nesne >, \<hedef işlev > temsilci Oluşturucu için kullanım dışı bırakıldı, \<Target işlevi >, \<hedef nesne = "" kullanın >|
...
|Derleyici Uyarısı (düzey 3) C4991|Warbird: '*Function*__forceinline ' işlevi satır içine alınmadı çünkü alınanın koruma düzeyi üst öğeden büyük|
...
| Derleyici Uyarısı (düzey 1) C5047 | modüllerle\_varsa \_standart olmayan \_kullanımı desteklenmez |
...
| Derleyici Uyarısı (düzey 1) C5100 | \_\_VA\_bağımsız değişkenleri\_\_, değişen sayıda bağımsız makrolarda kullanılmak üzere ayrılmıştır |
