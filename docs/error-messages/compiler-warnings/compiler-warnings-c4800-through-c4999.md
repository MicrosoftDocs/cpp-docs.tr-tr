...
|Derleyici Uyarısı (düzey 1) C4806|'işlemi': güvensiz işlemi: hiçbir değer türü 'type 'type' türü için yükseltilen' verilen sabiti eşit olabilir|
|Derleyici Uyarısı (düzey 1) C4807|'işlemi': türü 'type' ve 'type' türünde imzalı bit alan güvensiz karışımı|
|Derleyici Uyarısı (düzey 1) C4808|durumu 'value' anahtar koşul 'bool' türü için geçerli bir değer değil|
...
|Derleyici Uyarısı (düzey 1) C4812|eski bildirim stili: Lütfen bunun yerine 'new_syntax' kullanın|
|Derleyici Uyarısı (düzey 1) C4813|'function': yerel bir sınıfın arkadaş işlevi önceden bildirilmiş olmalıdır|
...
|Derleyici Uyarısı (düzey 1) C4817|'member': geçersiz kullanımı '.' Bu üye; erişmek için Derleyici yerine '->'|
...
|[Derleyici Uyarısı (düzey 1) C4838](compiler-warning-level-1-c4838.md)|daraltma dönüşümü 'type_2'-'type_1' dönüşümü gerektirir|
...
|[Derleyici Uyarısı (hata) C4867](compiler-warning-c4867.md)|'function': bağımsız değişken listesi; eksik işlev çağrısı 'çağrı' üyesi için bir işaretçi oluşturmak için kullanın|
...
|Derleyici Uyarısı (düzey 1) C4880|'const type_1' 'type_2' çevrim: hemen bir işaretçi veya reference constness atama sonuçlanabilir bir kısıtlanmış amp işlevinde tanımsız davranış|
|Derleyici Uyarısı (düzey 4) C4881|yapıcı ve/veya yıkıcı tile_static değişkeni 'değişkeni' çağrılan değil|
...
|C4900 uyarı derleyici|'Tool1' Sürüm 'version1' ve 'tool2' Sürüm 'version2' arasında IL uyuşmazlığı|
...
|Derleyici Uyarısı (düzey 1) C4912|'öznitelik': öznitelik iç içe geçmiş UDT davranışı tanımlı olmayan|
...
|Derleyici Uyarısı (düzey 4) C4918|'character': pragma iyileştirme listesinde geçersiz bir karakter|
...
|[Derleyici Uyarısı (düzey 1) C4929](compiler-warning-level-1-c4929.md)|'dosya': typelibrary içeren bir birleşim; 'embedded_idl' niteleyicisi yok sayılıyor|
...
|Derleyici Uyarısı (düzey 1) C4935|derleme erişim belirticisi 'erişimden' değiştirildi|
|Derleyici Uyarısı (düzey 1, hata) C4936|Bu __declspec yalnızca/CLR veya/CLR ile derlendiğinde desteklenir: Saf|
|Derleyici Uyarısı (düzey 4) C4937|'Metin1' ve 'Metin2' 'yönergesi' bağımsız değişken olarak ayırt|
...
|Derleyici Uyarısı (düzey 1) C4944|'simgesi': 'assembly1' simgesini alınamıyor: 'simgesi' geçerli kapsamda zaten mevcut olduğundan|
...
|Derleyici Uyarısı (düzey 1) C4952|'function': hiçbir profili veri program veritabanı 'pgd_file' içinde bulunamadı|
|Derleyici Uyarısı (düzey 1) C4953|Inlinee 'function' profili itibaren kullanılmıyor profil verileri, toplanan veriler, düzenlenebilir|
|C4954 uyarı derleyici|'function': değil profili (__int64 anahtar ifadesi içerir)|
|C4955 uyarı derleyici|'import2': göz ardı; içeri aktarma 'import1' aktarılmış|
|Derleyici Uyarısı (düzey 1, hata) C4956|'type': Bu tür doğrulanabilen değil|
|Derleyici Uyarısı (düzey 1, hata) C4957|'cast': 'cast' belgesi 'cast_to' den açık atama doğrulanabilen değil|
|Derleyici Uyarısı (düzey 1, hata) C4958|'işlemi': işaretçi aritmetiği doğrulanabilen değil|
|Derleyici Uyarısı (düzey 1, hata) C4959|üyelerine erişmeden doğrulanamayan kodu sağladığından yönetilmeyen türü 'type' / CLR: safe içinde tanımlanamıyor|
|Derleyici Uyarısı (düzey 4) C4960|'function' profili için çok büyük|
|Derleyici Uyarısı (düzey 1) C4961|Profil verileri '.pgd dosyasına', profil temelli iyileştirmeler devre dışı birleştirilmiş|
|Derleyici Uyarısı (düzey 4) C4962|'function': Profil temelli iyileştirmeler iyileştirmeler profil verileri tutarsız hale gelmesine neden olduğundan devre dışı|
