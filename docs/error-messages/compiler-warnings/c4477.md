...
İlk hata iletisinde, derleyici bir **int** beklendiğinde bir **Double** bağımsız değişkeninin sağlanmış olduğunu algılar. Kayan bağımsız değişken listesinde bir **float** otomatik olarak **Double** olarak yükseltilir. İkinci hata iletisi derleyicinin bir **Double** bağımsız değişkeni bekliyordu, ancak bir **int** sağlanmış olduğunu gösterir. Derleyici bağımsız değişkenlerin sırasını değiştirmiş olduğunu söyleyebilir. Gerçek bağımsız değişken sırasını tespit etmek ve doğru bağımsız değişken türlerini sağlamak için biçim dizesini incelemeniz gerekir.
