...
Derleyici kullanım dışı bir bildirimi ile karşılaşıldı.  
  
Bu uyarı veya hata bağlam bağlı olarak birkaç olası anlama gelir.  
  
C4996 derleyici bir işlevi veya olarak işaretlenmiş değişken karşılaştığında oluşur [kullanım dışı](../../cpp/deprecated-cpp.md) kullanarak bir `__declspec(deprecated)` değiştiricisi. Bir işlev, sınıf üyesi veya C ++ 14 sahip typedef erişmeyi denediğinde bu uyarı da verilen `[[deprecated]]` özniteliği. Daha fazla bilgi için bkz: [C++ Standart öznitelikleri](../../cpp/attributes2.md). Bu öznitelik kendiniz kitaplıklarınızda istemcileriniz kullanım dışı bırakılan İşlevler, üyeleri ve tür tanımları hakkında uyarmak için kullanabilirsiniz.  
  
Çeşitli işlevler, üye işlevleri, şablon işlevleri ve genel değişkenler Visual Studio'da kitaplıklarınızdaki kullanım dışı olarak işaretlenir. Bu işlevler farklı tercih edilen bir ad olabilir, güvenli olmayabilir veya daha güvenli bir değişken olması ya da geçersiz olabilir. Çok sayıda hata iletileri kullanım dışı işlev veya genel değişkeni için önerilen yenileme içerir.  
...
Belirli bir kod satırı için uyarı kullanarak devre dışı bırakabilirsiniz [uyarı](../../preprocessor/warning.md) pragma `#pragma warning(suppress : 4996)`. Bir dosyanın içinde uyarı pragma kullanarak kapatabilirsiniz `#pragma warning(disable : 4996)`. Genel komut satırı derlemeleri kullanarak kapatabilirsiniz **/wd4996** komut satırı seçeneği. Visual Studio IDE içinde projesi için uyarıyı kapatmak için açık **özellik sayfaları** iletişim kutusunda **yapılandırma özellikleri**, **C/C++**,  **Gelişmiş** sayfasında ve düzenleme **belirli uyarıları devre dışı** eklemek üzere özellik `4996`.  Önişlemci makroları, belirli sınıflar kitaplıklarda kullanılan kullanımdan kaldırma uyarıları devre dışı bırakmak için de kullanabilirsiniz. Bu makroları aşağıda açıklanmıştır.  
  
C4996 kitaplık kaynaklarını bazıları aşağıda verilmiştir.  
...
Bu işlevler için kullanımdan kaldırma uyarıları kapatmak için önişlemci makrosu tanımlama **_CRT_NONSTDC_NO_WARNINGS**. Seçeneğini ekleyerek bu komut satırında tanımlayabilirsiniz `/D_CRT_NONSTDC_NO_WARNINGS`. Visual Studio'da bu makrosu tanımlamak için açık **özellik sayfaları** projeniz için iletişim kutusu. Genişletme **yapılandırma özellikleri**, **C/C++**, **önişlemci**. İçinde **önişlemci tanımları**, ekleme `_CRT_NONSTDC_NO_WARNINGS`. Seçin **Tamam** kaydedin ve projenizi yeniden derleyin. Yalnızca belirli bir kaynak dosyalarında bu makrosu tanımlamak için satırı ekleyin `#define _CRT_NONSTDC_NO_WARNINGS` üstbilgi dosyası içeren herhangi bir satırı önce.  
  
## <a name="unsafe-crt-library-functions"></a>Güvenli olmayan CRT kitaplık işlevleri  
  
 **Bu işlev veya değişken güvenli olmayabilir. Kullanmayı***safe_version* **yerine.   Kullanımdan kaldırma devre dışı bırakmak için _CRT_SECURE_NO_WARNINGS kullanın.  Ayrıntılar için çevrimiçi yardıma bakın.**  
 ...
 CRT'deki bu işlevler için kullanımdan kaldırma uyarıları kapatmak için tanımlamak **_CRT_SECURE_NO_WARNINGS**. Kullanım dışı genel değişkenler hakkında uyarılar devre dışı bırakmak üzere tanımlamak **_CRT_SECURE_NO_WARNINGS_GLOBALS**. Bu kullanım dışı bırakılan işlevler ve genel öğeleri hakkında daha fazla bilgi için bkz: [CRT'deki güvenlik özellikleri](../../c-runtime-library/security-features-in-the-crt.md) ve [güvenli kitaplıklar: C++ Standart Kitaplığı](../../standard-library/safe-libraries-cpp-standard-library.md).  
  
## <a name="unsafe-standard-library-functions"></a>Güvenli standart kitaplığı işlevleri  
  
 **' std::** *işlev_adı* **::\_işaretlenmemiş\_yineleyiciler::\_Deprecate' çağrısı std::** *işlev_adı* **- olmayabilecek parametrelerle geçirilen değerlerin doğru olduğunu kontrol etmek için arayan bu çağrıyı kullanır. Bu uyarıyı devre dışı bırakmak için -D_SCL_SECURE_NO_WARNINGS kullanın. Visual C++ 'İşaretli yineleyiciler' kullanımı konusunda belgelerine bakın**  
  
Belirli C++ Standart kitaplığı şablon işlevleri parametreleri doğruluğunu denetleyin değil çünkü hata ayıklama derlemelerinde bu uyarı görüntülenir. Çoğu durumda, kapsayıcı sınırları denetlemek için işlev için yeterli bilgi bulunmadığından ya da yineleyiciler yanlış işlev ile kullanılan nedeni budur. Güvenlik açıklarını programınızdaki kaynağı olabileceğinden bu uyarı bu işlevi kullanımları belirlemenize yardımcı olur. Daha fazla bilgi için bkz: [işaretli yineleyiciler](../../standard-library/checked-iterators.md).  
...
Kodunuzu bu uyarıyı tetikleyen standart kitaplığı işlevlerinde hata taşması olamaz doğruladıysanız bu uyarıyı devre dışı bırakmak isteyebilirsiniz. Bu işlevler için uyarıları kapatmak için tanımlamak **_SCL_SECURE_NO_WARNINGS**.   
  
## <a name="example-checked-iterators-enabled"></a>Örnek: etkin işaretli yineleyiciler  
...
Bu öğeler için kullanımdan kaldırma uyarıları kapatmak için tanımlamak **_CRT_OBSOLETE_NO_WARNINGS**. Daha fazla bilgi için kullanım dışı işlev veya değişken belgelerine bakın.  
  
## <a name="example-marshalling-errors-in-clr-code"></a>Örnek: CLR kod hatalarını dizimi  
