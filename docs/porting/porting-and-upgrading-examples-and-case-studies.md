---
title: "Taşıma ve yükseltme: örnekler ve örnek olay incelemeleri | Microsoft Docs"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
ms.technology: cpp-language
ms.tgt_pltfrm: 
ms.topic: article
dev_langs: C++
ms.assetid: d48bbff9-1ea7-467f-8c8b-758601f01573
caps.latest.revision: "5"
author: mikeblome
ms.author: mblome
manager: ghogen
ms.workload: cplusplus
ms.openlocfilehash: 212b51f417fc84b602e6231f2e769cb73c9a1460
ms.sourcegitcommit: 8fa8fdf0fbb4f57950f1e8f4f9b81b4d39ec7d7a
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 12/21/2017
---
# <a name="porting-and-upgrading-examples-and-case-studies"></a>Taşıma ve Yükseltme: Örnekler ve Örnek Olay İncelemeleri
Tür süresi karşılaşabileceğiniz sorunları hakkında bir fikir edinmek başarılı geçişler hikayeleri inceleyerek geçerli bir Visual C++'ın önceki bir sürümünden geçiş.  
  
## <a name="examples-of-successful-migrations"></a>Başarılı geçişler örnekleri  
 Burada, ne bazı uygun örnekleri ve araçları, Visual C++'in en son sürüme geçirmek için sürdü hakkında bilgi edinebilirsiniz. Bu geçiş karşılaşılan sorunları türlerini tipik sorunları, tüm geçiş, ancak önemli bir durumla karşılaşabileceğiniz ve kod boyutu. Elbette gerçek uygulama koduna göre çok küçük ve kısa örneklerdir.  
  
-   [MFC karalama](../porting/porting-guide-mfc-scribble.md) MFC temelleri kullanan bir örnek verilmiştir.  
  
-   [COM Spy](../porting/porting-guide-mfc-scribble.md) bir makinede COM + bileşenleri hakkında bilgi görüntüler bir uygulamadır.  
  
-   [Spy ++](../porting/porting-guide-spy-increment.md) inceler ve çalışan uygulamalar, kullanıcı Arabirimi öğeleri hakkında bilgi sağlayan bir uygulamadır.  
  
## <a name="see-also"></a>Ayrıca Bkz.  
 [Visual C++ Taşıma ve Yükseltme Kılavuzu](../porting/visual-cpp-porting-and-upgrading-guide.md)