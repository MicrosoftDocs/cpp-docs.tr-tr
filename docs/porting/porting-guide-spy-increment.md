...
```  
 ...
 Bul (Ctrl + Shift + F) dosyalarında kullanın ve belirtmek için kullanılabilir dahil etme dosyalarında sembol bulunacak en kolay yolu olan **Visual C++ ekleme dizinleri**. Biz ntverp.h bulundu. Biz verstamp.h yerine ile ntverp.h içerir ve bu hata görünmüyor.  
  
##  <a name="linker_output_settings"></a> 3. adım. Bağlayıcı OutputFile ayarı  
 Eski projeler bazen yükselttikten sonra sorunlara neden olabilecek sıra dışı konumlarda yerleştirilen dosyaları gerekir. Bu durumda, biz $(SolutionDir) yoluna var. yerleştirilir bazı başlık dosyalarını Visual Studio bulabileceğinizden emin olmak için proje özelliklerinde yerine proje klasörleri birini eklemeniz gerekir.  
  
 MSBuild Link.OutputFile özelliği TargetPath ve TargetName değerleri eşleşmiyor MSB8012 verme şeklinde hata verir.  
  
```Output  
warning MSB8012: TargetPath(...\spyxx\spyxxhk\.\..\Debug\SpyxxHk.dll) does not match the Linker's OutputFile property value (...\spyxx\Debug\SpyHk55.dll). This may cause your project to build incorrectly. To correct this, please make sure that $(OutDir), $(TargetName) and $(TargetExt) property values match the value specified in %(Link.OutputFile).warning MSB8012: TargetName(SpyxxHk) does not match the Linker's OutputFile property value (SpyHk55). This may cause your project to build incorrectly. To correct this, please make sure that $(OutDir), $(TargetName) and $(TargetExt) property values match the value specified in %(Link.OutputFile).  
```  
  
 **Link.OutputFile** (EXE, DLL, örneğin) derleme çıktı ve genellikle oluşturulduğu $(TargetDir)$(TargetName)$(TargetExt), yol ve dosya adı uzantısı sağlar. Eski Visual C++ projeleri geçirme Aracı (vcbuild.exe) oluşturduğunuzda Yeni derleme aracını (MSBuild.exe) sık karşılaşılan budur. Visual Studio 2010'da derleme aracı değişikliğinin olduğundan, bu bir pre-2010 project 2010'a geçişi olduğunda sorunu veya sonraki bir sürümünü karşılaşabilirsiniz. Proje Yükseltme Sihirbazı'nı güncelleştirmez temel sorun olduğunu **Link.OutputFile** her zaman değeri olması gerektiğini belirlemek mümkün olmadığından değer bir proje ayarlarına göre. Bu nedenle, genellikle el ile ayarlamanız gerekir. Daha fazla ayrıntı için bkz. Bu [sonrası](http://blogs.msdn.com/b/vcblog/archive/2010/03/02/visual-studio-2010-c-project-upgrade-guide.aspx) Visual C++ blogundaki.  
  
 Bu durumda, **Link.OutputFile** dönüştürülmüş projedeki özellik,.\Debug\Spyxx.exe ve yapılandırmasına bağlı olarak Spy ++ projesi için.\Release\Spyxx.exe ayarlandığı. En iyi sonuç sadece bu sabit kodlanmış değerler ile $(TargetDir)$(TargetName)$(TargetExt) yapılandırmalarında değiştirmektir. Bu işe yaramazsa, buradan özelleştirme, veya bu değerleri ayarlandığı genel bölümündeki özelliklerini değiştirme (özellikleri **çıkış dizinine**, **hedef adı**, ve  **Hedef uzantısı**. Görüntülemekte olduğunuz özelliği makroları kullanıyorsa, seçebilirsiniz anımsa **Düzenle** yapılan makrosu değişimler ile son dizede gösteren bir iletişim kutusu açmak için aşağı açılan listede. Tüm uygun makroları ve geçerli değerlerini seçerek görüntüleyebilirsiniz **makroları** düğmesi.  
...
```  
 ...
 Hatadan kurtulmak için WINVER güncelleştirerek tanımlamanız **proje özellikleri** şu anda hedeflemek istediğiniz Windows için en düşük sürümünü ayarlama. Çeşitli Windows sürümleri için değerlerinin bir tablosunu bulun [burada](http://msdn.microsoft.com/library/windows/desktop/aa383745.aspx).  
...
```  
 ...
 Bu değişiklik, artık kullanılmayan ostrstream, sorunları sahibiz. Uygun yerini ostringstream alır. Biz ostrstream kodunda değişiklik çok büyük bir başlangıç olarak en az önlemek için bir typedef eklemeyi deneyin.  
...
```  
 ...
 Proje MBCS (çok baytlı karakter kümesi), bunu kullanarak şu anda oluşturulan `char` uygun karakter veri türü. Ancak, daha kolay bir güncelleştirme UTF-16 Unicode kod izin vermek için bu güncelleştiriyoruz `TCHAR`, çözümler için `char` veya `wchar_t` bağlı olarak **karakter kümesi** özelliği proje ayarları MBCS veya Unicode.  
  
 Bazı kod parçalarını güncelleştirilmesi gerekir.  Biz temel sınıf ios ios_base ile değiştirildi ve biz ostream yerine tarafından basic_ostream olduğu\<T >. Biz iki ek tür tanımları ekleyin ve bu bölümde derler.  
...
```  
 ...
 Freeze yöntemi bu basic_stringbuf yok sonraki sorunudur. Freeze yöntemi, bir bellek sızıntısı eski ostream önlemek için kullanılır. Artık, yeni ostringstream kullanıyoruz, gerekmez. Biz dondurmak için çağrı silebilirsiniz.  
  
```cpp  
//rdbuf()->freeze(0);  
```  
  
 Sonraki iki bitişik satırlarında hata. İlk biter kullanma hakkında bir dizeye null sonlandırıcıyı ekleyen eski iostream kitaplığının g/ç işleyici olduğu şeklinde hata verir.  Str yöntemi çıktısını bir const olmayan işaretçisine atanamaz bu hataların ikinci açıklar.  
...
```  
 ...
 Yeni akış Kitaplığı'nı kullanarak, sona erer gerekli değildir, böylece satır kaldırılabilir null ile sonlandırılmış dize her zaman olduğu. Bir dizeyi karakter dizisine bir işaretçi Str() döndürmüyor Şimdi ikinci sorun için sorunu olan; std::string türü döndürür. İkinci çözümü için LPCSTR türünü değiştirin ve işaretçiyi istemek için c_str() yöntemi kullanmaktır.  
...
```  
 ...
 Makro `MOUT` çözümler \*türünde bir nesne olan g_pmout `mstream`. Mstream sınıfı standart çıkış dizesi sınıfından türetilir `std::basic_ostream<TCHAR>.` ancak biz, işlecin aşırı yükleme çözünürlüğü Unicode dönüştürme için hazırlanırken yerleştirecek _T dize sabit değeri, geçici bir çözüm ile << şu hatayla başarısız oluyor İleti:  
...
```  
 ...
 Çok sayıda işleci vardır << tanımları bu tür bir hatayı göz korkutucu olabilir. Daha yakından baktığımızda kullanılabilir aşırı çoğunu ilgisiz ve daha yakından adresindeki görünümlü görebiliriz `mstream` sınıf tanımının çağrıldığında bu durumda düşünüyoruz aşağıdaki işlevi belirledik.  
...
```  
 ...
 Dize sabit değeri türüne sahip olduğundan değil adlı sebebi `const wchar_t[10]` , uzun bir hata iletisinin son satırından görebileceğiniz gibi bu nedenle sabit olmayan işaretçisine dönüştürme değil otomatik. LPCTSTR daha uygun parametre türü, bu nedenle ancak bu işleç giriş parametresi değiştirmemeniz gerekir (`const char*` MBCS derlenirken ve `const wchar_t*` Unicode olarak), LPTSTR değil (`char*` MBCS derlenirken ve `wchar_t*` olarak Unicode). Bu değişikliği yapmak bu hatayı düzeltir.  
...
```  
 ...
 Bu makro tanımına gitme, ' % s'işlevi OnNcHitTest başvurduğu görüyoruz.  
...
```  
 ...
 Üye işlevi türleri için işaretçi uyuşmazlık ile yapmak sorun vardır. Bu geçerli bir temel ile türetilmiş dönüştürme olduğundan sorun dönüştürme CHotLinkCtrl bir sınıf türü olarak CWnd sınıfı türü değil. Dönüş türü bir sorundur: UINT vs. LRESULT. UINT bu türe dönüştürme değil, bir 64-bit işaretçi ya da hedef ikili türüne bağlı olarak bir 32 bit işaretçi olan LONG_PTR LRESULT çözümler. Bu çok sayıda ileti eşlemesi yöntemlerin dönüş türü UINT Visual Studio 2005 LRESULT için 64 bit uyumlulukla değişiklikleri bir parçası olarak değiştiğinden, 2005'ten önce yazılmış kod yükseltirken nadir değildir. Dönüş türü biz LRESULT için tek tek aşağıdaki kodda UINT değiştirin:  
...
```  
 ...
 Bu işlev CWnd'den türetilen tüm farklı sınıflardaki yaklaşık on oluşumlarını olduğundan, kullanmanız yararlı **Tanıma Git** (klavye: F12) ve **bildirimine gidin** (klavye: Ctrl + F12) olduğunda İmleç işlevi bu bulun ve gitmek için düzenleyicide gelen onlara **sembol Bul'u** araç penceresi. **Tanımı Git** genellikle daha iki yararlıdır. **Bildirim Git** tanımlama dışında Bul bildirimleri sınıf bildiriminin, arkadaş sınıf bildirimleri gibi veya ileri başvuru.  
...
```  
 ...
 Sorun CWnd::OnActivateApp ikinci parametresi için DWORD HTASK değiştirilmiş olmasıdır. Bu değişiklik, Visual Studio, Visual Studio .NET 2002 sürümünde oluştu.  
...
##  <a name="compiler_warnings"></a> 10. adımı. Derleyici uyarılarını adresleme  
 Uyarıların tam bir listesini almak için yapmanız bir **Rebuild All** uyarı raporları yalnızca geçerli aldığından çözüm yerine normal bir derleme, yalnızca emin olmak için her şey daha önce derlenmiş, derlenecek derleme. Başka bir sorunuz uyarı düzeyine kabul edin veya daha yüksek bir uyarı düzeyi kullanmayı ' dir.  Kod, özellikle de eski kod taşırken daha yüksek bir uyarı düzeyi kullanarak uygun olabilir.  Varsayılan uyarı düzeyini ile başlatın ve ardından tüm uyarıları almak için uyarı düzeyini artırmak isteyebilirsiniz. /Wall kullanırsanız, bazı uyarılar sistemde üst bilgi dosyaları almak için çoğu kişi, sistem üstbilgileri için uyarıları almadan kendi kodlarına çoğu uyarıları almak için/W4 kullanın. Uyarıları hata olarak görünmesini istiyorsanız, /WX seçeneği ekleyin. Bu ayarlar, Proje Özellikleri iletişim kutusu C/C++ bölümünde bulunur.  
  
 CSpyApp sınıftaki yöntemlerin biri artık desteklenmeyen bir işlev hakkında bir uyarı üretir.  
...
```  
 ...
 Artık gerekli olmadığında SetDialogBkColor, herhangi bir referansı silmek için gerekli tek değişiklik şekilde WM_CTLCOLORDLG ileti Spy ++ kod içinde zaten işlenmiş.  
...
```  
 ...
 İşletim sistemi sürümü sorgu CSpyApp sınıfı yöntemleri vardır: IsWindows9x ve IsWindows4x IsWindows5x. İyi bir başlangıç noktası şu ana kadar bu eski bir uygulama tarafından kullanılan teknolojilerden olarak Windows NT 5 endişe (Windows 7 ve üzeri) desteklemek istediğiniz Windows sürümlerini kapatmak için tüm edileceğidir. Bu yöntemlerin kullandığı eski işletim sistemlerini kısıtlamalarla dağıtılacak yoktu. Bu nedenle bu yöntemler, diğerleri için TRUE ve FALSE IsWindows5x için döndürülecek değiştirdik.  
...
```  
 ...
 M_bStdMouse bildirimi bir bit alanı olduğunu gösterir.  
...
```  
 ...
 Bu kod, Visual c++'ta yerleşik bool türü destekleniyordu önce yazılmıştır. Bu kodda, BOOL int için bir typedef oluştu. Tür int imzalı bir türü ve bit imzalı int bit alanından mantıksal karşılaştırmaya int türü, 0 veya -1, büyük olasılıkla ne hedeflenen temsil eden olarak yorumlanabilecek şekilde ilk bit imza biti, kullanılacak gösterimidir.  
 ...
 Standart türü bool boyunca yararlı olacaktır, merak ediyor. BOOL türü gibi eski kod desenleri birçoğu BOOL bool yerleşik tür değiştirme böyle bir değişiklik yalnızca bir örneği başlangıçta çalışan kodunuzu aldıktan sonra yapılması göz önünde bulundurun, bu nedenle, daha sonra standard C++ içinde çözülen sorunları çözmek için invented yeni sürümde.  
...
```  
 ...
 Bu yeterince zararsız gibi görünüyor, ancak/W4 ve wx kümesiyle temiz bir derlemesini istedik olduğundan, biz yalnızca out değişkeni adı, okunabilirlik açısından çıkmadan yorum yaptı.  
  
```cpp  
virtual void OnSelectTab(int /*nTab*/) {};  
```  
  
 Aldığımız diğer uyarılar genel kod Temizleme için yararlıdır. Örtük dönüştürmelerine sayıda `int` veya `unsigned int` için `WORD` (Bu değer için bir typedef `unsigned short`). Bu, veri kaybını içerir. Bir atamanın ekledik `WORD` bu gibi durumlarda.  
...
```  
 ...
 Bir mfc eski statik kitaplık sürümünü giriş bağlayıcı üzerinde dahil olduğundan LNK1181 gerçekleşir. Artık biz dinamik olarak MFC'ye bu yana yalnızca giriş özelliği proje özelliklerini bağlayıcı bölümünde tüm MFC statik kitaplıklar kaldırmak yüzden bu gerekli değildir. Bu proje/nodefaultlıb seçeneği de kullanıyor ve bunun yerine, tüm kitaplık bağımlılıkları listeler.  
...
```  
 ...
 Bazı yerlerde kodu zaten kullanmakta olduğunuz `TCHAR`, sonunda Unicode destekleme görünüşe göre olasılığına. Bazı değildi. Biz Aranan örneklerini `CHAR`, char ve bunların TCHAR ile değiştirilen çoğu için bir typedef olduğu. Ayrıca, incelemiştik `sizeof (CHAR)`. Biz herhangi bir zamanda değiştirildi `CHAR` için `TCHAR`, genellikle değiştirmek vardı `sizeof(TCHAR)` olduğundan, bu genellikle bir dizedeki karakter sayısını belirlemek için kullanıldı. Bu durumda dikkat biraz ödeme değer, bu nedenle burada yanlış türde kullanarak bir derleyici hatası üretmez.  
...
```  
 ...
 _Tcscpy işlevi, bir dizeyi kopyalama için TCHAR strcpy işlevi olan kullanılan olsa da, ayrılan arabelleği, bir `char` arabellek. Bu, kolayca TCHAR için değiştirilir.  
...
```  
 ...
 Benzer şekilde değiştirdik `LPSTR` (uzun dize işaretçisi) ve `LPCSTR` (uzun sabit dize işaretçisi) için `LPTSTR` (uzun TCHAR dize işaretçisi) ve `LPCTSTR` (uzun sabit TCHAR dize işaretçisi) sırasıyla tarafından garanti edildiği zaman bir Derleyici Hatası. Her durumda ayrı ayrı incelenmesi çünkü bu tür değişiklik genel arama ve değiştirme, kullanarak yapmamak seçtik. Bazı durumlarda, `char` sürümü istiyordu, ne zaman belirli Windows bir son eke sahip Windows yapıları kullandığınız iletilerini işleme gibi. Windows API ' % s'soneki bir ASCII veya ANSI anlamına gelir (ve MBCS için de geçerlidir) ve geniş karakterler ya da UTF-16 Unicode W soneki anlamına gelir. Bu adlandırma desenine Windows üstbilgileri kullanılır ancak biz yalnızca bir MBCS sürümde zaten tanımlanmış olan bir işlevin bir Unicode sürümü gerekiyordu, ayrıca, Spy ++ kodda izlenen.  
  
 Doğru gideren bir sürümünü kullanmak için bir tür değiştirilecek vardı bazı durumlarda (örneğin WNDCLASSA yerine WNDCLASS).  
  
 Çoğu durumda biz bir Win32 API GetClassName (yerine GetClassNameA) gibi genel sürümünü (makro) kullanmanız gerekiyordu. İleti işleyicisi switch deyimi içinde bazı iletilerin bu durumda MBCS veya Unicode özel, çünkü biz W belirli işlevler ve genel olarak adlandırılan işlevlerin A ile'değiştirildi ve makro eklenen MBCS sürüm açıkça çağırmak için kodu değiştirmenizi vardı doğru A gideren genel adı veya UNICODE olup tanımlananlara göre W adı.  _UNICODE, tanımlama moduna olduğunda kod pek çok bölümünün W sürümü artık sürümünü ne istiyordu olsa bile seçilir.  
  
 Burada gerçekleştirilecek özel eylemler b birkaç yerde mevcuttur. Daha yakından WideCharToMultiByte veya MultiByteToWideChar herhangi bir kullanımını gerektirebilir. Burada WideCharToMultiByte kullanılan bir örnek aşağıda verilmiştir.  
...
```  
 ...
 Bunu ele almak için bir yazı tipinin adını temsil eden bir CString strFace iç arabelleğine geniş karakter dizesi kopyalamak için Bunun yapılmasının nedeni olduğunu anlamak vardı. Bu durumda ekledik bir #ifdef için bu biraz farklı kod olduğu geniş karakter CString dizeleri çok baytlı CString dizeleri için gereklidir.  
...
```  
 ...
 Elbette, yerine wcscpy biz gerçekten wcscpy_s, daha güvenli bir sürümü kullanmanız gerekir. Sonraki bölümde bu giderir.  
  
 Yaptığımız çalışmaların üzerinde bir denetimi biz karakter kümesi kullan çok baytlı karakter kümesi için sıfırlama ve kullanarak MBCS ve Unicode kod hala derlediğinden emin olmak gerekir. Deyin needless için bir tam test geçişi bu değişikliklerden sonra znovu uygulama yürütülmelidir.  
...
##  <a name="porting_to_secure_crt"></a> 12. adımı. Güvenli CRT kullanılacak bağlantı noktası oluşturma  
 CRT işlevlerinin güvenli sürümleri (_Yanları sonekine sahip sürümler) kullanmak için kodu taşıma sonraki. Bu durumda, genel işlev _Yanları sürümüyle değiştirin ve ardından, genellikle, gerekli ek arabellek boyutu parametreleri ekleyin stratejisidir. Boyut biliniyorsa bu yana çoğu durumda bu oldukça basittir. Burada boyutu hemen kullanılabilir değil, diğer durumlarda, bu CRT işlevi kullanarak işlev için ek parametreler eklemek gereklidir veya belki de hedef arabellek kullanımını inceleyin ve limitlerdir uygun boyutu bakın.  
...
```  
 ...
 Arabellek bir dizi olduğunda, tanımladığınızda, dizi türünden çıkarılan bir ham işaretçi yerine boyutuna ve sağlaması gerek kalmadan boyutu parametresi kullanılan. Bu, kodu yeniden yazma karmaşıklığı kesmek için yardımcı olur. Yine de işlev adı _Yanları sürümle gerekir, ancak genellikle göre arama yapılabilir ve işlem değiştirin.  
  
 Bazı işlevler dönüş değerlerini değiştirildi. Örneğin, _itoa_s (ve _itow_s ve makro _itot_s) bir hata kodu (errno_t) yerine bir dize döndürür. Bu nedenle bu gibi durumlarda, çağrı _itoa_s ayrı bir satır üzerine taşıyın ve arabellek tanımlayıcısıyla değiştirin gerekir.  
  
 Bazı genel örnekleri: memcpy_s için geçiş yaparken memcpy için, sık kopyalanan yapının boyutunu ekledik. Benzer şekilde, çoğu dizeleri ve arabellekleri için dizi veya arabellek boyutunu kolayca öğesinden bildirim arabellek veya arabellek ilk ayrıldığı bulma tarafından belirlenir. Bazı durumlarda, büyük bir arabellek gerçekten edinilebilir ve bu bilgileri değişiklik yaptığınız, ek bir parametre olarak eklenmelidir ve çağıran kod şu şekilde değiştirilmelidir: işlev kapsamında kullanılabilir değilse, belirlemeniz gerekir. bilgi sağlar.  
...
##  <a name="deprecated_forscope"></a> 13. adım. /ZC:forScope-kullanım dışıdır  
 Visual C++ 6.0 bu yana derleyici döngü kapsamı için bir döngü içinde bildirilmiş değişkenlerin kapsamını sınırlar geçerli standardına uyar. Derleyici seçeneği [/ZC: forscope](../build/reference/zc-forscope-force-conformance-in-for-loop-scope.md) (**döngü kapsamında uyumluluğu zorla** proje özelliklerinde) Bu hata olarak bildirilen olup olmadığını denetler. Biz kodumuz uyumlu olacak şekilde güncelleştirin ve hemen döngünün dışında bildirimleri ekleyin. Kod değişikliklerini yapmaktan kaçınmak için bu ayarı için C++ proje özelliklerini dil bölümünde değiştirebilirsiniz **yok (/Zc:forScope-)**. Ancak, unutmayın **/Zc:forScope-** Visual C++'ın standart uyacak şekilde değiştirmek için kodunuzu gerekir, böylece sonunda gelecek sürümlerde kaldırılacak.  
