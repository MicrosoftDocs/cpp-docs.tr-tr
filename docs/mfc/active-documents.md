...
-   Her biri yerinde etkinleştirme özelliğine sahip bir veya daha fazla görünümleri destekler. Diğer bir deyişle, görünümleri arabirimini desteklemelidir `IOleDocumentView` arabirimler yanı sıra `IOleInPlaceObject` ve `IOleInPlaceActiveObject` (kapsayıcının kullanarak `IOleInPlaceSite` ve `IOleInPlaceFrame` arabirimleri).  
