---
description: "Daha fazla bilgi edinin: TN054: MFC DAO sınıflarını kullanırken DAO 'YU doğrudan çağırma"
...
MFC 'nin DAO sarmalanması, pek çok ayrıntıyı işleyerek DAO kullanımını daha kolay hale getirir, böylece çok az şey konusunda endişelenmenize gerek kalmaz. Bu, OLE başlatma, DAO nesnelerinin oluşturulması ve yönetimi (özellikle koleksiyon nesneleri), hata denetimi ve kesin belirlenmiş, daha basit bir arabirim ( **değişken** veya `BSTR` bağımsız değişken olmadan) sağlama içerir. Doğrudan DAO çağrıları yapabilir ve yine de bu özelliklerden yararlanabilirsiniz. Tüm kodunuzun `Release` doğrudan DAO çağrıları tarafından oluşturulan herhangi bir nesne için çağrı yapması gerekir ve MFC  'nin dahili olarak güvenebileceherhangi bir arabirim işaretçilerinden hiçbirini değiştirmez. Örneğin,  `CDaoRecordset` *Tüm* iç sonuçları anlamadığınız müddetçe açık bir nesnenin m_pDAORecordset üyesini değiştirmeyin. Ancak, alanlar koleksiyonunu almak için *m_pDAORecordset* arabirimini kullanarak doğrudan DAO 'yu çağırabilirsiniz. Bu durumda *m_pDAORecordset* üye değiştirilmez. `Release`Nesne ile işiniz bittiğinde alanlar koleksiyonu nesnesini çağırmanız yeterlidir.
...
DAO arabirimlerinin birçok yöntemi için, bir `BSTR` nesneyi (OLE Otomasyonu 'nda kullanılan length-önekli dize) değiştirmeniz gerekecektir. `BSTR`Nesnesi genellikle **değişken** veri türü içinde kapsüllenir. MFC sınıfının `COleVariant` kendisi **değişken** veri türünden devralır. Projenizi ANSI veya Unicode için oluşturup yapılandırdığınıza bağlı olarak, DAO arabirimleri ANSI veya Unicode `BSTR` s döndürür. V_BSTR ve V_BSTRT iki makro, DAO arabiriminin beklenen türden birini aldığından emin olmak için yararlıdır `BSTR` .

V_BSTR, a 'nın *bstrVal* üyesini ayıklar `COleVariant` . Bu makro genellikle bir DAO arabiriminin bir yöntemine bir öğesine geçiş yapmanız gerektiğinde kullanılır `COleVariant` . Aşağıdaki kod parçası, V_BSTR makrodan faydalanan DAO DAOUser arabiriminin iki yöntemi için bildirimleri ve gerçek kullanımı gösterir:
...
`VT_BSTRT`Yukarıdaki oluşturucuda belirtilen bağımsız değişkenin, uygulamanızda BIR `COleVariant` `BSTR` `COleVariant` ANSI sürümü ve `BSTR` uygulamanızın Unicode sürümü için Unicode BIR Unicode sürümü oluşturup bir ANSI olacağını güvence altına alın. Bu, DAO 'nun beklediği şeydir.

V_BSTRT diğer makro, derleme türüne (ANSI veya Unicode) bağlı olarak bir ANSI veya Unicode *bstrVal* üyesini ayıklar `COleVariant` . Aşağıdaki kod, ' dan ' a değerin ' a nasıl ayıklanacağını göstermektedir `BSTR` `COleVariant` `CString` .
...
V_BSTRT makrosu, içinde depolanan diğer türleri açmaya yönelik diğer tekniklerle birlikte `COleVariant` DAOVIEW örneğinde gösterilmiştir. Özellikle, bu çeviri `CCrack::strVARIANT` yönteminde gerçekleştirilir. Mümkün olduğunda bu yöntem değerini bir `COleVariant` örneğine çevirir `CString` .

## <a name="simple-example-of-a-direct-call-to-dao"></a>DAO 'ya doğrudan çağrının basit örneği

Temel alınan DAO koleksiyon nesnelerinin yenilenmesi gerektiğinde durumlar ortaya çıkabilir. Normalde, bu gerekli değildir, ancak gerekirse basit bir yordamdır. Bir koleksiyonun yenilenmesi gerekebilen bir örnek, birden çok kullanıcı yeni TableDefs oluşturma ile çok kullanıcılı bir ortamda çalışırken. Bu durumda, TableDefs koleksiyonunuz eski hale gelebilir. Koleksiyonu yenilemek için, yalnızca `Refresh` belirli bir koleksiyon nesnesinin yöntemini çağırmanız ve hataları kontrol etmeniz gerekir:
