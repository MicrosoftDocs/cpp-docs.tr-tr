---
description: 'Hakkında daha fazla bilgi edinin: var olan bir ActiveX denetimini yükseltme'
...
Ayrıca, [ActiveX denetimleri: iyileştirme](../mfc/mfc-activex-controls-optimization.md)bölümünde açıklandığı gibi iyileştirmeler de ekleyebilirsiniz. Bilinen adlar, [Internet 'Teki ActiveX denetimlerinde](../mfc/activex-controls-on-the-internet.md)açıklandığı gibi, özellikleri ve büyük Blobları zaman uyumsuz olarak indirmek için kullanılabilir.

## <a name="packaging-code-for-downloading"></a><a name="_core_packaging_code_for_downloading"></a> Indirme için paketleme kodu

Bu konu hakkında daha fazla bilgi için bkz. [paketleme ActiveX denetimleri](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa751974%28v%3dvs.85%29).

### <a name="the-codebase-tag"></a>CODEBASE etiketi

ActiveX denetimleri etiketi kullanılarak Web sayfalarına katıştırılır `<OBJECT>` . `CODEBASE`Etiketin parametresi, `<OBJECT>` denetimin indirileceği konumu belirtir. `CODEBASE` , bir dizi farklı dosya türünü başarıyla işaret edebilir.
...
Bu çözüm yalnızca denetimin. ocx dosyasını indirir ve istemci makinesine zaten yüklü olan destekleyen dll 'Leri gerektirir. Internet Explorer Visual C++ denetimleri için destekleme dll 'Leriyle birlikte geldiği için, bu, Visual C++ ile oluşturulmuş Internet Explorer ve MFC ActiveX denetimleri için çalışır. Bu denetimi görüntülemek için ActiveX denetimi özellikli başka bir Internet tarayıcısı kullanılırsa bu çözüm çalışmaz.
...
Tarafından işaret edilen dolap dosyası, `CODEBASE` ActiveX denetiminizin. ocx dosyasını ve yüklemesini denetlemek için bir. inf dosyasını içermelidir. Denetim dosyanızın adını ve bir. inf dosyasını belirterek dolap dosyasını oluşturursunuz. Bu dolap dosyasında sistemde zaten mevcut olabilecek bağımlı dll 'Leri eklemeyin. Örneğin, MFC DLL 'Leri ayrı bir dolap dosyasında paketlenir ve denetleyen. inf dosyası tarafından başvurulur.
...
### <a name="the-object-tag"></a>\<OBJECT>Etiketi

Aşağıdaki örnek, `<OBJECT>` MFC Spındial örnek denetimini paketlemek için etiketinin kullanımını gösterir.
...
Bu durumda spindial.cab, spındial. ocx ve spındial. inf olmak üzere iki dosya içerir. Aşağıdaki komut, dolap dosyasını oluşturacak:
...
Buraya, `#Version` BIR cab dosyası ile belirtilen bilgilerin, etiketin *ClassID* parametresi tarafından belirtilen denetime uygulandığını unutmayın `<OBJECT>` .

Belirtilen sürüme bağlı olarak, denetiminizin indirilmesini zorla indirebilirsiniz. `OBJECT` *Kod temeli* parametresi dahil olmak üzere etiketin tüm özellikleri IÇIN bkz. W3C başvurusu.

## <a name="marking-a-control-safe-for-scripting-and-initializing"></a><a name="_core_marking_a_control_safe_for_scripting_and_initializing"></a> Bir denetimi betik oluşturma ve başlatma için güvenli olarak işaretleme
...
Anahtarlarınızı aşağıdaki sınıf KIMLIĞIYLE ilişkilendirmek için aşağıdakine benzer girdileri ekleyin (denetiminizin sınıf KIMLIĞINI yerine `{06889605-B8D0-101A-91F1-00608CEAD5B3}` ).
...
## <a name="licensing-issues"></a><a name="_core_licensing_issues"></a> Lisanslama sorunları
...
Internet Explorer çalıştıran bilgisayarın denetimi kullanmak üzere lisansı yoksa, bir HTML sayfasında lisanslı bir ActiveX denetimi düzgün yüklenmez. Örneğin, Visual C++ kullanılarak oluşturulmuş bir denetim oluşturulmuşsa, denetimi kullanan HTML sayfası, denetimin oluşturulduğu bilgisayara düzgün şekilde yüklenir, ancak lisanslama bilgileri dahil edilmedikleri takdirde farklı bir bilgisayara yüklenmeyecektir.
...
Lisanslı olmayan bir makinede bir HTML sayfasında lisanslı bir denetim kullanmak için, bir lisans paketi dosyası (LPK) oluşturmanız gerekir. LPK dosyası, HTML sayfasındaki lisanslı denetimlerin çalışma zamanı lisanslarını içerir. Bu dosya, ActiveX SDK ile birlikte gelen LPK_TOOL.EXE ile oluşturulur.

#### <a name="to-create-an-lpk-file"></a>Bir LPK dosyası oluşturmak için

1. LPK_TOOL.EXE, denetimini kullanmak için lisanslanan bir bilgisayarda çalıştırın.
...
1. \<OBJECT>Lisans Yöneticisi etiketinden sonra lisanslı denetiminizin etiketini ekleyin.
...
1. \<EMBED>Npusula ActiveX eklentisini kullanıyorsanız, LPK dosyanız için özniteliği ekleyin.

   Denetiminiz, diğer etkin etkin tarayıcılarda görüntülenebilir — Örneğin, Npusula ActiveX eklentisini kullanan Netscape —, \<EMBED> sözdizimini aşağıda gösterildiği gibi eklemeniz gerekir.
...
## <a name="signing-code"></a><a name="_core_signing_code"></a> İmzalama kodu
...
## <a name="managing-the-palette"></a><a name="_core_managing_the_palette"></a> Paleti yönetme
...
Bir denetim `OnGetColorSet` , paleti çizmek üzere bir renk kümesi döndürmek için geçersiz kılınabilir. Kapsayıcılar, bir denetimin palet duyarlı olup olmadığını anlamak için dönüş değerini kullanır.

OCX 96 yönergeleri altında, bir denetim her zaman arka planda paletini sağlamalıdır.

Çevresel palet özelliğini kullanmayan eski kapsayıcılar WM_QUERYNEWPALETTE ve WM_PALETTECHANGED iletileri gönderir. Bir denetim, `OnQueryNewPalette` Bu iletileri geçersiz kılabilir ve `OnPaletteChanged` işleyebilir.

## <a name="internet-explorer-browser-safety-levels-and-control-behavior"></a><a name="_core_internet_explorer_browser_safety_levels_and_control_behavior"></a> Internet Explorer tarayıcı güvenlik düzeyleri ve denetim davranışı
