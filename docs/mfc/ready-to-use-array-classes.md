---
title: "Kullanıma hazır dizi sınıfları | Microsoft Docs"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
ms.technology: cpp-windows
ms.tgt_pltfrm: 
ms.topic: article
f1_keywords: vc.classes.array
dev_langs: C++
helpviewer_keywords:
- arrays [MFC], classes
- collection classes [MFC], arrays
- classes [MFC], array
ms.assetid: fdeabf95-2fe7-43a8-8f88-d954133caf52
caps.latest.revision: "10"
author: mikeblome
ms.author: mblome
manager: ghogen
ms.workload: cplusplus
ms.openlocfilehash: 03046468fba206d01f7bd57273588a1c2c8c1000
ms.sourcegitcommit: 8fa8fdf0fbb4f57950f1e8f4f9b81b4d39ec7d7a
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 12/21/2017
---
# <a name="ready-to-use-array-classes"></a>Kullanıma Hazır Dizi Sınıfları
Kullanıma hazır dizi sınıfları sınıflarıdır.  
  
 [CLongBinary](../mfc/reference/cbytearray-class.md)  
 Depolar türündeki öğeler **bayt** dizi.  
  
 [CDWordArray](../mfc/reference/cdwordarray-class.md)  
 Depolar türündeki öğeler `DWORD` dizi.  
  
 [CObArray](../mfc/reference/cobarray-class.md)  
 Sınıf nesnelerine işaretçiler depolar `CObject` veya türetilen sınıfların nesnelerini `CObject` dizi.  
  
 [CPtrArray](../mfc/reference/cptrarray-class.md)  
 Depolar işaretçileri `void` (genel işaretçileri) dizi.  
  
 [CUIntArray](../mfc/reference/cuintarray-class.md)  
 Depolar türündeki öğeler **UINT** dizi.  
  
 [CWordArray](../mfc/reference/cwordarray-class.md)  
 Depolar türündeki öğeler **WORD** dizi.  
  
 [CStringArray](../mfc/reference/cstringarray-class.md)  
 Depoları `CString` bir dizi nesneleri.  
  
## <a name="see-also"></a>Ayrıca Bkz.  
 [Sınıfa genel bakış](../mfc/class-library-overview.md)

