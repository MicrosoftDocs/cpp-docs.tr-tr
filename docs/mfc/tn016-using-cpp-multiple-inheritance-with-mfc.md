...
```  
 ...
 Bu durumda `CObject` iki kez bulunur. Bu herhangi bir referans belirsizliğini ortadan kaldırmak için bir yol gerektiği anlamına gelir `CObject` yöntemleri ya da işleçler. `operator new` Ve [delete işleci](../mfc/reference/cobject-class.md#operator_delete) disambiguated gereken iki işleçler. Başka bir örnek olarak, aşağıdaki kod, derleme zamanında bir hataya neden olur:  
...
## <a name="reimplementing-cobject-methods"></a>CObject yöntemleri reimplementing  
 İki veya daha yeni bir sınıf oluştururken sahip `CObject` türetilmiş temel sınıfları, yeniden uygulamalı `CObject` diğer kişilerin kullanmasını istediğiniz yöntemleri. İşleçler `new` ve `delete` zorunludur ve [dökümü](../mfc/reference/cobject-class.md#dump) önerilir. Aşağıdaki örnek reimplements `new` ve `delete` işleçler ve `Dump` yöntemi:  
...
## <a name="cobjectiskindof-and-run-time-typing"></a>CObject::IsKindOf ve çalışma zamanı yazma  
 MFC'de tarafından desteklenen çalışma zamanı yazarak mekanizması `CObject` makrolar kullanan `DECLARE_DYNAMIC`, `IMPLEMENT_DYNAMIC`, `DECLARE_DYNCREATE`, `IMPLEMENT_DYNCREATE`, `DECLARE_SERIAL` ve `IMPLEMENT_SERIAL`. Bu makroları güvenli downcasts güvence altına almak için bir çalışma zamanı tür denetimi gerçekleştirebilir.  
  
 Bu makroları yalnızca tek bir temel sınıf desteklemek ve Çarp devralınan sınıflar için sınırlı bir şekilde çalışır. Belirttiğiniz temel sınıf `IMPLEMENT_DYNAMIC` veya `IMPLEMENT_SERIAL` ilk (veya en solundaki) temel sınıf olmalıdır. Bu yerleştirme tür en solundaki için temel sınıfı yalnızca denetleme olanak sağlar. Çalışma zamanı tür sistemi ek temel sınıfları hakkında hiçbir şey bilirsiniz. Aşağıdaki örnekte, çalışma zamanı sistemleri ne yapacağını yazın karşı denetimi `CFrameWnd`, ancak hiçbir şey hakkında bilirsiniz `CObList`.  
