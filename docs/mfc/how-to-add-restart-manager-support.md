...
1. Emin `InitInstance` uygulamanızı yöntemini çağırır, üst `InitInstance` yöntemi: [CWinApp::InitInstance](../mfc/reference/cwinapp-class.md#initinstance) veya `CWinAppEx::InitInstance`. `InitInstance` Yöntemdir denetlemek için sorumlu *m_dwRestartManagerSupportFlags* parametresi.
