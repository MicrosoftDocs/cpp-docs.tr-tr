...
1. Windows tarih ve Saat Seçici denetimini oluşturan ve ekler bu üye işlevi çağrısı `CDateTimeCtrl` nesne.
...
|Parametre|Açıklama|
|---------------|-----------------|
|*pDateTimePickerInfo*|[out] Bir işaretçi bir [DATETIMEPICKERINFO](/windows/desktop/api/commctrl/ns-commctrl-tagdatetimepickerinfo) yapısı geçerli tarih ve Saat Seçici denetiminin açıklamasını alır.<br /><br /> Bu yapı ayırma için çağıran sorumludur. Ancak, bu yöntem başlatır *cbSize* yapı üyesi.|
...
*pTimeDest*<br/>
Bir işaretçi [SYSTEMTIME](/windows/desktop/api/minwinbase/ns-minwinbase-systemtime) sistem saat bilgilerini almak için yapısı. NULL olmamalıdır.
...
|Parametre|Açıklama|
|---------------|-----------------|
|*psize*|[out] İşaretçi bir [BOYUTU](/windows/desktop/api/windef/ns-windef-tagsize) denetimi için ideal boyutu içeren yapısı.|
...
|Parametre|Açıklama|
|---------------|-----------------|
|*dwStyle*|[in] Ay takvim denetimi stilleri bit düzeyinde birleşimi (veya) olan yeni aylık takvim denetimi stili. Daha fazla bilgi için [aylık takvim denetimi stilleri](/windows/desktop/Controls/month-calendar-control-styles).|
...
*pTimeNew*<br/>
İkinci Sürüm yukarıda, işaretçi bir [CTime](../../atl-mfc-shared/reference/ctime-class.md) olduğu denetim ayarlanacak saati içeren bir nesne. Bir işaretçi yukarıdaki üçüncü sürümünde bir [SYSTEMTIME](/windows/desktop/api/minwinbase/ns-minwinbase-systemtime) için Denetim ayarlanacak zaman içeren yapısı.
