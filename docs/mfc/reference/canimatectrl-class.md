...
|[CAnimateCtrl:: Create](#create)|Bir animasyon denetimi oluşturur ve bunu bir `CAnimateCtrl` nesneye ekler.|
...
İletişim kutusu içinde veya `CAnimateCtrl` iletişim kutusu düzenleyicisini kullanarak bir iletişim kutusu kaynağından bir nesne oluşturursanız, Kullanıcı iletişim kutusunu kapattığında otomatik olarak yok edilir.

Bir pencere içinde bir `CAnimateCtrl` nesne oluşturursanız, bunu yok etmeniz gerekebilir. `CAnimateCtrl` Nesneyi yığında oluşturursanız, otomatik olarak yok edilir. `CAnimateCtrl` Nesneyi **Yeni** işlevi kullanarak yığında oluşturursanız, nesneyi yok etmek için **silme** çağrısı yapmanız gerekir. ' Dan `CAnimateCtrl` yeni bir sınıf türetirsiniz ve bu sınıftaki herhangi bir bellek ayırdıysanız, ayırmaları `CAnimateCtrl` atmak için yıkıcıyı geçersiz kılın.

Kullanma `CAnimateCtrl`hakkında daha fazla bilgi için bkz. [Controls](../../mfc/controls-mfc.md) ve [CAnimateCtrl kullanma](../../mfc/using-canimatectrl.md).
...
*pParentWnd*<br/>
Animasyon denetiminin ana penceresini (genellikle a `CDialog`) belirtir. NULL olmaması gerekir.
...
İki adımda oluşturursunuz `CAnimateCtrl` . İlk olarak, oluşturucuyu çağırın ve ardından, animasyon `Create`denetimini oluşturan ve bunu `CAnimateCtrl` nesnesine ekleyen çağırın.
...
Animasyon denetiminizin genişletilmiş Windows stillerini kullanmak istiyorsanız yerine `Create` [CreateEx](#createex) çağırın.
...
##  <a name="isplaying"></a>CAnimateCtrl:: ısçalıyor
...
*lpszFileName*<br/>
Bir `CString` nesne veya AVI dosyasının adını ya da bir avı kaynağının adını içeren null ile sonlandırılmış bir dize işaretçisi. Bu parametre NULL ise sistem, varsa animasyon denetimi için daha önce açılmış olan avı klibini kapatır.
...
İş yazışmanız yürütülmeye devam ederken animasyon denetimi klibi arka planda oynatacak. Animasyon denetiminin stili varsa `ACS_TRANSPARENT` , AVI klibi animasyon klibinde belirtilen arka plan rengi yerine saydam bir arka plan kullanılarak yürütülür.
