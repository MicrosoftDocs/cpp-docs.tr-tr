...
Aşağıdaki örnek nasıl kullanılacağını gösterir `SetImage` yönteminde `CMFCDesktopAlertWndButton` sınıfı. Bu kod parçacığı parçasıdır [Masaüstü uyarı gösterim örneği](../../overview/visual-cpp-samples.md).
