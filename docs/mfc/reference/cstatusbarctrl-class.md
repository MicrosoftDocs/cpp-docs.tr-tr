...
|Ad|Açıklama|
|----------|-----------------|
|[CStatusBarCtrl:: Create](#create)|Bir durum çubuğu denetimi oluşturur ve bunu bir nesneye ekler `CStatusBarCtrl` .|
...
|[CStatusBarCtrl:: SetSimple](#setsimple)|Bir durum çubuğu denetiminin basit metin görüntüleyip görüntülemediğini veya önceki bir çağrısıyla ayarlanan tüm denetim parçalarını görüntülediğini belirtir `SetParts` .|
...
Kullanma hakkında daha fazla bilgi için `CStatusBarCtrl` bkz. [denetimler](../../mfc/controls-mfc.md) ve [CStatusBarCtrl kullanma](../../mfc/using-cstatusbarctrl.md).
...
Bir durum çubuğu denetimi oluşturur ve bunu bir nesneye ekler `CStatusBarCtrl` .
...
*Rect*<br/>
Durum çubuğu denetiminin boyutunu ve konumunu belirtir. Bu, bir [CRect](../../atl-mfc-shared/reference/crect-class.md) nesnesi veya bir [Rect](/windows/win32/api/windef/ns-windef-rect) yapısı olabilir.

*pParentWnd*<br/>
Durum çubuğu denetiminin üst penceresini (genellikle a) belirtir `CDialog` . NULL olmaması gerekir.
...
`CStatusBarCtrl`İki adımda oluşturursunuz. İlk olarak, oluşturucuyu çağırın ve sonra `Create` durum çubuğu denetimini oluşturan ve bunu nesnesine ekleyen çağırın `CStatusBarCtrl` .
...
Genişletilmiş pencere stilleriyle bir durum çubuğu oluşturmak için yerine [CStatusBarCtrl:: CreateEx](#createex) çağırın `Create` .
...
Varsayılan olarak, bu üye işlevi hiçbir şey yapmaz. Bir sahip çizim nesnesi için çizimi uygulamak üzere bu üye işlevini geçersiz kılın `CStatusBarCtrl` .
...
## <a name="cstatusbarctrlgetborders"></a><a name="getborders"></a> CStatusBarCtrl:: Getkenarlýklar
...
*ıpart*\
'ndaki Alınacak simgeyi içeren parçanın sıfır tabanlı dizini. Bu parametre-1 ise, durum çubuğunun bir basit mod durum çubuğu olduğu varsayılır.
...
Aşağıdaki kod örneği, `m_statusBar` geçerli durum çubuğu denetimine erişmek için kullanılan bir değişkenini tanımlar. Bu değişken bir sonraki örnekte kullanılır.
