---
description: 'Daha fazla bilgi edinin: CMFCPropertySheet Sınıfı'
...
|`CMFCPropertySheet::PreTranslateMessage`|[TranslateMessage](/windows/win32/api/winuser/nf-winuser-translatemessage) ve [DispatchMessage](/windows/win32/api/winuser/nf-winuser-dispatchmessage) Windows işlevlerine dağıtılmadan önce pencere iletilerini çevirir. (Geçersiz kılmalar `CPropertySheet::PreTranslateMessage` .)|
...
`CMFCPropertySheet`Sınıfı, sekme iletişim kutusu olarak da bilinen özellik sayfalarını temsil eder. `CMFCPropertySheet`Sınıfı, bir özellik sayfasını çeşitli yollarla gösterebilir.

Uygulamanızda sınıfını kullanmak için aşağıdaki adımları gerçekleştirin `CMFCPropertySheet` :

1. Sınıfından bir sınıf türetirsiniz `CMFCPropertySheet` ve sınıfı adlandırın, örneğin, CMyPropertySheet.
...
1. Bir `CMFCPropertySheet` denetim oluşturun ve metodunu çağırın `DoModal` .
...
Bu yöntem bir ağaç denetiminin yaprağı olarak bir özellik sayfası ekler. Bir özellik sayfası eklemek için, bir `CMFCPropertySheet` nesne oluşturun, *Bak* parametresi olarak ayarlanmış [CMFCPropertySheet:: SetLook](#setlook) metodunu çağırın `CMFCPropertySheet::PropSheetLook_Tree` ve sonra özellik sayfasını eklemek için bu yöntemi kullanın.
...
Ağaç denetimine kategori olarak da adlandırılan yeni bir düğüm eklemek için bu yöntemi kullanın. Bir düğüm eklemek için bir nesne oluşturun, bir `CMFCPropertySheet` nesne oluşturun, *Bak* parametresi olarak ayarlanmış [CMFCPropertySheet:: SetLook](#setlook) metodunu çağırın `CMFCPropertySheet::PropSheetLook_Tree` ve ardından bu yöntemi kullanarak düğümü ekleyin.
...
*aþaðýdaki*<br/>
'ndaki Özellik sayfasının görünümünü belirten sabit listesi değerlerinden biri. Bir özellik sayfası için varsayılan stil `CMFCPropertySheet::PropSheetLook_Tabs` . Daha fazla bilgi için bu konunun açıklamalar bölümündeki tabloya bakın.
