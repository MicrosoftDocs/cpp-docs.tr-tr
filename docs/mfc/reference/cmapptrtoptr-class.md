---
title: CMapPtrToPtr sınıfı | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
## <a name="remarks"></a>Açıklamalar  
 `CMapPtrToPtr` bir araya getirir `IMPLEMENT_DYNAMIC` çalışma zamanı tür erişimi ve için dökme desteklemek için makrosu bir `CDumpContext` nesnesi. Tek tek harita öğelerine (kayan nokta değerlerini), bir dökümü gerekiyorsa, 1 veya daha büyük döküm bağlam derinliği ayarlamanız gerekir.  
  
 İşaretçi işaretçi eşlemeleri getirilemez.  
  
 Zaman bir `CMapPtrToPtr` Nesne silindiğinden veya öğeleri kaldırıldığında, yalnızca işaretçileri kaldırılır, yok oldukları varlıklar.  
  
 Daha fazla bilgi için `CMapPtrToPtr`, makaleye bakın [koleksiyonları](../../mfc/collections.md).  
