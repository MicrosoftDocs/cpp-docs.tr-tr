...
|[COleControl::FireEvent](#fireevent)|Özel bir olay gönderir.|  
...
|[COleControl::InternalGetText](#internalgettext)|Stok resim yazısını veya metin özelliğini alır.|  
...
|[COleControl::SetAppearance](#setappearance)|Stok görünüm özellik değerini ayarlar.|  
...
- **MIDDLE_BUTTON** orta fare düğmesini basılı.  
...
- **OLEDC_PAINTBKGND** kapsayıcısı DC döndürmeden önce arka plan boyama ister. Saydam arka plan sahip bir alan yeniden çizim için DC istediği bir nesne bu bayrak kullanmanız gerekir.  
  
- **OLEDC_OFFSCREEN** nesne ekrana kopyalanacağı bir ekran dışı bitmap halinde işlemek istediği kapsayıcı bildirir. Bir nesne, çizim işlemi gerçekleştirmek üzere olan çok sayıda titreşimi oluşturduğunda bu bayrak kullanmanız gerekir. Kapsayıcı veya bu isteği kabul etmeniz ücretsizdir. Bu bayrak ayarlanmazsa, ancak kapsayıcı geri el gerekir bir ekran DC. Bu seçim gösteren gibi doğrudan ekran işlemler gerçekleştirmek nesneleri sağlar (aracılığıyla bir **XOR** işlemi).  
...
### <a name="return-value"></a>Dönüş Değeri  
 Kapsayıcı için bir işaretçi denetim nesnesi genişletilmiş. Kullanılabilir hiçbir nesnesi yoksa değerdir **NULL**.  
  
 Bu nesne aracılığıyla yönetilebilmesini, `IDispatch` arabirimi. Aynı zamanda `QueryInterface` nesne tarafından sağlanan diğer kullanılabilir arabirimleri elde edilir. Ancak, nesne arabirimleri belirli bir dizi desteklemek için gerekli değildir. Bir kapsayıcının genişletilmiş denetim nesnesi belirli özelliğe bağlı diğer rasgele kapsayıcılara denetiminizin taşınabilirlik sınırlar unutmayın.  
...
### <a name="return-value"></a>Dönüş Değeri  
 Denetim etkinleştirilir ve penceresiz ise döndürür **bu** denetimi şu anda (denetimin kapsayıcı tarafından belirlendiği şekilde) klavye odağı varsa veya **NULL** odağı yoksa.  
  
 Aksi takdirde, döndürür `CWnd` odağa sahip nesne (aynı `CWnd::GetFocus`).  
...
### <a name="return-value"></a>Dönüş Değeri  
 Denetimin stock Font özelliği yazı tipi gönderme arabiriminin bir işaretçi.  
...
### <a name="return-value"></a>Dönüş Değeri  
 Denetim aşağıdaki değerlerden birini hazır olma durumu:  
...
### <a name="return-value"></a>Dönüş Değeri  
 Denetim yerinde etkin ise sıfır olmayan; Aksi takdirde 0.  
...
### <a name="return-value"></a>Dönüş Değeri  
 Denetim metin dizesi veya dize varsa, sıfır uzunlukta bir dize geçerli değeri.  
  
> [!NOTE]
>  Daha fazla bilgi için `BSTR` veri türü, bkz: [veri türleri](../../mfc/reference/data-types-mfc.md) makroları ve genel öğeleri bölümünde.  
...
### <a name="return-value"></a>Dönüş Değeri  
 Nesnenin işaretçi `IDropTarget` arabirimi. Bir pencere olmadığından penceresiz nesne kaydedilemiyor bir `IDropTarget` arabirimi. Ancak, sürükle ve bırak katılmak için penceresiz nesne hala arabirimini uygulayan ve içinde dönüş `GetWindowlessDropTarget`.  
  
### <a name="remarks"></a>Açıklamalar  
 Normalde, bu denetimin penceresi bir bırakma hedefi olarak kaydedilmesi gerekir. Ancak denetim kendi pencere olduğundan, kapsayıcı bir bırakma hedefi olarak kendi pencere kullanın. Denetimi yalnızca uygulaması sağlaması gerekir `IDropTarget` için kapsayıcı temsil çağrıları uygun zamanda arabirimi. Örneğin:  
...
### <a name="parameters"></a>Parametreler  
 *piidPrimary*  
 Denetimin birincil dağıtma arabirimi arabirim kimliği işaretçi.  
  
 *piidEvents*  
 Denetimin olay arabirimi arabirim kimliği işaretçi.  
...
### <a name="remarks"></a>Açıklamalar  
 OLE denetimleri için özel yükleme kodu, VBX kullanan bir form dönüştürme OLE denetimleri kullanan başka birine denetimleri, gerekli olabilir. OLE denetim örneği yüklüyorsanız, örneğin, bir çağrı olabilir [PX_Font](persistence-of-ole-controls.md#px_font) içinde `DoPropExchange`:  
  
 [!code-cpp[NVC_MFCAxCtl#3](../../mfc/reference/codesnippet/cpp/colecontrol-class_4.cpp)]  
  
 Ancak, VBX denetimleri yazı tipi nesnesi yok; her font özelliği ayrı ayrı kaydedildi. Bu durumda, kullanacağınız `IsConvertingVBX` bu iki örnekleri arasında ayrım yapmak için:  
  
 [!code-cpp[NVC_MFCAxCtl#4](../../mfc/reference/codesnippet/cpp/colecontrol-class_5.cpp)]  
  
 Başka bir durumda VBX denetiminiz özel ikili verileri kaydettiyseniz olur (içinde kendi **VBM_SAVEPROPERTY** ileti işleyicisi), ve OLE denetiminizi ikili verileri farklı bir biçimde kaydeder. OLE denetim olmasını istiyorsanız, geriye dönük olarak uyumlu VBX denetimi ile kullanarak hem eski ve yeni biçimleri okuyabilir `IsConvertingVBX` VBX denetimi veya OLE denetimi yüklenmekte olup olmadığını ayrım tarafından işlevi.  
...
### <a name="return-value"></a>Dönüş Değeri  
 Son kaydedilişinden denetimin durumunu güncellendiyse sıfır olmayan; Aksi takdirde 0.  
...
### <a name="return-value"></a>Dönüş Değeri  
 **DOĞRU** kapsayıcı destekliyorsa, en iyi duruma getirilmiş geçerli çizim işlemi için çizim; Aksi takdirde **FALSE**.  
...
### <a name="return-value"></a>Dönüş Değeri  
 Denetim Sınıflandırma, sıfır olmayan; Aksi takdirde 0.  
...
### <a name="parameters"></a>Parametreler  
 *strNewPath*  
 Zaman uyumsuz denetim özelliği mutlak konumu başvuran yolunu içeren bir dize için bir işaretçi.  
...
### <a name="return-value"></a>Dönüş Değeri  
 Kilitleme başarılı olduğunda sıfır olmayan; Aksi takdirde 0.  
...
### <a name="parameters"></a>Parametreler  
 *iButton*  
 Fare düğmesini dizini. Aşağıdaki değerlerden biri olabilir:  
  
- **LEFT_BUTTON** sol fare düğmesini tıklandığını.  
  
- **MIDDLE_BUTTON** orta fare düğmesini tıklandığını.  
  
- **RIGHT_BUTTON** farenin sağ düğmesiyle tıklandığını.  
...
### <a name="return-value"></a>Dönüş Değeri  
 Çağrı başarılı ise sıfır olmayan; Aksi takdirde 0.  
  
### <a name="remarks"></a>Açıklamalar  
 Varsayılan uygulama kullanan `ON_OLEVERB` ve `ON_STDOLEVERB` iletisi eşleme girdilerini çağırmak için uygun işlev belirlemek için.  
...
### <a name="remarks"></a>Açıklamalar  
 Bu denetimin çağırma aynı etkiye sahip `OLEIVERB_UIACTIVATE` fiil.  
  
 Bu işlev genellikle işleyici işlevi olarak kullanılan bir `ON_OLEVERB` iletisi eşleme girişi. Bu bir "Düzenle" fiili denetimin "Nesne" menüsünden kullanılabilmesini sağlar. Örneğin:  
...
### <a name="return-value"></a>Dönüş Değeri  
 Fiiller mevcutsa, sıfır olmayan; Aksi takdirde 0.  
  
### <a name="remarks"></a>Açıklamalar  
 Varsayılan uygulama numaralandırır `ON_OLEVERB` ileti eşlemesi girdiler.  
...
### <a name="remarks"></a>Açıklamalar  
 Varsayılan Uygulama çağrıları `COleControl::InvalidateControl`. Varsayılan uygulama denetimi bir Windows denetimini alt sınıf yapma, ayrıca gönderir bir **WM_SETFONT** denetimin pencere iletisi.  
...
### <a name="return-value"></a>Dönüş Değeri  
 Geçerli bir renk kümesi döndürdüyse sıfır olmayan; Aksi takdirde 0.  
  
### <a name="remarks"></a>Açıklamalar  
 Kapsayıcı OLE denetimi çizmek için gereken tüm renkleri elde etmek için bu işlevi çağırır. Kapsayıcı genel renk paletini ayarlamak için gereken renkleri birlikte elde renk kümelerini kullanabilirsiniz. Varsayılan uygulama döndürür **FALSE**.  
...
### <a name="return-value"></a>Dönüş Değeri  
 İçinde bir dize döndürdü, sıfır olmayan *strValue;* Aksi halde 0.  
...
### <a name="return-value"></a>Dönüş Değeri  
 Denetimin menüsünde tanıtıcısı veya **NULL** denetimin hiçbiri varsa. Varsayılan uygulama döndürür **NULL**.  
...
- **DVEXTENT_CONTENT** Inquire denetimi içeriğin (ek boyutu) tam olarak sığması için ne kadar büyük olmalıdır  
...
### <a name="return-value"></a>Dönüş Değeri  
 Başarıyla döndürür veya boyutunu ayarlar sıfır olmayan; Aksi takdirde 0.  
 ...
 *lpsizel*  
 Nesnenin boyutu burada döndürülür konuma noktaları.  
...
### <a name="return-value"></a>Dönüş Değeri  
 Nesneye boyutta bir dikdörtgen başarıyla döndürülürse, sıfır olmayan; Aksi takdirde 0.  
 ...
 **VIEWSTATUS_DVASPECTTRANSPARENT**  
 Nesne destekler **DVASPECT_TRANSPARENT**. Tüm **IViewObjectEx** bir parametre ile bu yönünün çağrılabilir gibi bir çizim yönü ele yöntemleri.  
...
- **HITRESULT_TRANSPARENT** *ptlLoc* OLE nesnesinin ancak görüntü değil yakın sınırları içinde değil. Örneğin, saydam bir daire ortasında bir nokta olabilir **HITRESULT_TRANSPARENT**.  
  
- **HITRESULT_CLOSE** `ptlLoc` içinde veya OLE nesnesi ancak içinde değerlendirilmesi için nesnesine kadar yakın dışında. Küçük, basit veya ayrıntılı nesneleri bu değeri kullanabilirsiniz. Sınırlayıcı dışında bir nokta olsa bile, bir nesne hala dikdörtgen Kapat (Bu küçük nesneleri basmak için gereklidir).  
  
- **HITRESULT_HIT** `ptlLoc` nesne içinde görüntüdür.  
...
- **HITRESULT_HIT** dikdörtgen en az bir noktasında, nesne üzerinde isabet olacaktır.  
...
### <a name="remarks"></a>Açıklamalar  
 Belirtilen biçim biridir denetim nesnesini kullanarak daha önce yerleştirilen [DelayRenderData](../../mfc/reference/coledatasource-class.md#delayrenderdata) veya [DelayRenderFileData](../../mfc/reference/coledatasource-class.md#delayrenderfiledata) üye işlevlerini Gecikmeli işleme için. Bu işlev varsayılan uygulamasını çağıran `OnRenderFileData` veya `OnRenderGlobalData`, sırasıyla sağlanan depolama ortamına bir dosya veya bellek ise. İstenen biçim ise `CF_METAFILEPICT` veya kalıcı özelliğini ayarlayın biçimi, varsayılan uygulama uygun veriler işler ve sıfır olmayan bir değer döndürür. Aksi halde 0 döndürür ve hiçbir şey yapmaz.  
  
 Varsa *lpStgMedium ortam türü ->* olan **TYMED_NULL**, **STGMEDIUM** tarafından belirtilen doldurulmuş ve ayrılan gerekir *lpFormatEtc -> ortam türü*. Aksi takdirde **TYMED_NULL**, **STGMEDIUM** verilerle yerinde dolu olması gerekir.  
...
### <a name="remarks"></a>Açıklamalar  
 Varsayılan Uygulama çağrıları [DoPropExchange](#dopropexchange), geçen bir `CPropExchange` varsayılan değerlerine ayarlanacak özellikler neden olan nesne.  
...
### <a name="remarks"></a>Açıklamalar  
 Varsayılan olarak, `OnSetClientSite` veri yolu özellikleri yüklü olup olmadığını denetler ve varsa, çağıran `DoDataPathPropExchange`.  
...
### <a name="remarks"></a>Açıklamalar  
 Varsayılan uygulama, denetimin kapsamını yeniden boyutlandırma işler. Denetim yerinde etkin, kapsayıcının çağrısına ise **OnPosRectChanged** sonra yapılır.  
...
### <a name="remarks"></a>Açıklamalar  
 Varsayılan uygulama otomatik olarak yeniden konumlandırma ve denetim pencerenin yeniden boyutlandırma ve işler döndürür **doğru**.  
...
- **HTBOTTOM** penceresinin alt yatay kenarlık içinde.  
  
- **HTBOTTOMLEFT** pencere kenarlığı sol alt köşesindeki.  
  
- **HTBOTTOMRIGHT** pencere kenarlığı sağ alt köşesindeki.  
  
- **HTCAPTION** bir başlık çubuğu alanı.  
  
- **HTCLIENT** istemci alanında.  
  
- **HTERROR** ekran arka plan ya da windows arasındaki bir çizgi (aynı **HTNOWHERE** dışında **DefWndProc** Windows işlevi hatayı belirtmek için sistem bir bip oluşturur).  
  
- **HTGROWBOX** boyutu kutusuna.  
  
- **HTHSCROLL** yatay kaydırma çubuğunda.  
  
- **HTLEFT** penceresinin sol kenarlığın içinde.  
  
- **HTMAXBUTTON** düğmesine bir ekranı kaplamasını sağlayın.  
  
- **HTMENU** menü alanında.  
  
- **HTMINBUTTON** içinde bir simge durumuna küçült düğmesi.  
  
- **HTNOWHERE** ekran arka plan ya da windows arasındaki bir çizgi.  
  
- **HTREDUCE** içinde bir simge durumuna küçült düğmesi.  
  
- **HTRIGHT** penceresinin sağ kenarlığın içinde.  
  
- **HTSIZE** boyut kutusu içinde (aynı **HTGROWBOX**).  
  
- **HTSYSMENU** Denetim menüsü veya bir alt penceresinde Kapat düğmesi.  
  
- **HTTOP** penceresinin üst yatay kenarlık içinde.  
  
- **HTTOPLEFT** pencere kenarlığı sol üst köşesindeki.  
  
- **HTTOPRIGHT** pencere kenarlığı sağ üst köşesindeki.  
...
### <a name="remarks"></a>Açıklamalar  
 Özellikler: görünümü, arka plan rengi, kenarlık stili, başlık, etkin, yazı tipi, ForeColor, hWnd ve metin. Stok özellikleri açıklaması için bkz: [ActiveX denetimleri: stok Özellikler ekleme](../../mfc/mfc-activex-controls-adding-stock-properties.md).  
...
### <a name="parameters"></a>Parametreler  
 *sAppearance*  
 A **kısa** ( `VT_I2`), denetimin görünümünü için kullanılacak değer. Sıfır değeri denetimin görünümünü düz ayarlar ve 1 değeri denetimin görünümünü 3D olarak ayarlar.  
...
### <a name="parameters"></a>Parametreler  
 *dwBackColor*  
 Bir **OLE_COLOR** denetiminizi çizim arka planı için kullanılacak değer.  
...
### <a name="parameters"></a>Parametreler  
 *sBorderStyle*  
 Denetim için yeni kenarlık stili; hiçbir kenarlık 0 gösterir ve normal kenarlık 1 gösterir.  
...
### <a name="return-value"></a>Dönüş Değeri  
 Bir işaretçi **CWnd** önceden fare girdisi alınan pencere nesnesi.  
...
### <a name="return-value"></a>Dönüş Değeri  
 Bir işaretçi **CWnd** daha önce giriş odağa sahip pencere nesnesi veya **NULL** penceresi yok ise.  
  
### <a name="remarks"></a>Açıklamalar  
 Denetim etkinleştirilir ve penceresiz ise bu işlev giriş odağını elinde denetimin adınıza yapılacak denetimin kapsayıcı pencere neden olur. Giriş odağını klavye girişi kapsayıcının penceresine yönlendirir ve kapsayıcı çağırır OLE nesnesi için tüm sonraki klavye iletileri gönderir `SetFocus`. Giriş odağını daha önce sahip penceresi, kaybeder.  
...
### <a name="parameters"></a>Parametreler  
 *dwForeColor*  
 Bir **OLE_COLOR** denetiminizi çizim ön için kullanılacak değer.  
...
### <a name="parameters"></a>Parametreler  
 *lpptlHimetric*  
 İşaretçi bir **POINTL** koordinatlarında içeren yapısını **HIMETRIC** birimleri.  
...
### <a name="return-value"></a>Dönüş Değeri  
 Ortam özelliklerine geçerli olacağını NonZero gösterir; Aksi takdirde ortam özelliklerine geçerli olmaz.  
...
### <a name="return-value"></a>Dönüş Değeri  
 Gönderilen ileti dönüş değeri.  
  
### <a name="remarks"></a>Açıklamalar  
 Denetimin ileti eşlemesi ile belirli iletileri gönderme bu işlevini çağırın.  
