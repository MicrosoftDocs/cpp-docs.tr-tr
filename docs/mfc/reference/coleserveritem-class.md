...
|Ad|Açıklama|
|----------|-----------------|
|[Cotaserverıtem:: AddOtherClipboardData](#addotherclipboarddata)|Bir nesneye sunum ve dönüştürme biçimleri koyar `COleDataSource` .|
...
|[Cotaserverıtem:: OnRenderFileData](#onrenderfiledata)|`CFile`Gecikmeli işlemenin bir parçası olarak bir nesneye veri alır.|
...
*pDataSource*<br/>
`COleDataSource`Desteklenen tüm BIÇIMLERDE OLE öğesinin verilerini alacak nesneye yönelik işaretçi.
...
`COleDataSource`Dönüştürme biçimlerini depolamak için kullanılan nesneye yönelik bir işaretçi.
...
Bu biçim öğenin yerel verilerini içerir. `Serialize`Bu işlevin düzgün çalışması için üye işlevini uygulamış olmanız gerekir.
...
Kapsayıcı uygulama Microsoft Foundation Class Kitaplığı yazılmışsa, bu işlev, karşılık gelen nesnenin [Cotaclientidıtem:: Activate](../../mfc/reference/coleclientitem-class.md#activate) üye işlevi `COleClientItem` çağrıldığında çağrılır. Varsayılan uygulama, birincil fiil veya OLEIVERB_SHOW belirtilmişse [OnShow](#onshow) üye işlevini çağırır, ikincil fiil veya OLEIVERB_OPEN [belirtilirse, ve](#onopen) OLEIVERB_HIDE belirtilmişse [OnHide](#onhide) . `OnShow` *Iverb* , yukarıda listelenen fiillerden biri değilse, varsayılan uygulama çağırır.
...
`OnDraw`DVASPECT DVASPECT_CONTENT eşitse varsayılan uygulama çağırır; aksi takdirde başarısız olur.
...
*rSize*<br/>
`CSize`OLE öğesinin boyutunu alacak bir nesneye başvuru.
...
*pFile*<br/>
`CFile`Verilerin işlendiği bir nesneyi işaret eder.
...
Belirtilen biçim, `COleDataSource` gecikmeli işleme Için [DelayRenderData](../../mfc/reference/coledatasource-class.md#delayrenderdata) üye işlevi kullanılarak nesnesine daha önce yerleştirilmiş bir biçimdir. Bu işlevin varsayılan uygulanması yalnızca FALSE değerini döndürür.
...
Kapsayıcı uygulaması Microsoft Foundation Class Kitaplığı kullanılarak yazılmışsa, ilgili nesnenin [IOleObject:: SetColorScheme](/windows/win32/api/oleidl/nf-oleidl-ioleobject-setcolorscheme) işlevi çağrıldığında bu işlev çağrılır `COleClientItem` . Varsayılan uygulama yanlış döndürür. Önerilen paleti kullanmak istiyorsanız bu işlevi geçersiz kılın. Önerilen paleti kullanmak için sunucu uygulaması gerekli değildir.
