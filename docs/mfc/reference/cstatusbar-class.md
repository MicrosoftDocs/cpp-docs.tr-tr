---
description: 'Daha fazla bilgi edinin: CStatusBar sınıfı'
...
|[CStatusBar:: CreateEx](#createex)|`CStatusBar`Katıştırılmış nesne için ek stillere sahip bir nesne oluşturur `CStatusBarCtrl` .|
...
MFC 4,0 ' ye yeni bir üye işlevi olan [CStatusBar:: GetStatusBarCtrl](#getstatusbarctrl), durum çubuğu özelleştirmesi ve ek Işlevler için Windows ortak denetim desteğinden yararlanmanızı sağlar. `CStatusBar` üye işlevleri, Windows ortak denetimleri işlevlerinin çoğunu sağlar; Ancak, `GetStatusBarCtrl` öğesini çağırdığınızda, bir Windows 95/98 durum çubuğunun özelliklerine daha fazla sahip olmak üzere durum çubuklarınızı da verebilirsiniz. `GetStatusBarCtrl`Öğesini çağırdığınızda, bir nesnesine bir başvuru döndürür `CStatusBarCtrl` . Windows ortak denetimleri kullanarak araç çubukları tasarlama hakkında daha fazla bilgi için bkz. [CStatusBarCtrl](../../mfc/reference/cstatusbarctrl-class.md) . Ortak denetimler hakkında daha fazla genel bilgi için bkz. Windows SDK [ortak denetimleri](/windows/win32/Controls/common-controls-intro) .
...
1. Nesnesini oluşturun `CStatusBar` .

1. Durum çubuğu penceresini oluşturmak ve nesneye iliştirmek için [Create](#create) (veya [CreateEx](#createex)) işlevini çağırın `CStatusBar` .
...
Kullanma hakkında daha fazla bilgi için `CStatusBar` [MFC 'de durum çubuğu uygulama](../../mfc/status-bar-implementation-in-mfc.md) ve [Teknik Not31: denetim çubukları](../../mfc/tn031-control-bars.md)makalesine bakın.
...
Bir durum çubuğu (bir alt pencere) oluşturmak ve bunu nesnesiyle ilişkilendirmek için bu işlevi çağırın `CStatusBar` .
...
- Durum çubuğu denetimi SBARS_SIZEGRIP durum çubuğunun sağ ucunda bir boyutlandırma tutamacı içerir. Boyutlandırma tutamacı boyutlandırma kenarlığına benzerdir; Bu, kullanıcının üst pencereyi yeniden boyutlandırmak için tıklave sürükleyebilmesini sağlayan dikdörtgen bir alandır.

- Durum çubuğunun araç ipuçlarını desteklemesi SBT_TOOLTIPS.
...
`CreateEx`Katıştırılmış durum çubuğu denetiminin oluşturulması sırasında belirli stillerin bulunması gereken durumlarda, [Oluştur](#create)yerine kullanın. Örneğin, araç ipuçlarını bir durum çubuğu nesnesinde göstermek için *dwCtrlStyle* öğesini SBT_TOOLTIPS olarak ayarlayın.
...
`itemAction` `DRAWITEMSTRUCT` Yapının üyesi gerçekleştirilecek çizim eylemini tanımlar. Bir sahip çizim nesnesi için çizimi uygulamak üzere bu üye işlevini geçersiz kılın `CStatusBar` . Uygulamanın, bu üye işlevin sonlandırılması için *Lpdrawitemstruct* içinde sağlanan görüntüleme bağlamı için seçilen tüm grafik cihaz ARABIRIMI (GDI) nesnelerini geri yüklemesi gerekir.
...
*lpRect*<br/>
Bir [Rect](/windows/win32/api/windef/ns-windef-rect) yapısına veya *nIndex* tarafından belirtilen göstergenin koordinatlarını alacak bir [CRect](../../atl-mfc-shared/reference/crect-class.md) nesnesine işaret eder.

### <a name="remarks"></a>Açıklamalar

Koordinatlar durum çubuğunun sol üst köşesine göre piksel cinsinden görüntülenir.

## <a name="cstatusbargetpaneinfo"></a><a name="getpaneinfo"></a> CStatusBar:: Getbölmesi bilgileri

*NID*, *nStyle* ve *Cxwidth* ' i, *NıNDEX* tarafından belirtilen konumdaki gösterge bölmesinin kimliği, stili ve genişliğine göre ayarlar.
...
## <a name="cstatusbargetpanestyle"></a><a name="getpanestyle"></a> CStatusBar:: Getbölmesi stili
...
## <a name="cstatusbargetpanetext"></a><a name="getpanetext"></a> CStatusBar:: Getbölmesi metni
...
`CString`Bölmenin metnini içeren nesne.
...
`GetStatusBarCtrl`Windows durum çubuğu ortak denetiminin işlevselliğinin avantajlarından yararlanmak ve [CStatusBarCtrl](../../mfc/reference/cstatusbarctrl-class.md) desteğinin avantajlarından yararlanmak için, durum çubuğu özelleştirmesi için kullanın. Örneğin, ortak denetimi kullanarak durum çubuğunda boyutlandırma tutamacı içeren bir stil belirtebilir veya durum çubuğunun üst pencerenin istemci alanının en üstünde görünmesini sağlamak için bir stil belirtebilirsiniz.
...
## <a name="cstatusbarsetpaneinfo"></a><a name="setpaneinfo"></a> CStatusBar:: Setbölmesi bilgileri
...
- Bölmenin etrafında 3-b sınır SBPS_NOBORDERS.

- Metnin "açılan" metni için ters kenarlık SBPS_POPOUT.
...
## <a name="cstatusbarsetpanestyle"></a><a name="setpanestyle"></a> CStatusBar:: Setbölmesi stili
...
## <a name="cstatusbarsetpanetext"></a><a name="setpanetext"></a> CStatusBar:: Setbölmesi metni
...
`SetPaneText`' İ çağırdığınızda, durum çubuğunda yeni metni göstermek için BIR UI güncelleştirme işleyicisi eklemeniz gerekir.
