---
description: 'Daha fazla bilgi edinin: CMFCEditBrowseCtrl Class'
...
## <a name="how-to-specify-an-edit-browse-control"></a>Nasıl yapılır: düzenleme tarama denetimi belirtme

Uygulamanızda bir düzenleme tarama denetimi eklemek için aşağıdaki adımları gerçekleştirin:

1. Özel bir tarama modu uygulamak istiyorsanız, sınıfından kendi sınıfınızı türetirsiniz `CMFCEditBrowseCtrl` ve ardından [CMFCEditBrowseCtrl:: ongözatıt](#onbrowse) metodunu geçersiz kılın. Geçersiz kılınan yöntemde, özel bir gözden geçirme eylemi yürütün ve sonuçla düzenleme gözden geçirme denetimini güncelleştirin.

1. `CMFCEditBrowseCtrl`Üst pencere nesnesine nesne veya türetilmiş düzenleme Gözat denetim nesnesini ekleyin.

1. Bir iletişim kutusu oluşturmak için **sınıf sihirbazını** kullanırsanız, iletişim kutusu formuna bir düzenleme denetimi ( `CEdit` ) ekleyin. Ayrıca, üst bilgi dosyanızdaki denetime erişmek için bir değişken ekleyin. Üst bilgi dosyanızda değişkeninin türünü `CEdit` olarak değiştirin `CMFCEditBrowseCtrl` . Düzenleme gözden geçirme denetimi otomatik olarak oluşturulur. **Sınıf Sihirbazı**'nı kullanmıyorsanız, `CMFCEditBrowseCtrl` üst bilgi dosyanıza bir değişken ekleyin ve sonra `Create` metodunu çağırın.
...
Aşağıdaki örnek, sınıfında iki yöntemin nasıl kullanılacağını gösterir `CMFCEditBrowseCtrl` : `EnableFolderBrowseButton` ve `EnableFileBrowseButton` . Bu örnek, [Yeni denetimler örneğinin](../../overview/visual-cpp-samples.md)bir parçasıdır.
...
*BEnable* parametresi true ise, gözden geçirme düğmesine tıklandığında gerçekleştirilecek özel bir eylem uygulayın. Özel bir eylem uygulamak için sınıfından bir sınıf türetirsiniz `CMFCEditBrowseCtrl` ve sonra [ongözatıt](#onbrowse) yöntemini geçersiz kılın.
...
## <a name="cmfceditbrowsectrlonbrowse"></a><a name="onbrowse"></a> CMFCEditBrowseCtrl:: Onzat
