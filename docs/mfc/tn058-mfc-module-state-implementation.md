...
2. `RawDllMain` ilk olarak adlandırılır. Modül durumu DLL için bilinen statik modülü durumuna ayarlar. Bu nedenle `RawDllMain` DLL'e statik olarak bağlanır.
...
5. `COleObjectFactory::XClassFactory2::CreateInstance` adı verilir. Bu işlev Modül durumu nesnesi oluşturma ve onu döndürmeden önce adım 3'te geçerli bir modül durumuna ayarlar. (ne zaman geçerli bir `COleObjectFactory` örneğinin başlatılmasından). Bu içinde yapılır [METHOD_PROLOGUE](com-interface-entry-points.md).
...
AFX_MODULE_STATE makrosu kullanılmıyorsa, DLL'leri kaynaklarında sorunları ortaya çıkar. Varsayılan olarak, MFC kaynak şablonu yüklemek için ana uygulama kaynak tanıtıcısı kullanır. Bu şablon, aslında DLL'de depolanır. MFC'nin Modül durumu bilgilerini tarafından AFX_MODULE_STATE makrosu geçti değil, temel nedeni oluşturur. Kaynak tanıtıcısı MFC'nin modülü durumundan kurtarıldı. Modül durumu geçiş değil, kullanılacak yanlış kaynağı tanıtıcısı neden olur.

AFX_MODULE_STATE her işlevi dll put gerekmez. Örneğin, `InitInstance` MFC modül durumu önce otomatik olarak kayar çünkü uygulamayı AFX_MODULE_STATE olmadan MFC kod tarafından çağrılabilir `InitInstance` ve ardından geri sonra anahtarları `InitInstance` döndürür. Aynı tüm ileti eşlemesi işleyicileri için geçerlidir. Normal MFC DLL'leri gerçekte herhangi ileti yönlendirme önce Modül durumu otomatik olarak geçer özel ana pencere yordamı vardır.
...
MFC bu iki adımda uygular. İlk olarak, Win32 en üstünde bir katmanı yok __Tls\*__  API'leri (**TlsAlloc**, **TlsSetValue**, **TlsGetValue**, vb.), İşlem başına yalnızca iki TLS dizinleri kullanın, kaç tane DLL'leri olsun gerekir. İkinci, `CProcessLocal` şablonu bu verilere erişmek için sağlanır. İşleci geçersiz kılmalar -> olduğu ne yukarıda gördüğünüz sezgisel sözdizimi sağlar. Tarafından Sarmalanan tüm nesneleri `CProcessLocal` öğesinden türetilmelidir `CNoTrackObject`. `CNoTrackObject` alt düzey ayırıcısı sağlar (**LocalAlloc**/**LocalFree**) ve sanal yıkıcı sağlayacak şekilde işlem sonlandırıldığında MFC işlem yerel nesneleri otomatik olarak yok edebilir. Ek temizleme gerekliyse, bu tür nesneleri özel yıkıcı olabilir. Derleyici katıştırılmış yok etmek için bir varsayılan yok Edicisi oluşturacak bu yana yukarıdaki örnekte, gerektirmeyen `CString` nesnesi.
