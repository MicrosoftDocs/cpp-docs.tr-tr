...
Tüketiciler, satır kümesi veri güncelleştirmeleri aşağıdaki türde tamamlayabilir: bir satırdaki sütun değerleri ayarlama, satır ekleme ve bir satırın silinmesi. OLE DB Şablon sınıfı bu işlemlerin tamamlanması [CRowset](../../data/oledb/crowset-class.md) uygulayan [IRowsetChange](/previous-versions/windows/desktop/ms715790) arabirim ve aşağıdaki arabirim yöntemini geçersiz kılar:  
...
[CRowset::SetData](../../data/oledb/crowset-setdata.md) geçerli satırın bir veya daha fazla sütun veri değerlerini ayarlar. Aşağıdaki kod sütunları için "Name" ve "Stoktaki" tablosunun ilişkili veri üyelerinin değerlerini ayarlar `Products` ve `SetData` bu değerleri kümesi % 100'lük bir satır yazmak için:  
...
Güncelleştirme, güncelleştirme yöntemleri tarafından kullanılan komutunda değişiklikler yapma belirli bir anlamı vardır ve SQL güncelleştirme komut ile karıştırılır olması değil (`SetData` için SQL güncelleştirme komut eşdeğerdir).  
...
Ertelenmiş güncelleştirmeleri destekleyecek şekilde ayarlamalısınız `DBPROP_IRowsetChange` özelliği birlikte "Güncelleştirme işlemleri destekleyen" açıklanan özellikler:  
