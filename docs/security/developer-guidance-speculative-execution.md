...
Bu makalede tarafından sağlanan kılavuza sınıfı temsil CVE-2017-5753 tarafından Spectre değişken 1 olarak da bilinen güvenlik açıklarının ilgilidir. Bu donanım güvenlik açığı sınıfı koşullu dal misprediction sonucunda ortaya çıkan kurgusal yürütme nedeniyle ortaya çıkabilir yan kanalları ilgilidir. Visual Studio (15.5.5 sürümünden başlayarak) 2017'de Visual C++ Derleyici desteği içeren `/Qspectre` anahtarı 2017 CVE 5753 için ilgili savunmasız olabilecek kodlama desenleri sınırlı sayıda için derleme zamanı azaltma sağlar. Belgelerine [/Qspectre](https://docs.microsoft.com/en-us/cpp/build/reference/qspectre) bayrağı kendi etkiler ve kullanımı hakkında daha fazla bilgi sağlar. 
...
Modern CPU yaparak daha yüksek derece performansını sağlamak kurgusal ve sıra dışı yürütme yönergeleri kullanın. Örneğin, bu genellikle CPU speculatively gerçek şube hedef olana kadar bu nedenle bir kabini önleme tahmin edilen şube hedef yönergeleri yürütülmeye başlamaya için sağlayan dalları (koşullu ve indrect) hedefi tahmin ederek çalışmasını gerçekleştirilir çözümlendi. CPU, daha sonra bir misprediction oluştuğunu bulur, durumunda, tüm speculatively hesaplanmıştır makine durumunun atılır. Bu mimari görünür hiçbir mispredicted Spekülasyon etkilerini olmasını sağlar.

Kurgusal yürütme architecturaly görünür duruma etkilemez, ancak CPU tarafından kullanılan çeşitli önbellekleri gibi mimari olmayan durumda fazlalık izlemeleri bırakılabilir. Bu kalan izleri yan kanal güvenlik açıklarına neden veren kurgusal yürütme olur. Bu daha iyi anlamak için CVE 2017 5753 (denetleyin sınırları atlama) örneği sağlayan aşağıdaki kod parçası göz önünde bulundurun:
...
|Enclave sınır|Bir uygulama, enclave dışında verilerden risk altında olabilir recieves güvenilmeyen güvenli bir enclave (örneğin, Intel SGX) içinde yürütür.|
...
Bu kategori desenleri kodlama kurgusal türü Karışıklığı önlemek için müşteri adayları koşullu dal misprediction içerir. Bu bölümdeki kodlama desenleri aşağıdaki örnek kod başvurur.
...
|Mimari|Spekülasyon engelle|
|----------------|----------------|
|x86/x64|_mm_lfence()|
|ARM|Şu anda kullanılamıyor|
|ARM64|Şu anda kullanılamıyor|
...
[Kurgusal yürütme yan kanal güvenlik açıkları azaltmak için yönergeler](https://portal.msrc.microsoft.com/en-US/security-guidance/advisory/ADV180002)

[Azaltıcı kurgusal yürütme yan kanal donanım güvenlik açıkları](https://blogs.technet.microsoft.com/srd/2018/03/15/mitigating-speculative-execution-side-channel-hardware-vulnerabilities/)
