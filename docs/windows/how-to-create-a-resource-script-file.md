...
**Kaynak Ekle** içerikleri iletişim kutusu **yeni özel kaynak** iletişim kutusunda aşağıdaki özelliklerle bir C++ projesinde yeni bir özel kaynak oluşturmak için:
...
## <a name="resource-scripts"></a>Kaynak komut dosyaları
...
1. Kaynak kodunuzu için bir ad sağlayın **adı** metin kutusu ve select **açık**.

### <a name="to-open-a-resource-script"></a>Kaynak betiği açmak için

Bir proje açmak zorunda kalmadan, kaynakları bir kaynak betik görüntüleyebilirsiniz. Komut dosyası, başlangıcı yerine sonundan belge penceresinde açılır **kaynak görünümü** penceresi.
...
#### <a name="to-open-a-resource-script-outside-of-a-project"></a>Proje dışındaki kaynak betik açmak için

1. Gelen **dosya** menüsünde **açık**, ardından **dosya**.

1. Kaynak betik dosyasına gidin, dosyayı vurgulayın ve seçme **açık**.

#### <a name="to-open-multiple-resource-scripts-outside-of-a-project"></a>Proje dışındaki birden fazla kaynak komut dosyası açmak için
...
1. Açmak istediğiniz ilk kaynak betik dosyasına gidin (*Source1.rc*), dosyayı vurgulayın ve seçme **açın**.

1. İkinci .rc dosyasını açmak için önceki adımı yineleyin (*Source2.rc*).

   Her iki .rc dosyası artık ayrı bir belge pencerelerinde açıktır.

1. Kullanım **penceresi** menü (veya .rc dosyaları sağ) ve **yeni yatay sekme grubu** veya **yeni bir dikey sekme grubu**.
...
1. Seçin **birlikte Aç** seçin ve açılan menüden **kaynak kodu (metin) Düzenleyicisi**.
...
> [!TIP]
> Burada .rc dosyasına sağ kısayoludur **Çözüm Gezgini**, seçin **birlikte Aç...**  seçip **kaynak kodu (metin) Düzenleyicisi**.
...
Aşağıdakilerden birini kullanarak yeni bir kaynak oluşturabilirsiniz:
...
- İçinde **proje** menüsünde **kaynak Ekle**.

### <a name="support-for-mfc"></a>MFC desteği

Normalde, kullanarak Windows için Microsoft Foundation Class (MFC) uygulama derleme, [MFC Uygulama Sihirbazı](../mfc/reference/mfc-application-wizard.md), sihirbaz'in temel özellikleri içeren dosyaları (kaynak betiği (.rc) dosyası dahil) temel bir dizi oluşturur. MFC. Ancak, MFC'ye dayalı olmayan bir Windows uygulaması için bir .rc dosyası düzenlenirken, MFC özgü aşağıdaki özellikler kullanılamaz: Menü komut istemi dizeleri, MFC kodu sihirbazları birleşik giriş kutusu denetimleri ve ActiveX denetimi barındırma içeriğini listeler.

#### <a name="to-add-mfc-support-to-a-resource-script-file"></a>Kaynak betik dosyası için MFC desteği eklemek için
...
## <a name="resource-templates"></a>Kaynak şablonları
...
1. Artı işaretini seçin (**+**) kaynak düğümünü genişletin ve o kaynak için kullanılabilir tüm şablonları görüntülemek için bir kaynak yanında.
