...
> [!NOTE]
>  Ancak [ClickOnce](/visualstudio/deployment/clickonce-security-and-deployment) Visual Studio'daki Visual C++ yerel uygulamalar için dağıtım mümkündür, ek adımlar gerektirir. Daha fazla bilgi için [Visual C++ uygulamaları için ClickOnce dağıtımı](clickonce-deployment-for-visual-cpp-applications.md).
...
Visual C++ kitaplıklarının yanlış dağıtılması, onlara bağımlı olan bir uygulamanın çalıştırılması esnasında çalışma zamanı hatalarına neden olabilir. İşletim sistemi uygulamayı yüklerken, açıklanan arama düzenini kullanır. [LoadLibraryEx](/windows/desktop/api/libloaderapi/nf-libloaderapi-loadlibraryexw).
