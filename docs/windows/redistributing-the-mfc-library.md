...
MFC uygulamanız Windows Forms denetimleri kullanıyorsa, mfcmifc80. dll dosyasını Uygulamanızla yeniden dağıtmanız gerekir. Bu DLL, uygulamanın yerel klasöründeki bir uygulamayla veya [Gacutil. exe (genel bütünleştirilmiş kod önbelleği aracı)](/dotnet/framework/tools/gacutil-exe-gac-tool)kullanılarak genel derleme önbelleği 'NE (GAC) dağıtarak bir uygulamayla yeniden dağıtılarak, tanımlayıcı ad imzalı bir .net derlemesidir.

Bir MFC DLL 'yi yeniden dağıtırsanız, hata ayıklama sürümünü değil, perakende sürümünü yeniden dağıttığınızdan emin olun. Dll 'lerin hata ayıklama sürümleri yeniden dağıtılabilir değildir. MFC DLL 'lerinin hata ayıklama sürümlerinin adları "d" ile biter, örneğin, Mfc140d. dll.

MFC 'yi VCRedist_*mimari*. exe ' yi kullanarak, Visual Studio ile yüklenen birleştirme MODÜLLERINI veya MFC DLL 'yi uygulamanızla aynı klasöre dağıtarak yeniden dağıtabilirsiniz. MFC 'yi yeniden dağıtma hakkında daha fazla bilgi için bkz. [görsel C++ dosyaları yeniden dağıtma](redistributing-visual-cpp-files.md).

## <a name="installation-of-localized-mfc-components"></a>Yerelleştirilmiş MFC bileşenlerini yükleme

Uygulamanızı bir MFC yerelleştirme DLL 'SI yükleyerek yerelleştirmeye karar verirseniz, yeniden dağıtılabilir birleştirme dosyalarını (. msm) kullanmanız gerekir. Örneğin, uygulamanızı bir x86 bilgisayarda yerelleştirmek isterseniz, bir x86 bilgisayar için Microsoft_VC`<version>`_MFCLOC_x86. msm ' i yükleme paketine birleştirmeniz gerekir.
...
MFC DLL 'sini uygulama yerel klasörünüzde dağıtarak MFC yerelleştirme dll 'Lerini yeniden dağıtabilirsiniz. Görsel C++ kitaplıkları yeniden dağıtma hakkında daha fazla bilgi için bkz. [ C++ görsel dosyaları yeniden dağıtma](redistributing-visual-cpp-files.md).
