...
MFC projeleri her zaman Windows 'un desteklediği birkaç başlık dosyasını içerir. Bunlar `#include` , dosyadaki deyimler aracılığıyla eklenir `StdAfx.h` :
...
Üst bilgi dosyaları, MFC ortak değerlerinin sembol KIMLIĞI değerlerini içerir. Bu semboller yalnızca bir MFC projesinde çalışırken kullanılabilir. `AFX_`Ön ek, standart sembol adı önekleri tarafından izlenir.
