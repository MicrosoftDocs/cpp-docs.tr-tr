---
title: "Varsayılan sinyaller | Microsoft Docs"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
ms.technology:
- cpp-language
ms.tgt_pltfrm: 
ms.topic: article
dev_langs:
- C++
helpviewer_keywords:
- default signals
- defaults, signals
ms.assetid: db9fc17b-75c0-4d33-86be-c536584bbede
caps.latest.revision: 
author: mikeblome
ms.author: mblome
manager: ghogen
ms.workload:
- cplusplus
ms.openlocfilehash: 690ff46bf8942bb124f374d02a229ace731476ed
ms.sourcegitcommit: 8fa8fdf0fbb4f57950f1e8f4f9b81b4d39ec7d7a
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 12/21/2017
---
# <a name="default-signals"></a>Varsayılan Sinyaller
**ANSI 4.7.1.1** durumunda denk **sinyal (***SIG***, sıg_dfl)** sinyal engelleyen bir sinyal işleyici çağrıdan önce yürütülmedi, gerçekleştirilir  
  
 Bir program çalışmaya başladığında, sinyaller varsayılan durumlarına ayarlanır.  
  
## <a name="see-also"></a>Ayrıca Bkz.  
 [Kitaplık İşlevleri](../c-language/library-functions.md)