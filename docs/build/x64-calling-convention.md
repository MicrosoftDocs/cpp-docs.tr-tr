...
Setjmpex. h veya setjmp. h dahil ettiğinizde, [setjmp](../c-runtime-library/reference/setjmp.md) veya [longjmp](../c-runtime-library/reference/longjmp.md) 'ye yapılan tüm çağrılar, yok ediciler ve `__finally` çağrılar çağıran bir geriye doğru sonuçlanır.  Bu, setjmp. h dahil olmak üzere, yan tümcelerde `__finally` ve yıkıcılarda çağrılan x86 'dan farklıdır.

Geçerli yığın işaretçisini `setjmp` , geçici olmayan kayıtları ve MXCSR yazmaçlarını koruyan bir çağrı.  En son `longjmp` `setjmp` çağrı sitesine geri dönmek ve yığın işaretçisi, geçici olmayan Yazmaçları ve MXCSR yazmaçlarını en son `setjmp` çağrıya göre korunan duruma geri döndürecek şekilde çağırır.
