...
Üzerinde x86 veya x64 mimarileri makinelerinden aynı Visual C++ kaynak kodu ARM mimariyi farklı sonuçlar doğurabilir.  
...
*Tanımsız davranış*  
C++ Standart tanımlamaz ve makul sonuç sahip bir işlem tarafından neden davranış — örneğin, bir kayan nokta değer imzalanmamış tamsayı dönüştürme veya bir değer, negatif veya sayısını aşıyor bir sayı konumları ile kaydırma BITS, yükseltilen yazın.  
  
*Uygulama tanımlı davranış*  
C++ Standart tanımlamak ve belge derleyici satıcı gerektirir davranışı. Bunun nedenle taşınabilir olmayabilir olsa bile bir programın güvenli bir şekilde uygulama tanımlı davranış üzerinde güvenebilirsiniz. Uygulama tanımlı davranış örnekleri yerleşik veri türlerini ve bunların hizalama gereksinimleri boyutları içerir. Uygulama tanımlı davranış tarafından etkilenen bir işlem örneği değişken bağımsız değişken listesi erişiyor.  
  
*Belirtilmeyen davranışı*  
C++ Standart kasıtlı olarak belirleyici olmayan bırakır davranışı. Davranış belirleyici değerlendirilir rağmen belirtilmeyen davranışının belirli çağrılarını derleyici uygulaması tarafından belirlenir. Ancak, bir derleyici satıcısının sonucu önceden veya karşılaştırılabilir çağrılarını arasında tutarlı davranışı garanti gereksinimi yoktur ve belgeleri için gereksinimi yoktur. Belirtilmeyen davranışı, hangi alt ifadeler sırayla örneğidir — bir işlev çağrısı için bağımsız değişkenleri şunlardır: değerlendirilir.  
...
Microsoft C++ Derleyici, iki farklı yorumlar destekleyen `volatile` derleyici anahtarları kullanarak belirtebilirsiniz depolama niteleyicisi. **/Volatile:ms** anahtar x86 hem de Microsoft derleyici üzerinde x64 geleneksel durumlarda bu güçlü bellek modeli nedeniyle bırakıldı gibi güçlü sıralamasını, garanti volatile semantiği genişletilmiş Microsoft seçer mimariler. **/Volatile:iso** anahtar güçlü sıralama garanti etmez katı C++ Standart volatile semantiği seçer.  
...
X86 hem x64 mimarileri varsayılandır **/volatile:ms** Microsoft C++ derleyicisini kullanarak bu mimari için zaten oluşturuldu yazılımın çoğu üzerlerinde kullandığından. X86 hem x64 programlar derlerken belirtebilirsiniz **/volatile:iso** anahtarı geleneksel volatile semantiğini gereksiz bağımlılık önlemeye yardımcı olmak ve taşınabilirlik yükseltmek için.  
