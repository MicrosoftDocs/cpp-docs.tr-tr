...
3.  Ham bir işaretçiyi bir **yeni**-akıllı işaretçi Oluşturucudaki nesne. (Bazı yardımcı program işlevleri veya akıllı işaretçi oluşturucuları bunu sizin için yapar.)  
  
4.  Aşırı yüklenen kullanın `->` ve `*` işleçlerini nesnelere erişmek üzere.  
