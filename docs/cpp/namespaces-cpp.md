...
Ad alanı, tanımlayıcılara kapsam sağlayan bildirime dayalı bir bölgedir (türlerin, işlevlerin, değişkenlerin adları, vb.). Ad alanları, kodu mantıksal gruplar halinde düzenlemek ve özellikle kod tabanınız birden çok kitaplık içerdiğinde ortaya çıkabilecek ad çakışmalarını engellemek için kullanılır. Ad alanı kapsamındaki tüm tanımlayıcılar, nitelendirme olmadan bir diğeri ile görünür. Ad alanı dışındaki tanımlayıcılar, her tanımlayıcı için tam adı `std::vector<std::string> vec;` (örneğin, ya da tek bir tanımlayıcı () için bir [using bildirimi](../cpp/using-declaration.md) `using std::string` veya ad alanındaki () tüm tanımlayıcılar için bir [using yönergesi](../cpp/namespaces-cpp.md#using_directives) kullanarak üyelere erişebilir `using namespace std;` . Üstbilgi dosyalarındaki kod her zaman tam nitelikli ad alanı adını kullanmalıdır.
...
Contosodata. cpp içindeki işlev uygulamaları, dosyanın en üstüne bir yönerge yerleştirseniz bile tam nitelikli adı kullanmalıdır **`using`** :
...
Bir tanımlayıcı açık bir ad alanında bildirilirse, örtük genel ad alanının bir parçasıdır. Genel olarak, genel ad alanında olması gereken giriş noktası [ana işlevi](../c-language/main-function-and-program-execution.md)dışında, mümkün olduğunda genel kapsamda bildirim oluşturmaktan kaçınmaya çalışın. Genel tanımlayıcıyı açıkça nitelemek için, içinde olduğu gibi, ad olmadan kapsam çözümleme işlecini kullanın `::SomeFunction(x);` . Bu, tanımlayıcıyı diğer ad alanındaki aynı ada sahip herhangi bir şeyle ayırt eder ve ayrıca kodunuzun başkalarının anlaması için daha kolay hale getirmeye yardımcı olur.

## <a name="the-std-namespace"></a>std ad alanı

Tüm C++ standart kitaplık türleri ve işlevleri `std` içinde iç içe yerleştirilmiş ad alanı veya ad alanlarında bildirilmiştir `std` .
...
Aşağıdaki örnek, her biri iç içe yerleştirilmiş bir ad alanında bir arabirimin iki sürümünü gösterir. `v_20`Ad alanı, arabirimden bazı değişikliklere sahiptir `v_10` ve satır içi olarak işaretlenir. Yeni kitaplığı ve çağrıları kullanan istemci kodu `Contoso::Funcs::Add` v_20 sürümünü çağıracaktır. Çağrı yapmaya çalışacak kod `Contoso::Funcs::Divide` Şimdi bir derleme zamanı hatası alacak. Bu işleve gerçekten ihtiyaç duyuyorsanız, açıkça çağırarak sürüme erişmeye devam edebilirler `v_10` `Contoso::v_10::Funcs::Divide` .
