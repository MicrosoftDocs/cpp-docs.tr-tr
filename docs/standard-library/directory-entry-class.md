...
Her iki üye işlev dönüş `mysymstat` büyük olasılıkla ilk gibi değiştirilemez: Varsa `status_known(mysymstat)` hiçbir şey yapma. Aksi takdirde, `mysymstat = symlink_status(mypval)`.
