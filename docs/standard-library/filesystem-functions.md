...
ms.locfileid: "68240706"
---
# <a name="ltfilesystemgt-functions"></a>&lt;dosya sistemi&gt; işlevleri

Bu işlevleri ücretsiz [ \<filesystem >](../standard-library/filesystem.md) üstbilgi yolları, dosyaları, çözümlemeyin, dizinleri ve birimleri değiştirme ve sorgu işlemleri yapın. Daha fazla bilgi ve kod örnekleri için bkz. [dosya sistemi gezintisi (C++)](../standard-library/file-system-navigation.md).
...
Aksi takdirde `is_regular_file(f)`, ardından:
...
Aksi takdirde `is_directory(f) && (opts & copy_options::recursive || !opts)`, ardından:
...
Varsa `exists(to) && !(opts & (copy_options::skip_existing | copy_options::overwrite_existing | copy_options::update_existing))`, ardından bir rapor dosyası zaten var. bir hata olarak.

Aksi takdirde `!exists(to) || opts & copy_options::overwrite_existing || opts & copy_options::update_existing&& last_write_time(to) < last_write_time(from) || !(opts & (copy_options::skip_existing | copy_options::overwrite_existing | copy_options:update_existing))`, öznitelikleri ve içeriği kopyalama girişimi *gelen* dosyasına *için*. Kopyalama girişimi başarısız olursa hata olarak bildirin.
...
İşlevler dönüş **true** yalnızca *sol* ve *doğru* aynı dosya sistemi varlık seçin.
...
İşlevler tarafından seçilen dosyanın bayt cinsinden boyutu döndürür. *pval*, `exists(pval) && is_regular_file(pval)` ve dosya boyutu belirlenebilir. Aksi takdirde bir hata bildirecek ve dönüş `uintmax_t(-1)`.
...
Varsa `is_directory(pval)`, işlev verir `directory_iterator(pval) == directory_iterator()`; Aksi halde döndürür `file_size(pval) == 0`.
...
İşlevler tarafından seçmiş yol için izinler *pval* için `mask & perms::mask` denetiminde `perms & (perms::add_perms | perms::remove_perms)`. *Maske* biri içeriyor `perms::add_perms` ve `perms::remove_perms`.

Varsa `mask & perms::add_perms`, İşlevler izinleri ayarlamak `status(pval).permissions() | mask & perms::mask`. Aksi takdirde `mask & perms::remove_perms`, İşlevler izinleri ayarlamak `status(pval).permissions() & ~(mask & perms::mask)`. Aksi takdirde izinler işlevleri kümesine `mask & perms::mask`.
...
İşlevler dönüş **true** yalnızca `exists(symlink_status(pval))` ve dosyayı başarıyla kaldırıldı. Bir sembolik bağlantısını kendisi kaldırılmış seçtiği dosya değildir.
...
İşlevleri Yeniden Adlandır *gelen* için *için*. Bir sembolik bağlantısını kendisi yeniden adlandırılmış seçtiği dosya değildir.
...
İşlev tarafından seçilen birim hakkındaki bilgileri döndürür *pval*, bir yapı türü `space_info`. Yapısının `uintmax_t(-1)` için herhangi bir değer belirlenemiyor.
...
İşlevler pathname durum, dosya türü ve ilişkili izinler döndürür *pval*. Kendi test hedefine sembolik bağlantı olduğu halde dosya seçer.
...
İşlevler pathname hedefine sembolik bağlantı durumunu, dosya türü ve ilişkili izinler döndürür *pval*. İşlevler, aynı şekilde davranır `status(pval)` bir sembolik bağlantısını test kendisi dışında olduğuna göre dosya seçer.
...
İlk işlev gibi davranır `path(source)` ve ikinci işlevi gibi davranır `path(first, last)` dışında her durumda seçtiğiniz kaynak karakter öğeleri dizisi UTF-8, hangi dosya sistemi kodlanmış olarak alınır.
