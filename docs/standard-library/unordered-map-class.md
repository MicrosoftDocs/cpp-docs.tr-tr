...
`operator[]` öğe bir eşlemeye eklemek için kullanılabilir *m* kullanarak *m*[*anahtarı*] = `DataValue`; burada `DataValue` değeri `mapped_type` bir anahtara sahip öğe değerini *anahtar*.
