---
title: '&lt;atomic&gt;'
description: Standart C++ kitaplığının atomik üst bilgisinde kullanılabilen türleri ve işlevleri açıklar.
ms.date: 12/06/2019
f1_keywords:
- <atomic>
...
Bazı platformlarda, kilitleri kullanmadan bazı türler için atomik işlemleri verimli bir şekilde uygulamak mümkün olmayabilir `mutex` . Bir atomik tür, bu tür üzerinde hiçbir Atomik işlem kilit kullanıyorsa *kilit ücretsizdir* .

**C++ 11**: sinyal işleyicilerinde, `obj` veya true olduğunda bir nesne üzerinde Atomik işlemler gerçekleştirebilirsiniz `obj.is_lock_free()` `atomic_is_lock_free(x)` .

[Atomic_flag](../standard-library/atomic-flag-structure.md) sınıfı, bayrak tutan en düşük atomik bir tür sağlar **`bool`** . İşlemleri her zaman kilitsiz değildir.

Sınıf şablonu, `atomic<T>` bağımsız değişken türünün bir nesnesini depolar `T` ve bu depolanan değere atomik erişim sağlar. [Memckopyala](../c-runtime-library/reference/memcpy-wmemcpy.md) kullanılarak kopyalanabilen ve [memcmp](../c-runtime-library/reference/memcmp-wmemcmp.md)kullanarak eşitlik için test edilmiş herhangi bir tür kullanarak bu örneği oluşturabilirsiniz. Özellikle, bu gereksinimleri karşılayan Kullanıcı tanımlı türlerle ve birçok durumda kayan nokta türleriyle kullanabilirsiniz.
...
`atomic<integral>`Specialmeler tüm integral türleri için geçerlidir. Bunlar, birincil şablon aracılığıyla kullanılamayan ek işlemler sağlar.

Her `atomic<integral>` türün, `if directive` Bu türdeki işlemlerin kilitleme dışı olup olmadığını derleme zamanında tespit etmek için bir içinde kullanabileceğiniz bir makro vardır. Makronun değeri sıfırsa, türdeki işlemler kilit ücretsizdir. Değer 1 ise, işlemler kilit boş olabilir ve bir çalışma zamanı denetimi gereklidir. Değer 2 ise, işlemler kilitleme ücretsizdir. `atomic_is_lock_free`Çalışma zamanında, tür üzerindeki işlemlerin kilitli olup olmadığını anlamak için işlevini kullanabilirsiniz.

İntegral türlerinin her biri için, söz konusu integral türünün bir nesnesini yöneten, karşılık gelen bir adlandırılmış atomik tür vardır. Her `atomic_integral` tür, karşılık gelen örneklemeyle aynı üye işlevleri kümesine sahiptir `atomic<T>` ve üye olmayan atomik işlevlerden herhangi birine geçirilebilir.

|`atomic_integral`Türüyle|Integral türü|`atomic_is_lock_free`Makroya|
|----------------------------|-------------------|---------------------------------|
...
Üst bilgide tanımlanmış bazı türlerin atomik şablonunun uzmanlıkları için typedef adları vardır \<inttypes.h> .
...
|[atomic_flag Yapısı](../standard-library/atomic-flag-structure.md)|Bir bayrağı sıradan olarak ayarlayan ve temizleyen bir nesneyi tanımlar **`bool`** .|
...
Aşağıdaki listede, ' ın `_explicit` `_explicit` örtük [memory_order](../standard-library/atomic-enums.md#memory_order_enum) bağımsız değişkenlerine sahip olmaları dışında, içinde bitolmayan işlevler karşılık gelen semantiği vardır `memory_order_seq_cst` .
...
|[atomic_fetch_and](../standard-library/atomic-functions.md#atomic_fetch_and)|`and`Belirtilen bir değer ve var olan bir depolanan değer üzerinde bir bit düzeyinde gerçekleştirir.|
|[atomic_fetch_and_explicit](../standard-library/atomic-functions.md#atomic_fetch_and_explicit)|`and`Belirtilen bir değer ve var olan bir depolanan değer üzerinde bir bit düzeyinde gerçekleştirir.|
|[atomic_fetch_or](../standard-library/atomic-functions.md#atomic_fetch_or)|`or`Belirtilen bir değer ve var olan bir depolanan değer üzerinde bir bit düzeyinde gerçekleştirir.|
|[atomic_fetch_or_explicit](../standard-library/atomic-functions.md#atomic_fetch_or_explicit)|`or`Belirtilen bir değer ve var olan bir depolanan değer üzerinde bir bit düzeyinde gerçekleştirir.|
...
|[atomic_fetch_xor](../standard-library/atomic-functions.md#atomic_fetch_xor)|`exclusive or`Belirtilen bir değer ve var olan bir depolanan değer üzerinde bir bit düzeyinde gerçekleştirir.|
|[atomic_fetch_xor_explicit](../standard-library/atomic-functions.md#atomic_fetch_xor_explicit)|`exclusive or`Belirtilen bir değer ve var olan bir depolanan değer üzerinde bir bit düzeyinde gerçekleştirir.|
|[atomic_flag_clear](../standard-library/atomic-functions.md#atomic_flag_clear)|Bir nesnedeki bayrağı olarak ayarlar `atomic_flag` **`false`** .|
|[atomic_flag_clear_explicit](../standard-library/atomic-functions.md#atomic_flag_clear_explicit)|Bir nesnedeki bayrağı olarak ayarlar `atomic_flag` **`false`** .|
|[atomic_flag_test_and_set](../standard-library/atomic-functions.md#atomic_flag_test_and_set)|Bir nesnedeki bayrağı olarak ayarlar `atomic_flag` **`true`** .|
|[atomic_flag_test_and_set_explicit](../standard-library/atomic-functions.md#atomic_flag_test_and_set_explicit)|Bir nesnedeki bayrağı olarak ayarlar `atomic_flag` **`true`** .|
|[atomic_init](../standard-library/atomic-functions.md#atomic_init)|Bir nesne içindeki saklı değeri ayarlar `atomic` .|
