---
title: adopt_lock_t Yapısı
ms.date: 11/04/2016
f1_keywords:
- mutex/std::adopt_lock_t
ms.assetid: 549972fb-e2e9-4415-8657-c159b09aa3f4
ms.openlocfilehash: f613aeda7b92d7d452af9449bfbf47419cb45470
ms.sourcegitcommit: c21b05042debc97d14875e019ee9d698691ffc0b
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 06/09/2020
ms.locfileid: "84623897"
---
# <a name="adopt_lock_t-structure"></a>adopt_lock_t Yapısı

Bir [adopt_lock](mutex-functions.md#adopt_lock)tanımlamak için kullanılan bir türü temsil eder.

## <a name="syntax"></a>Söz dizimi

```cpp
struct adopt_lock_t;
```

## <a name="requirements"></a>Gereksinimler

**Üst bilgi:**\<mutex>

**Ad alanı:** std

## <a name="see-also"></a>Ayrıca bkz.

[Üst bilgi dosyaları başvurusu](cpp-standard-library-header-files.md)\
[\<mutex>](mutex.md)
