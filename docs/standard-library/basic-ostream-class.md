...
`right` Rvalue başvuru türünde bir nesneye `basic_ostream`.
...
`strbuf` Bir işaretçi bir **stream_buf** nesnesi.

`val` Akışa yazmak için bir öğe.
...
\<Ostream > Üstbilgi ayrıca çeşitli genel ekleme işleçlerini tanımlar. Daha fazla bilgi için bkz: [işleci <<](../standard-library/ostream-operators.md#op_lt_lt).
...
`right` Bir `rvalue` başvuru bir `basic_ostream` nesnesi.
...
`_Off` Göreli uzaklık `_Way`.

`_Way` Aşağıdakilerden birini [ios_base::seekdir](../standard-library/ios-base-class.md#seekdir) numaralandırmalar.
...
sınıf sentry {ortak: açık sentry (basic_ostream\<Elem, Tr > & _Ostr); işleci bool() const; ~ sentry();};
...
`right` Bir başvuru bir `basic_ostream` nesnesi.
...
`count` Akışa yerleştirilecek karakter sayısı.

`str` Akışa yerleştirilecek karakter.
