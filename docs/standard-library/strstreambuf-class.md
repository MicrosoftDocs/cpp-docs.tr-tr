...
### <a name="parameters"></a>Parametreler  
 `_Meta`  
 Arabelleğe eklemek için karakter veya `EOF`.  
 ...
 *_Allocfunc*  
 ...
 `count`  
 Gösterdiği arabellek uzunluğu belirler `_Getptr`. Varsa `_Getptr` bir bağımsız değişken değil (ilk Oluşturucusu form), önerilen bir ayırma arabellekleri boyutu.  
 ...
 *_Freefunc*  
 ...
 `_Putptr`  
 Çıktı için kullanılan arabellek.  
