...
Kullanırken dikkate alınması gereken bazı ipuçları \<rastgele >:
...
De nereden seçmek için pek çok seçenek \<rastgele > Üstbilgi ve bunlardan herhangi birinin eski C çalışma zamanı işlevine tercih `rand()`. Sorun nedir hakkında bilgi için `rand()` ve nasıl \<rastgele > Bu eksik adresleri bkz [bu videoyu](http://go.microsoft.com/fwlink/p/?linkid=397615).
...
Aşağıdaki bölümlerde sağlanan Tekdüzen rastgele sayı oluşturucuları (URNGs) listesinde \<rastgele > Üstbilgi.
...
Bu bölümde sağlanan işleçleri listeler \<rastgele > Üstbilgi.
