...
### <a name="remarks"></a>Açıklamalar  
 İşleç parametresinin bir türde bir değer döndürür `bool`. Dönüştürme sonucu `bool` olan `true` zaman `get() != 0`, aksi takdirde `false`.  
...
### <a name="parameters"></a>Parametreler  
 `Other`  
 Bağımsız değişken işaretçiyi tarafından denetlenen türü.  
...
### <a name="parameters"></a>Parametreler  
 `sp`  
 Paylaşılan işaretçisi ile değiştirilecek.  
