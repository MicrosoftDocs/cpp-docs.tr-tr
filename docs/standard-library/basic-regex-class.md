...
### <a name="parameters"></a>Parametreler  
 ...
 `STalloc`  
 Allocator sınıfı bir dize kaynağı.  
  
 `InIt`  
 Yineleyici türü için bir aralığı kaynağı girin.  
  
 `right`  
 Kopyalamak için Regex kaynağı.  
  
 `ptr`  
 Kopyalamak için dizisi başlangıcını işaretçi.  
  
 `flags`  
 Kopyalanırken eklemek için seçenek bayraklarını sözdizimi.  
  
 `len/TD>`  
 Kopyalamak için sırası uzunluğu.  
 ...
 `last`  
 Kopyalamak için dizisi sonu.  
  
 `IList`  
 Kopyalamak için initializer_list.  
...
### <a name="parameters"></a>Parametreler  
 `loc`  
 Depolamak için yerel ayar nesnesi.  
...
### <a name="parameters"></a>Parametreler  
 `right`  
 İle değiştirme için normal ifade nesnesi.  
