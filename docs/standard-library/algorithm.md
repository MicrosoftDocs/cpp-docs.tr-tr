...
Algoritmalar gerçekleştiren C++ Standart Kitaplığı kapsayıcı şablon işlevini tanımlar.
...
Bunlar, çeşitli veri yapıları üzerinde çalışabilir olduğundan C++ Standart Kitaplığı algoritmaları geneldir. Üzerinde çalışabildiği veri yapıları yalnızca C++ Standart Kitaplığı kapsayıcı sınıfları gibi dahil `vector` ve `list`, ancak aynı zamanda program tarafından tanımlı veri yapılarını ve de belirli gereksinimleri karşılayan öğeleri dizileri algoritma. C++ Standart Kitaplığı algoritmaları dolaylı olarak yineleyiciler üzerinden bir kapsayıcının öğelerine erişerek ve bu genellik düzeyine erişir düzeyini elde edin.

C++ Standart Kitaplığı algoritmaları genellikle kendi başlangıç veya bitiş konumları tarafından belirtilen yineleyici aralıklarını işler. Aralıklardaki tüm işaretçilerin tekrar başvurulabilir olması ve her aralığın dizisinde olması gerekliliği bakımından, başvurulan aralıkların geçerli olması gerekir; son konum da artış yoluyla birinciden erişilebilir olmalıdır.

C++ Standart Kitaplığı algoritmaları, işlemler ve her bir C++ Standart Kitaplığı kapsayıcının üye işlevleri tarafından desteklenen eylemleri genişletir ve, örneğin, kapsayıcı nesnelerinin aynı anda farklı türde çalışmaya izin verin. İki sonek, algoritmaların amacı hakkında bilgi iletmek için kullanılmıştır.

- `_if` Soneki, algoritmanın öğelerinin değerlerini değil öğelerin değerlerini çalıştıran işlev nesneleriyle kullanılır belirtir. `find_if` Algoritması değerleri bir işlev nesnesi tarafından belirtilen ölçütü karşılayan öğeleri arar ve `find` algoritması belirli bir değeri arar.

- _Copy soneki algoritmanın yalnızca öğelerinin değerlerini yönetmeyeceğini, aynı zamanda değiştirilen değerlerin bir hedef aralığına da kopyalayacağını belirtir. `reverse` Algoritması bir aralık içindeki öğelerin sırasını tersine çevirir ve `reverse_copy` algoritması da sonucu bir hedef aralığına kopyalar.

C++ Standart Kitaplığı algoritmaları genellikle kendi amacı ve gereksinimleri hakkında bir şeyler belirten gruplar içinde sınıflandırılır. Değiştirme değiştirmeyen öğelerin değerini değiştiren algoritmalara bunlar olmayan algoritmaları. Algoritmaların değiştirilmesi öğelerin sırasını değiştirir, ancak kendi öğelerinin değerlerini değiştirmez. Algoritmaların kaldırılması bir aralıktaki ya da aralıktaki bir kopyadaki öğeleri yok sayabilir. Algoritmaların sıralanması, çeşitli yollarla bir aralıktaki öğeleri yeniden sıralamak ve sıralanmış aralıktaki algoritmalar yalnızca öğeleri belirli bir şekilde sıralanmış aralıkları işlem.

Sayısal işlemler için sağlanan C++ Standart Kitaplığı sayısal algoritmalarının kendi üst bilgi dosyası sahip [ \<sayısal >](../standard-library/numeric.md), ve işlev nesneleri ve bağdaştırıcıları üstbilgisinde tanımlanır [ \<işlevsel >](../standard-library/functional.md) Boolean değerleri döndüren işlev nesneleri koşullar olarak bilinir. Varsayılan ikili koşul karşılaştırmasıdır `operator<`. Genelde, sıralanan öğelerin küçüktür biçiminde karşılaştırılabilir olması gerekir; böylece, herhangi iki öğe belirtildiğinde, eşit oldukları (yani birinin diğerinden daha küçük olmadığı anlamında) veya birinin diğerinden küçük olduğu belirlenebilir. Bu eşdeğer olmayan öğeler arasında bir sıralamaya neden olur.
...
|[Dolgu](../standard-library/algorithm-functions.md#fill)|Aynı yeni değeri belirtilen bir aralıktaki her öğeye atar.|
...
|[is_heap](../standard-library/algorithm-functions.md#is_heap)|Döndürür **true** Belirtilen aralıktaki öğeler bir yığın biçimlendiriyorsa.|
|[is_heap_until](../standard-library/algorithm-functions.md#is_heap_until)|Döndürür **true** Belirtilen aralıktaki son öğeye kadar bir yığın biçimlendiriyorsa.|
|[is_partitioned](../standard-library/algorithm-functions.md#is_partitioned)|Döndürür **true** verili aralıktaki tüm öğeleri, test, **true** test herhangi bir öğeden önce gelen bir koşul için **false**.|
|[is_permutation](../standard-library/algorithm-functions.md#is_permutation)|Belirtilen aralıktaki öğeler, geçerli bir sıralamaya form olup olmadığını belirler.|
|[is_sorted](../standard-library/algorithm-functions.md#is_sorted)|Döndürür **true** sıralanmış olarak belirtilen aralıktaki öğeler varsa.|
|[is_sorted_until](../standard-library/algorithm-functions.md#is_sorted_until)|Döndürür **true** sıralanmış olarak belirtilen aralıktaki öğeler varsa.|
...
|[en fazla](../standard-library/algorithm-functions.md#max)|İki nesneyi karşılaştırır ve ikisinden büyük olanı döndürür, burada sıralama ölçütü ikili bir koşula göre belirtilebilir.|
...
|[minmax_element](../standard-library/algorithm-functions.md#minmax_element)|Tarafından gerçekleştirilen işi yapar [min_element](../standard-library/algorithm-functions.md#min_element) ve [max_element](../standard-library/algorithm-functions.md#max_element) tek bir çağrıdaki.|
...
|[none_of](../standard-library/algorithm-functions.md#none_of)|Döndürür **true** koşul olduğunda hiçbir zaman verilen aralıktaki öğeler arasında yok.|
|[nth_element](../standard-library/algorithm-functions.md#nth_element)|Doğru şekilde bularak öğelerin bir aralığını bölümler *n*öğedeki aralığındaki dizinin önündeki tüm öğeler ona eşit veya ve sırayla izleyen tüm öğeler, böylece daha büyük veya eşit kendisine.|
...
|[partition_copy](../standard-library/algorithm-functions.md#partition_copy)|Bir koşul olduğu öğeleri kopyalar **true** bir hedef ve koşulun **false** diğerine. Öğeler belirtilen bir aralıktan gelmelidir.|
...
|[random_shuffle](../standard-library/algorithm-functions.md#random_shuffle)|Bir dizi düzenlemelerinin *N* birine bir aralıktaki öğeleri *N*! olası düzenlemeleri rastgele seçilmiş.|
...
|[karışık](../standard-library/algorithm-functions.md#shuffle)|Seçeneği (düzenlemelerinin) öğeleri bir rastgele sayı oluşturucusunu kullanarak belirtilen aralık.|
