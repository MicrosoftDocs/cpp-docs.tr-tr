...
#### <a name="parameters"></a>Parametreler  
 ...
 `CharType`  
 Karakter türünü temsil eden tür `ostream_iterator`. Bu bağımsız değişken isteğe bağlıdır ve varsayılan değer `char`.  
  
 `Traits`  
 Karakter türünü temsil eden tür `ostream_iterator`. Bu bağımsız değişken isteğe bağlıdır ve varsayılan değer `char_traits` \< *CharType >.*  
...
### <a name="parameters"></a>Parametreler  
 `val`  
 Nesne türü değeri `Type` çıkış akışı eklenecek.  
...
### <a name="parameters"></a>Parametreler  
 `_Ostr`  
 Çıkış akışı türü [ostream_iterator::ostream_type](#ostream_type) üzerinden yinelendiğinde için.  
  
 `_Delimiter`  
 Değerler arasında çıktı akışına eklenir ve sınırlayıcı.  
