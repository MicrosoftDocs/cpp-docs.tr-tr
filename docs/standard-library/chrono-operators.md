---
title: '&lt;chrono&gt; işleçler | Microsoft Docs'
ms.custom: ''
ms.date: 11/04/2016
ms.topic: reference
f1_keywords:
...
*Sol* sol `duration` veya `time_point` nesne.

*Sağ* sağ `duration` veya `time_point` nesne.

*Zaman* A `time_point` nesne.
...
`Left` Sol `duration` veya `time_point` nesne.

`Right` Sağa `duration` veya `time_point` nesne.
...
*Div* bir tamsayı değeri.

*Sol* sol `duration` nesne.

*Sağ* sağ `duration` nesne.
