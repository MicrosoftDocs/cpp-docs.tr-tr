---
description: 'Daha fazla bilgi edinin: &lt; Tarih/s &gt; işleçleri'
...
İkinci işlev, zaman `time_point` içinde belirtilen zaman aralığında, *süresi ile* temsil edilen zaman aralığının ne kadar süre içinde olduğunu gösteren bir noktayı temsil eden bir nesne döndürür. 
...
Her işlev `duration` , Aralık uzunluğu, *süre* uzunluğu ile  çarpıldığı bir nesne döndürür.
...
İkinci ve üçüncü işlevleri, zaman aralığı ile zaman içinde zaman içindeki noktadan sonra gelen zaman `time_point` aralığını temsil eden bir nesne döndürür .
