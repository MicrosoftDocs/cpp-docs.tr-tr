---
title: C++ Destek Kitaplığı
ms.date: 11/04/2016
ms.topic: reference
helpviewer_keywords:
- Managed Support Library
- C++ Support Library
- Visual C++ Support Library
ms.assetid: fe6246eb-9ac7-478d-a272-dc9b09b14788
ms.openlocfilehash: 88b7112962e003d825bac85112224130b5827b9c
ms.sourcegitcommit: 0ab61bc3d2b6cfbd52a16c6ab2b97a8ea1864f12
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 04/23/2019
ms.locfileid: "62209141"
---
# <a name="c-support-library"></a>C++ Destek Kitaplığı

C++ destek kitaplığı, C++'da yönetilen programlamayı destekleyen sınıflar sağlar.

## <a name="in-this-section"></a>Bu Bölümde

[Overview of Marshaling in C++](../dotnet/overview-of-marshaling-in-cpp.md)

[Resource Management Classes](../dotnet/resource-management-classes.md)

[Eşitleme (lock Sınıfı)](../dotnet/synchronization-lock-class.md)

[Belirli Bir Uygulama Etki Alanında İşlevleri Çağırma](../dotnet/calling-functions-in-a-specific-application-domain.md)

[com::ptr](../dotnet/com-ptr.md)