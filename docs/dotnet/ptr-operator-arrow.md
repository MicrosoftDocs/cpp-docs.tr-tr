...
Bu örnekte kullanan bir CLR sınıf uygulayan bir `com::ptr` kendi özel üye sarmalamak için `IXMLDOMDocument` nesne. `WriteDocument` İşlevleri kullanan `operator->` çağrılacak `get_firstChild` belge nesnenin bir üyesi.
