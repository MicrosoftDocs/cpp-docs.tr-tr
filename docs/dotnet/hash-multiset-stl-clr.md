...
ms.locfileid: "87221399"
---
# <a name="hash_multiset-stlclr"></a>hash_multiset (STL/CLR)

Şablon sınıfı, çift yönlü erişime sahip olan öğelerin değişen uzunluklu bir dizisini denetleyen bir nesneyi tanımlar. Bir `hash_multiset` dizi öğeyi bir karma tablo olarak yönetmek için kapsayıcıyı, çift yönlü bağlantılı düğümlerin bir listesini depolayan her tablo girişini ve bir öğeyi depolayan her düğümü kullanın. Her öğenin değeri, diziyi sıralamak için anahtar olarak kullanılır.
...
Kapsayıcının yalnızca, anahtarları eşdeğer sıralama (ve aynı tamsayı değeri ile aynı karma değer) olan öğelerin bir demet içinde bitişik olduğunu garanti edin. Şablon sınıfının [hash_set (STL/CLR)](../dotnet/hash-set-stl-clr.md)aksine, şablon sınıfının bir nesnesi `hash_multiset` tüm öğeler için anahtarların benzersiz olmasını gerektirmez. (İki veya daha fazla anahtarın eşdeğer sıralaması olabilir.)
...
Üye işlevi boş bir denetlenen dizi için true döndürür. [Hash_multiset:: size (STL/CLR)](../dotnet/hash-multiset-size-stl-clr.md)eşdeğerdir `() == 0` . Hash_multiset boş olup olmadığını test etmek için kullanırsınız.
...
İlk üye *işlevi, öğesinin*işaret ettiği denetlenen sıranın öğesini kaldırır ve kaldırılan öğenin ötesinde ilk öğeyi atayan bir yineleyici döndürür ya da böyle bir öğe yoksa [hash_multiset:: End (STL/CLR)](../dotnet/hash-multiset-end-stl-clr.md) `()` . Tek bir öğeyi kaldırmak için bunu kullanırsınız.
...
denetlenen sırayı, `right.begin()` `right.end()` varsayılan sıralama koşulu ile ve varsayılan karma işlevle birlikte [,) sırasıyla başlatır. Bu, varsayılan sıralama koşulu ve karma işlevi ile *hash_multiset nesnesi tarafından*denetlenen sıranın bir kopyası olan bir ilk denetimli sıra belirtmek için kullanılır.
...
Üye işlevi, denetimli dizideki ilk öğeyi, `X` *anahtar* ile aynı demetini karma hale getirir ve *anahtara*eşdeğer sıralama içerir. Böyle bir öğe yoksa [hash_multiset:: End (STL/CLR)](../dotnet/hash-multiset-end-stl-clr.md)döndürür `()` ; Aksi takdirde, öğesini atayan bir yineleyici döndürür `X` . Belirtilen bir anahtarla eşleşen, denetlenen dizide bulunan bir öğe dizisinin başlangıcını bulmak için bunu kullanırsınız.
...
Üye işlevi karma tabloyu yeniden oluşturur ve [hash_multiset:: load_factor (STL/CLR)](../dotnet/hash-multiset-load-factor-stl-clr.md) `() <=` [hash_multiset:: max_load_factor (STL/CLR)](../dotnet/hash-multiset-max-load-factor-stl-clr.md). Aksi takdirde, karma tablo, bir ekleme işleminden sonra gerektiğinde yalnızca boyut artar. (Boyut olarak hiçbir şekilde azalmayın.) Karma tablonun boyutunu ayarlamak için bunu kullanırsınız.
...
Üye işlevi denetlenen sıranın uzunluğunu döndürür. Bu, şu anda denetlenen dizideki öğelerin sayısını tespit etmek için kullanılır. Her şey için, sıranın sıfır dışında bir boyut içerip içermediğini öğrenmek için bkz. [hash_multiset:: Empty (STL/CLR)](../dotnet/hash-multiset-empty-stl-clr.md) `()` .
...
Üye işlevi, denetimli dizideki son öğeyi, `X` *anahtar* ile aynı demetini karma hale getirir ve *anahtara*denk sıralama içerir. Böyle bir öğe yoksa veya `X` denetlenen dizideki son öğe ise [hash_multiset:: End (STL/CLR)](../dotnet/hash-multiset-end-stl-clr.md)döndürür `()` ; Aksi takdirde ilk öğeyi daha sonra atayan bir yineleyici döndürür `X` . Belirtilen bir anahtarla eşleşen denetimli dizide bulunan bir öğe dizisinin sonunu bulmak için bunu kullanırsınız.
