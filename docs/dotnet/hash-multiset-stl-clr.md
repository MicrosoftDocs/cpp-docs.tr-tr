...
ms.locfileid: "80208696"
---
# <a name="hash_multiset-stlclr"></a>hash_multiset (STL/CLR)

Şablon sınıfı, çift yönlü erişime sahip olan öğelerin değişen uzunluklu bir dizisini denetleyen bir nesneyi tanımlar. Bir dizi öğeyi karma tablo olarak yönetmek için kapsayıcı `hash_multiset`, çift yönlü bağlantılı düğümlerin bir listesini depolayan her tablo girişini ve bir öğeyi depolayan her bir düğümü kullanın. Her öğenin değeri, diziyi sıralamak için anahtar olarak kullanılır.

Aşağıdaki açıklamada `GValue`, ikincisi bir başvuru türü olmadığı sürece bu *anahtarla* aynı `GKey`ve bu durumda `Key^`.
...
|[hash_multiset::const_reverse_iterator (STL/CLR)](#const_reverse_iterator)|Denetlenen sıra için bir sabit ters yineleyicinin türü.|
|[hash_multiset::difference_type (STL/CLR)](#difference_type)|İki öğe arasındaki mesafe (muhtemelen imzalı) türü.|
|[hash_multiset::generic_container (STL/CLR)](#generic_container)|Kapsayıcının genel arabiriminin türü.|
|[hash_multiset::generic_iterator (STL/CLR)](#generic_iterator)|Kapsayıcının genel arabirimi için bir yineleyici türü.|
|[hash_multiset::generic_reverse_iterator (STL/CLR)](#generic_reverse_iterator)|Kapsayıcının genel arabirimi için ters yineleyicinin türü.|
|[hash_multiset::generic_value (STL/CLR)](#generic_value)|Kapsayıcının genel arabirimi için bir öğenin türü.|
...
|[hash_multiset::key_compare (STL/CLR)](#key_compare)|İki anahtar için sıralama temsilcisi.|
...
|[hash_multiset::reverse_iterator (STL/CLR)](#reverse_iterator)|Denetlenen sıra için ters yineleyicinin türü.|
|[hash_multiset::size_type (STL/CLR)](#size_type)|İki öğe arasındaki (negatif olmayan) bir mesafe türü.|
|[hash_multiset::value_compare (STL/CLR)](#value_compare)|İki öğe değeri için sıralama temsilcisi.|
...
|[hash_multiset::bucket_count (STL/CLR)](#bucket_count)|Demetlerin sayısını sayar.|
...
|[hash_multiset::hash_delegate (STL/CLR)](#hash_delegate)|Bir anahtar için karma temsilciyi kopyalar.|
...
|[hash_multiset::key_comp (STL/CLR)](#key_comp)|Sıralama temsilcisini iki anahtar için kopyalar.|
|[hash_multiset::load_factor (STL/CLR)](#load_factor)|Demet başına ortalama öğeyi sayar.|
|[hash_multiset::lower_bound (STL/CLR)](#lower_bound)|Belirtilen anahtarla eşleşen aralığın başlangıcını bulur.|
|[hash_multiset::make_value (STL/CLR)](#make_value)|Değer nesnesi oluşturur.|
|[hash_multiset::max_load_factor (STL/CLR)](#max_load_factor)|Demet başına en yüksek öğe sayısını alır veya ayarlar.|
|[hash_multiset::rbegin (STL/CLR)](#rbegin)|Ters denetlenen sıranın başlangıcını belirtir.|
|[hash_multiset::rehash (STL/CLR)](#rehash)|Karma tabloyu yeniden oluşturur.|
|[hash_multiset::rend (STL/CLR)](#rend)|Ters denetlenen sıranın sonunu belirtir.|
...
|[hash_multiset::to_array (STL/CLR)](#to_array)|Denetimli sırayı yeni bir diziye kopyalar.|
|[hash_multiset::upper_bound (STL/CLR)](#upper_bound)|Belirtilen anahtarla eşleşen aralığın sonunu bulur.|
|[hash_multiset::value_comp (STL/CLR)](#value_comp)|İki öğe değeri için sıralama temsilcisini kopyalar.|

|İşleç|Açıklama|
|--------------|-----------------|
|[hash_multiset::operator= (STL/CLR)](#op)|Denetlenen sırayı değiştirir.|
...
Kapsayıcının yalnızca, anahtarları eşdeğer sıralama (ve aynı tamsayı değeri ile aynı karma değer) olan öğelerin bir demet içinde bitişik olduğunu garanti edin. Şablon sınıfının [hash_set (STL/CLR)](../dotnet/hash-set-stl-clr.md)aksine, şablon sınıfının `hash_multiset` bir nesnesi tüm öğeler için anahtarların benzersiz olmasını gerektirmez. (İki veya daha fazla anahtarın eşdeğer sıralaması olabilir.)
...
Hash_multiset çift yönlü yineleyiciler destekler, bu, denetlenen dizide bir öğe atayan bir yineleyici verilen bitişik öğelere adım adım yol açabilir. Özel bir baş düğüm [hash_multiset:: End (STL/CLR)](../dotnet/hash-multiset-end-stl-clr.md)`()`tarafından döndürülen Yineleyici öğesine karşılık gelir. Varsa, denetlenen sıradaki son öğeye ulaşmak için bu yineleyiciyi azaltamazsınız. Baş düğüme ulaşmak için bir hash_multiset yineleyicisini artırabilirsiniz ve daha sonra, `end()`eşit olarak karşılaştırırsınız. Ancak `end()`tarafından döndürülen yineleyici başvurusu oluşturamazsınız.

Bir hash_multiset öğesine, bir rastgele erişim Yineleyici gerektiren sayısal konumuna doğrudan başvurmayacağınızı unutmayın.

Hash_multiset Yineleyici, ilişkili kapsayıcısına bir tutamacı depolayan bir tutamacı ilişkili hash_multiset düğümüne depolar. Yineleyiciler yalnızca ilişkili kapsayıcı nesneleriyle birlikte kullanabilirsiniz. Hash_multiset Yineleyici, ilişkili hash_multiset düğümü bazı hash_multiset ilişkili olduğu sürece geçerli kalır. Ayrıca, geçerli bir yineleyici bir şekilde başvurulmasını sağlar--bunu, oluşturduğu öğe değerine erişmek veya değiştirmek için kullanabilirsiniz. `end()`eşit olmadığı sürece.
...
Üye işlevi etkin bir şekilde [hash_multiset:: Erase](../dotnet/hash-multiset-erase-stl-clr.md) (stl/clr)`(` [hash_multiset:: BEGIN (STL/](../dotnet/hash-multiset-begin-stl-clr.md) clr)`(),` [HASH_MULTISET:: End (STL/CLR](../dotnet/hash-multiset-end-stl-clr.md) )`())`çağırır. Denetlenen sıranın boş olduğundan emin olmak için bunu kullanırsınız.
...
Üye işlevi boş bir denetlenen dizi için true döndürür. [Hash_multiset:: size (STL/CLR)](../dotnet/hash-multiset-size-stl-clr.md)`() == 0`eşdeğerdir. Hash_multiset boş olup olmadığını test etmek için kullanırsınız.
...
Üye işlevi yineleyicilerin bir çiftini döndürür `cliext::pair<iterator, iterator>(` [hash_multiset:: lower_bound (STL/CLR)](../dotnet/hash-multiset-lower-bound-stl-clr.md)`(key),` [hash_multiset:: upper_bound (STL/CLR)](../dotnet/hash-multiset-upper-bound-stl-clr.md)`(key))`. Belirli bir anahtarla eşleşen denetimli dizide bulunan öğe aralığını öğrenmek için bunu kullanırsınız.
...
İlk üye *işlevi, öğesinin*işaret ettiği denetimli sıranın öğesini kaldırır ve kaldırılan öğenin ötesinde ilk öğeyi atayan bir yineleyici döndürür ya da böyle bir öğe yoksa [hash_multiset:: End (STL/CLR)](../dotnet/hash-multiset-end-stl-clr.md)`()`. Tek bir öğeyi kaldırmak için bunu kullanırsınız.
...
Denetlenen dizide en az bir öğe *anahtar*ile eşdeğer sıralamaya sahipse, üye işlevi bu öğelerden birini tanımlayarak bir yineleyici döndürür; Aksi takdirde [hash_multiset:: End (STL/CLR)](../dotnet/hash-multiset-end-stl-clr.md)`()`döndürür. Belirtilen bir anahtarla eşleşen denetimli dizide bulunan bir öğeyi bulmak için bunu kullanırsınız.
...
denetlenen sırayı, varsayılan sıralama koşulu ile ve varsayılan karma işlevi ile [`right.begin()`, `right.end()`) sırasıyla başlatır. Bu, varsayılan sıralama koşulu ve karma işlevi ile *hash_multiset nesnesi tarafından*denetlenen sıranın bir kopyası olan bir ilk denetimli sıra belirtmek için kullanılır.
...
## <a name="hash_multisetiterator-stlclr"></a><a name="iterator"></a>hash_multiset:: yineleyici (STL/CLR)
...
Üye işlevi `(float)`[hash_multiset:: size (STL/CLR)](../dotnet/hash-multiset-size-stl-clr.md)`() /` [hash_multiset:: bucket_count (STL/CLR)](../dotnet/hash-multiset-bucket-count-stl-clr.md)`()`döndürür. Bu, ortalama demet boyutunu bulmak için kullanılır.
...
Üye işlevi, *anahtar* ile aynı demetini karma hale uygulayan denetimli dizideki ilk öğeyi `X` belirler ve *anahtara*denk sıralama içerir. Böyle bir öğe yoksa, [hash_multiset:: End (STL/CLR)](../dotnet/hash-multiset-end-stl-clr.md)`()`; döndürür Aksi takdirde, `X`atayan bir yineleyici döndürür. Belirtilen bir anahtarla eşleşen, denetlenen dizide bulunan bir öğe dizisinin başlangıcını bulmak için bunu kullanırsınız.
...
Üye işlevi, anahtarı *anahtar*olan bir `value_type` nesnesi döndürür. Diğer birçok üye işlevi ile kullanmak üzere uygun bir nesne oluşturmak için bunu kullanırsınız.
...
Üye işlevi karma tabloyu yeniden oluşturur; [hash_multiset:: load_factor (STL/CLR)](../dotnet/hash-multiset-load-factor-stl-clr.md)`() <=` [hash_multiset:: max_load_factor (STL/CLR)](../dotnet/hash-multiset-max-load-factor-stl-clr.md). Aksi takdirde, karma tablo, bir ekleme işleminden sonra gerektiğinde yalnızca boyut artar. (Boyut olarak hiçbir şekilde azalmayın.) Karma tablonun boyutunu ayarlamak için bunu kullanırsınız.
...
Üye işlevi denetlenen sıranın uzunluğunu döndürür. Bu, şu anda denetlenen dizideki öğelerin sayısını tespit etmek için kullanılır. Her şey için, sıranın sıfır dışında bir boyuta sahip olup olmadığı hakkında bilgi için bkz. [hash_multiset:: Empty (STL/CLR)](../dotnet/hash-multiset-empty-stl-clr.md)`()`.
...
Üye işlevi, *anahtar* ile aynı demetini karma hale uygulayan denetimli dizideki son öğeyi `X` belirler ve *anahtara*denk sıralama içerir. Böyle bir öğe yoksa veya denetlenen dizideki son öğe `X`, [hash_multiset:: End (STL/CLR)](../dotnet/hash-multiset-end-stl-clr.md)`()`döndürür. Aksi takdirde, `X`ötesinde ilk öğeyi atayan bir yineleyici döndürür. Belirtilen bir anahtarla eşleşen denetimli dizide bulunan bir öğe dizisinin sonunu bulmak için bunu kullanırsınız.
