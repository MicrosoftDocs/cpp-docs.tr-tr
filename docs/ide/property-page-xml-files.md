...
1. **Kural:** kuralı genellikle xml dosyasının kök düğümünde; birçok özniteliklere sahip olabilir:
   ...
   a. **Ad:** Name özniteliği bir kural kimliğidir. Tüm özellik sayfası xml dosyaları için bir proje arasında benzersiz olması gerekir.
   ...
   d. **Sipariş:** bu kural sistemdeki diğer tüm kurallar kıyasla göreli konumunu olası bir kullanıcı Arabirimi istemciye bir öneri budur.
   ...
   g. **Veri kaynağı:** bu proje sistemi okunan ve yazılan özellik değeri, gereken konum ve kendi gruplandırma (aşağıda açıklanmıştır) belirten, çok önemli bir özelliktir. Cl.XML için bu değerler şunlardır:
...
1. **Özellikler:** bu xml dosyasının Et ve bu kural, tüm özellikler listesini içerir. Her bir özellik XAML skeleton yukarıda gösterilen beş olası türlerden biri olabilir. Elbette, yalnızca birkaç bu türlerin dosyanızda olabilir. Bir dizi zengin açıklanan izin öznitelikleri bir özelliğe sahiptir. Yalnızca anlatayım **StringProperty** burada. Rest da oldukça benzerdir.
   ...
   f. **IncludeInCommandLine:** bazı özellikleri bir aracı için derleme zamanında geçirilmesi gerekmez. Bu öznitelik false olarak ayarlamayı geçirilen öğesinden engeller.
