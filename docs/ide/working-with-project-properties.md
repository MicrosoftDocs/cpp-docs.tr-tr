...
### <a name="user-defined-macros"></a>Kullanıcı tanımlı makrolar  
 Oluşturabileceğiniz *kullanıcı tanımlı makrolar* proje yapılarında değişkenler olarak kullanılacak. Örneğin, bir özel yapı adımına veya özel yapı aracına değer sağlayan kullanıcı tanımlı bir makro oluşturabilirsiniz. Kullanıcı tanımlı makro bir ad/değer çiftidir. Bir proje dosyasında kullanmak **$(***adı***)** değere erişmek için bir gösterimi.  
...
- Geçersiz kılmak istediğiniz özellikleri belirten bir .props dosyası oluşturun. 
- Komut isteminden: ForceImportBeforeCppTargets="C:\sources\my_props.props Ayarla"
