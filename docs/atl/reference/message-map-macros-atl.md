...
Bir `CMyExtWindow` nesne WM_PAINT bir ileti aldığında, ileti `CMyExtWindow::OnPaint` gerçek işleme için yönlendirilir. `OnPaint`İletinin daha fazla işlem gerektirdiğini gösteriyorsa ileti, içindeki varsayılan ileti eşlemesine yönlendirilir `CMyBaseWindow` .
...
- Bir pencere yordamı `CMyClass` varsayılan ileti eşlemesini kullanıyorsa ve `OnPaint` bir iletiyi işlemezse ileti, `CMyBaseClass` işlenmek üzere varsayılan ileti eşlemesine yönlendirilir.

- Bir pencere yordamı ' de ilk alternatif ileti haritasını kullanıyorsa `CMyClass` , tüm iletiler `CMyBaseClass` varsayılan ileti eşlemesine yönlendirilir.

- Bir pencere yordamı, `CMyClass` ikinci alternatif ileti eşlemesini kullanıyorsa ve `OnChar` bir iletiyi işlemezse ileti, içindeki belirtilen alternatif ileti eşlemesine yönlendirilir `CMyBaseClass` . `CMyBaseClass` Bu ileti haritasını ALT_MSG_MAP (1) ile bildirmeli.
...
- Bir pencere yordamı `CMyClass` varsayılan ileti eşlemesini kullanıyorsa ve `OnPaint` bir iletiyi işlemezse ileti, `m_obj` işlenmek üzere varsayılan ileti eşlemesine yönlendirilir.

- Bir pencere yordamı ' de ilk alternatif ileti haritasını kullanıyorsa `CMyClass` , tüm iletiler `m_obj` varsayılan ileti eşlemesine yönlendirilir.

- Bir pencere yordamı, `CMyClass` ikinci alternatif ileti eşlemesini kullanıyorsa ve `OnChar` bir iletiyi işlemezse ileti, belirtilen alternatif ileti eşlemesine yönlendirilir `m_obj` . Sınıf `CMyContainedClass` , ALT_MSG_MAP (1) ile bu ileti haritasını bildirmeli.
...
İleti eşlemesi `bHandled` çağrılmadan önce true olarak ayarlanır `CommandHandler` . `CommandHandler`İletiyi tam olarak işlemezse, `bHandled` iletinin daha fazla işleme ihtiyacı olduğunu göstermek için false olarak ayarlanmalıdır.
...
COMMAND_HANDLER ek olarak, bir tanımlayıcı veya kodla ilgili olarak WM_COMMAND iletisi eşlemek için [message_handler](#message_handler) kullanabilirsiniz. Bu durumda, `MESSAGE_HANDLER(WM_COMMAND, OnHandlerFunction)` tüm WM_COMMAND iletilerini öğesine yönlendirir `OnHandlerFunction` .
...
, Yansıtılan iletileri alacak alt pencere (denetim) için varsayılan bir işleyici sağlar; işleyici işlenmeyen iletileri öğesine doğru bir şekilde geçilecektir `DefWindowProc` .
...
İleti eşlemesi `bHandled` çağrılmadan önce true olarak ayarlanır `MessageHandler` . `MessageHandler`İletiyi tam olarak işlemezse, `bHandled` iletinin daha fazla işleme ihtiyacı olduğunu göstermek için false olarak ayarlanmalıdır.
...
İleti eşlemesi `bHandled` çağrılmadan önce true olarak ayarlanır `NotifyHandler` . `NotifyHandler`İletiyi tam olarak işlemezse, `bHandled` iletinin daha fazla işleme ihtiyacı olduğunu göstermek için false olarak ayarlanmalıdır.

> [!NOTE]
> [BEGIN_MSG_MAP](#begin_msg_map)bir ileti eşlemesine her zaman başlayın. Daha sonra, [ALT_MSG_MAP](#alt_msg_map)ile sonraki alternatif ileti eşlemelerini bildirebilirsiniz. [End_msg_map](#end_msg_map) makro ileti eşlemesinin sonunu işaretler. Her ileti eşlemesinin BEGIN_MSG_MAP ve END_MSG_MAP tam olarak bir örneği olmalıdır.

NOTIFY_HANDLER ek olarak, bir tanımlayıcı veya kodla ilgili olarak WM_NOTIFY iletisi eşlemek için [message_handler](#message_handler) kullanabilirsiniz. Bu durumda, `MESSAGE_HANDLER(WM_NOTIFY, OnHandlerFunction)` tüm WM_NOTIFY iletilerini öğesine yönlendirir `OnHandlerFunction` .
