---
title: ATL kapsamını | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-atl
ms.topic: conceptual
dev_langs:
- C++
helpviewer_keywords:
- ATL, scope
ms.assetid: 381adf50-3cb0-4d0f-a79a-07da093bc280
author: mikeblome
ms.author: mblome
ms.workload:
- cplusplus
ms.openlocfilehash: ec7af04df819ed64c7c693392ee03b30893ce3d1
ms.sourcegitcommit: be2a7679c2bd80968204dee03d13ca961eaa31ff
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 05/03/2018
ms.locfileid: "32360037"
---
# <a name="scope-of-atl"></a>ATL kapsamı
ATL COM nesneleri, otomasyon sunucuları ve ActiveX denetimlerini kolayca oluşturmanıza olanak sağlar. ATL birçok temel COM arabirimleri için yerleşik destek sağlar.  
  
 ATL uygulamanızda içeren kaynak kodu olarak geliyordu. ATL bileşenler genelinde paylaşılabilir kodu içeren bir DLL kullanılabilir (atl90.dll) de sağlar. Ancak, bu DLL gerekli değildir.  
  
## <a name="see-also"></a>Ayrıca Bkz.  
 [ATL giriş](../atl/introduction-to-atl.md)   
 [ATL Projesi Sihirbazı](../atl/reference/atl-project-wizard.md)

