...
|Ad|Açıklama|  
|----------|-----------------|  
|[reader_writer_lock::scoped_lock sınıfı](#scoped_lock_class)|Almak üzere kullanılabilecek bir özel durum güvenli RAII sarmalayıcı `reader_writer_lock` nesneleri yazan kilitleyin.|  
|[reader_writer_lock::scoped_lock_read sınıfı](#scoped_lock_read_class)|Almak üzere kullanılabilecek bir özel durum güvenli RAII sarmalayıcı `reader_writer_lock` nesneleri okuyucu olarak kilitleyin.|  
