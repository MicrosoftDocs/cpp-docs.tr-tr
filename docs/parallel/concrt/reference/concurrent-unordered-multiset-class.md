...
ms.locfileid: "75297524"
---
# <a name="concurrent_unordered_multiset-class"></a>concurrent_unordered_multiset Sınıfı
...
*_Allocator_type*<br/>
Eşzamanlı vektör için bellek ayırma ve ayırmayı kaldırma hakkındaki ayrıntıları kapsülleyen saklı ayırıcı nesnesini temsil eden tür. Bu bağımsız değişken isteğe bağlıdır ve varsayılan değer `std::allocator<K>`.
...
|Name|Açıklama|
|----------|-----------------|
|[concurrent_unordered_multiset](#ctor)|Aşırı yüklendi. Eşzamanlı olarak sıralı bir çoklu küme oluşturur.|
...
|[ekleyin](#insert)|Aşırı yüklendi. `concurrent_unordered_multiset` nesnesine öğe ekler.|
|[key_eq](#key_eq)|Depolanan eşitlik karşılaştırma işlevi nesnesi.|
|[Kur](#swap)|İki `concurrent_unordered_multiset` nesnesinin içeriğini değiştirir. Bu yöntem eşzamanlılık açısından güvenli değildir.|
|[unsafe_erase](#unsafe_erase)|Aşırı yüklendi. Belirtilen konumlarda `concurrent_unordered_multiset` öğeleri kaldırır. Bu yöntem eşzamanlılık açısından güvenli değildir.|

### <a name="public-operators"></a>Genel İşleçler

|Name|Açıklama|
|----------|-----------------|
|[operator=](#operator_eq)|Aşırı yüklendi. Başka bir `concurrent_unordered_multiset` nesnesinin içeriğini buna atar. Bu yöntem eşzamanlılık açısından güvenli değildir.|

## <a name="remarks"></a>Açıklamalar

`concurrent_unordered_multiset` sınıfı hakkında ayrıntılı bilgi için bkz. [paralel kapsayıcılar ve nesneler](../../../parallel/concrt/parallel-containers-and-objects.md).
...
Eşzamanlı olarak sıralı bir çoklu küme oluşturur.
...
Bu sırasız çoklu küme için ilk Demet sayısı.

*_Hasher*<br/>
Bu sırasız çoklu küme için karma işlev.

*key_equality*<br/>
Bu sırasız çoklu küme için eşitlik karşılaştırma işlevi.

*_Allocator*<br/>
Bu sırasız çoklu küme için ayırıcı.

*first*<br/>
*last*<br/>
*_Uset*<br/>
Öğelerin taşınacağı kaynak `concurrent_unordered_multiset` nesnesi.

### <a name="remarks"></a>Açıklamalar

Tüm oluşturucular bir ayırıcı nesne `_Allocator` depolar ve sıralanmamış çoklu küme 'yi başlatır.

İlk Oluşturucu boş bir başlangıç çoklu kümesini belirtir ve açık olarak, kullanılacak demet sayısını, karma işlevi, eşitlik işlevini ve ayırıcı türünü belirtir.

İkinci Oluşturucu, sıralanmamış çoklu küme için bir ayırıcı belirtir.

Üçüncü Oluşturucu yineleyici aralığı tarafından sağlanan değerleri belirtir [`_Begin`, `_End`).

Dördüncü ve beşinci oluşturucular, eşzamanlı sıralanmamış çoklu küme `_Uset`bir kopyasını belirtir.

Son Oluşturucu, eşzamanlı sıralanmamış çoklu küme `_Uset`taşımayı belirtir.
...
`concurrent_unordered_multiset` nesnesine öğe ekler.
...
İlk üye işlevi, öğe `value` denetimli sırayla ekler ve eklenen öğeyi atayan yineleyiciyi döndürür.
...
Başka bir `concurrent_unordered_multiset` nesnesinin içeriğini buna atar. Bu yöntem eşzamanlılık açısından güvenli değildir.
...
Bu `concurrent_unordered_multiset` nesnesine bir başvuru.

### <a name="remarks"></a>Açıklamalar

Eş zamanlı sıralanmamış bir çoklu kümeli var olan öğeleri sildikten sonra, `operator=` `_Uset` içeriğini eşzamanlı sıralanmamış çoklu kümeli olarak kopyalar ya da taşısın.
...
İki `concurrent_unordered_multiset` nesnesinin içeriğini değiştirir. Bu yöntem eşzamanlılık açısından güvenli değildir.
...
*_Uset*<br/>
İle takas edilecek `concurrent_unordered_multiset` nesnesi.
...
Belirtilen konumlarda `concurrent_unordered_multiset` öğeleri kaldırır. Bu yöntem eşzamanlılık açısından güvenli değildir.
...
İlk üye işlevi, `_Where`tarafından işaret edilen öğeyi kaldırır. İkinci üye işlevi [`_Begin`, `_End`) aralığındaki öğeleri kaldırır.
