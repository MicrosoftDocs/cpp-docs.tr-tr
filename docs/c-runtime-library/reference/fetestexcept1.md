...
## <a name="remarks"></a>Açıklamalar  
 Bir kayan hangi özel durumları ortaya çıktı belirlemek için fetestexcept işlevini nokta işlemi. Kullanım `excepts` sınamak için hangi özel durumu bayrakları belirtmek için parametre. `fetestexcept` İşlevini kullanan tanımlanan bu özel durum makroları \<fenv.h > içinde `excepts` ve dönüş değeri:  
