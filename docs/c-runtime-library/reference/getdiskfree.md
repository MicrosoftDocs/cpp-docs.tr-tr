...
```  
 ...
 Bu işlev parametrelerini doğrular. Varsa `driveinfo` işaretçi `NULL` veya `drive` geçersiz bir sürücü belirtir açıklandığı gibi bir geçersiz parametre işleyicisi bu işlevi çağırır [parametre doğrulaması](../../c-runtime-library/parameter-validation.md). Yürütme devam etmek için izin verilip verilmediğini, işlevi döndürür `EINVAL` ve ayarlar `errno` için `EINVAL`. 26 geçerli sürücüler aralığı 0'dan. A `drive` 0 değeri, geçerli sürücüyü belirtir; bundan sonra numaraları eşleme alfabedeki İngilizce bu tür için 1 sürücüsü gösterir, 3, C sürücüsünün vb. gösterir.  
  
 `total_clusters`  
 Kümeler, kullanılan ve diskteki kullanılabilir toplam sayısı.  
  
 `avail_clusters`  
 Diskte kullanılmayan küme sayısı.  
  
 `sectors_per_cluster`  
 Her küme iz sayısı.  
  
 `bytes_per_sector`  
 Her kesim bayt cinsinden boyutu.  
