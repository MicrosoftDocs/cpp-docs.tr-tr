...
**_aligned_malloc** , `__declspec(noalias)` ve `__declspec(restrict)`işlevin genel değişkenleri değiştirmeyeceği ve döndürülen işaretçinin diğer ad olmadığından garanti edilen anlamına gelir. Daha fazla bilgi için bkz. [noalias](../../cpp/noalias.md) ve [Restrict](../../cpp/restrict.md).
...
Hem **_aligned_malloc** hem de elde edilen belleği serbest bırakmak için `_aligned_offset_malloc` [_aligned_free](aligned-free.md) kullanın. Kullanmayın `free`, bu, hizalanmış belleği doğru bir şekilde geri almamıştır ve tanılayan hatalara yol açabilir.
