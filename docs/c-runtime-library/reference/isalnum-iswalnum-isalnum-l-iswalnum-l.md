...
## <a name="return-value"></a>Dönüş Değeri  
 ...
 Davranışını `isalnum` ve `_isalnum_l` tanımsız ise `c` EOF değil veya 0'dan 0xFF (bunlar dahil) aralığında. CRT hata ayıklama Kitaplığı kullanıldığında ve `c` işlevleri olursa bu değerleri onayı ifade değil.  
