...
**_Aligned_free_dbg** işlevi, [_aligned_free](aligned-free.md) işlevinin hata ayıklama sürümüdür. [_DEBUG](../../c-runtime-library/debug.md) tanımlı olmadığında, her **_aligned_free_dbg** çağrısı `_aligned_free`çağrısına düşürülür. Hem `_aligned_free` hem de **_aligned_free_dbg** temel yığında bir bellek bloğu boşaltın, ancak **_aligned_free_dbg** bir hata ayıklama özelliğine sahiptir: düşük bellek koşullarının benzetimini yapmak için yığının bağlantılı listesinde serbest bırakılan blokları saklama özelliği.
...
Belleği boşaltmaya yönelik bir hata oluşursa, `errno` hatanın doğasından işletim sisteminden bilgi olarak ayarlanır. Daha fazla bilgi için bkz. [errno, _doserrno, _sys_errlist ve _sys_nerr](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md).
