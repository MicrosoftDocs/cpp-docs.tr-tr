...
## <a name="return-value"></a>Dönüş Değeri  
 Başarılı, bu işlevlerin her biri karakter bağımsız verir durumunda `c`. Varsa `c` geri gönderilir ya da hiçbir karakter okuyorsanız Giriş akışı değiştirilmemiştir ve `_ungetc_nolock` döndürür `EOF`; `_ungetwc_nolock` döndürür `WEOF`. Varsa `stream` olan `NULL`, `EOF` veya `WEOF` döndürülür ve `errno` ayarlanır `EINVAL`.  
