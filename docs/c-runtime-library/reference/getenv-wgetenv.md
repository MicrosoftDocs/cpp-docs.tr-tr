...
## <a name="remarks"></a>Açıklamalar  
 ...
 Benzer şekilde, bir Unicode (`_wmain`) programı `_environ` başlangıçta `NULL` joker karakter dizelerini oluşturulan ortam olduğundan. Ardından, ilk çağrıda `_putenv`, ya da ilk çağrıda `getenv` (Unicode) ortam zaten varsa, karşılık gelen bir MBCS ortamı oluşturulur ve ardından işaret ediyor `_environ`.  
...
|`_tgetenv`|`getenv`|`getenv`|`_wgetenv`|  
 ...
 Denetlemek veya değerini değiştirmek için `TZ` ortam değişkeni, kullanım `getenv`, `_putenv` ve `_tzset` gerektiği. Hakkında daha fazla bilgi için `TZ`, bkz: [_tzset](../../c-runtime-library/reference/tzset.md) ve [_daylight, saat dilimi ve _tzname](../../c-runtime-library/daylight-dstbias-timezone-and-tzname.md).  
