...
ms.locfileid: "32405347"
---
# <a name="open-wopen"></a>_open, _wopen

Bir dosyayı açar. Daha güvenli sürümlerinde kullanılabilir olmadığından, bu işlevler kullanım dışıdır; bkz: [_sopen_s, _wsopen_s](sopen-s-wsopen-s.md).
...
|errno değeri|Koşul|
|-|-|
**EACCES**|Yazma, dosyanın salt okunur bir dosyayı açmaya çalıştı paylaşma modunda belirtilen işlemleri izin verme ya da verilen yolu bir dizindir.
...
**EMFILE**|Daha fazla hiçbir dosya tanımlayıcıları kullanılabilir (çok fazla dosya açık).
...
**_Kurulum Aç** işlev tarafından belirtilen dosyayı açar *filename* okuma veya yazma belirtildiği için hazırlar *oflag*. **_wopen** bir joker karakter sürümü **_kurulum Aç**; *filename* bağımsız değişkeni **_wopen** bir joker karakter dizesidir. **_wopen** ve **_kurulum Aç** Aksi takdirde aynı şekilde davranır.

### <a name="generic-text-routine-mappings"></a>Genel Metin Yordam Eşleşmeleri

|Tchar.h yordamı|_UNICODE ve _MBCS tanımlanmaz|_MBCS tanımlanmış|_UNICODE tanımlanmış|
|---------------------|--------------------------------------|--------------------|-----------------------|
|**_topen**|**_kurulum Aç**|**_kurulum Aç**|**_wopen**|

*oflag* bir tamsayı ifade bir veya daha fazla bildirim sabitlerden veya tanımlanan sabit birleşimleri biçimlendirildiğinden \<fcntl.h >.
...
Her iki sabitleri verildiğinde olan bit düzeyinde OR işleci katılan ( **&#124;** ). Windows, tüm dosyaları okunabilir; yalnızca yazma izni kullanılabilir değil. Bu nedenle, modları **_s_ıwrıte** ve **_s_ıread** | **_s_ıwrıte** eşdeğerdir.

Bir değer, şunların bir birleşimi dışında **_s_ıread** ve **_s_ıwrıte** için belirtilen *pmode*— geçerli bir belirtin olsa bile *pmode*başka bir işletim sisteminde — veya herhangi bir diğer izin verilenden değer *oflag* değerleri belirtilirse, işlev hata ayıklama modunda bir onaylama oluşturur ve anlatıldığıgibigeçersizbirparametreişleyiciyiçağırır[Parametre doğrulaması](../../c-runtime-library/parameter-validation.md). Devam etmek için ayarlar ve işlev dönüşleri -1 yürütülmesine izin veriliyorsa **errno** için **EINVAL**.

## <a name="requirements"></a>Gereksinimler

|Yordam|Gerekli başlık|İsteğe bağlı üstbilgi|
|-------------|---------------------|---------------------|
|**_kurulum Aç**|\<io.h >|\<fcntl.h >, \<sys\types.h >, \<sys\stat.h >|
|**_wopen**|\<io.h > veya \<wchar.h >|\<fcntl.h >, \<sys\types.h >, \<sys\stat.h >|

**_kurulum Aç** ve **_wopen** Microsoft uzantıları. Daha fazla uyumluluk bilgileri için bkz: [Uyumluluk](../../c-runtime-library/compatibility.md).
