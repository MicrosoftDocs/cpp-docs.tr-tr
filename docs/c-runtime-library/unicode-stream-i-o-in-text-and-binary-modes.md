...
Ancak, bir Unicode Stream-g/ç işlevi ikili modda çalışırken, dosyanın Unicode olduğu varsayılır ve giriş veya çıkış sırasında CR-LF çevirisi veya karakter dönüştürme gerçekleşmez. Bir UNICODE metin dosyasında `wcin` doğru şekilde kullanmak için `_setmode( _fileno( stdin ), _O_BINARY );` yönergesini kullanın.
