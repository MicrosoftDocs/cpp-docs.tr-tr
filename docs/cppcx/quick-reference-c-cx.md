...
|Sınıf bildirimi|`class`  *identifier* `{}`<br /><br /> `struct`  *Tanımlayıcı* `{}`|`ref class`  *Tanımlayıcı* `{}`<br /><br /> `ref struct`  *Tanımlayıcı* `{}`|Varsayılan özel erişilebilirlik olan bir çalışma zamanı sınıfı bildirir.<br /><br /> Varsayılan ortak erişilebilirlik olan bir çalışma zamanı sınıfı bildirir.|  
