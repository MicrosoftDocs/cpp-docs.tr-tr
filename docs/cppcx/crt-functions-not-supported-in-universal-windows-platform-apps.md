---
title: Evrensel Windows platformu uygulamaları desteklenmeyen CRT işlevleri | Microsoft Docs
ms.custom: ''
ms.date: 12/30/2016
...
||||  
|-|-|-|  
|_beep _sleep _seterrormode|Bu işlevler CRT önceki sürümleri artık kullanılmıyor. Ayrıca, karşılık gelen Win32 API'ları UWP uygulamaları için kullanılabilir değildir.|Geçici çözüm yok.|  
...
|||  
|-|-|  
|Tek baytlı dize işlevleri — Örneğin, `strcat`, `strcpy`, `strlwr`ve benzeri.|UWP uygulamalarınızın kesinlikle Unicode karakter kümeleri yalnızca Unicode tüm Win32 API'ları ve Windows çalışma zamanı API'de gösterilen kullandığından.  Tek baytlı işlevler büyük kod bağlantı noktası oluşturma taban ancak Aksi halde kaçınılmalıdır ve karşılık gelen geniş karakter işlevleri, bunun yerine mümkün olduğunda kullanılmalıdır kalmıştır.|  
|Akış g/ç ve alt düzey dosya g/ç işlevleri — Örneğin, `fopen`, `open`ve benzeri.|Bu işlevler zaman uyumlu, UWP uygulamaları için önerilmez. UWP uygulamalarınızdaki zaman uyumsuz API'lerini kullanan açmak için okuma ve dosyalara kullanıcı Arabirimi iş parçacığı kilitlenmelerini önlemek için yazma. Bu tür API'leri örnekler Listedekilerin `Windows::Storage::FileIO` sınıfı.|  
...
## <a name="windows-8x-store-apps-and-windows-phone-8x-apps"></a>Windows 8.x mağazası uygulamaları ve Windows Phone 8.x uygulamaları  
 Yukarıda açıklanan API'leri ek olarak, Windows 8.x mağazası uygulamaları ve Windows Phone 8.x uygulamaları aşağıdaki API'leri kullanılamaz.  
