...
-   Ortak enum sınıfı  
 ...
 UnorderedMap, aslında için sarmalayıcı [std::unordered_map](../standard-library/unordered-map-class.md) Windows çalışma zamanı türleri depolanmasını destekler. Bu somut bir uyarlamasını [Windows::Foundation::Collections::IMap](http://go.microsoft.com/fwlink/p/?LinkId=262408) ve [IObservableMap](http://msdn.microsoft.com/library/windows/apps/br226050.aspx) Windows çalışma zamanı arasında ortak geçirilen türleri arabirimi. Platform::Collections:: unorderedmap türü bir public kullanmayı denerseniz değeri veya parametre, C3986 tetiklenir derleyici hatası döndürür. Parametresi veya dönüş değerinin türünü değiştirerek hatayı düzeltmek [Windows::Foundation::Collections::IMap](http://go.microsoft.com/fwlink/p/?LinkId=262408).  
...
|-|-|  
|Ad|Açıklama|  
|[Map::MapChanged](#mapchanged)`event`|Harita değiştiğinde gerçekleşir.|  
...
### <a name="parameters"></a>Parametreler  
 `key`  
 UnorderedMap öğesi bulmak için kullanılan anahtar. Türü `key` TypeName *K*.  
...
### <a name="return-value"></a>Dönüş Değeri  
 `true`Geçerli eşlemesindeki var olan bir öğe anahtarı eşleşip eşleşmediğini `key` ve o öğenin değeri kısmı kümesine `value`. `false`Geçerli eşlemesindeki var olan bir öğe eşleşmiyorsa `key` ve `key` ve `value` parametreleri bir anahtar-değer çifti yapılan ve geçerli UnorderedMap eklendi.  
...
### <a name="parameters"></a>Parametreler  
 `key`  
 Bir öğenin UnorderedMap bulmak için kullanılan anahtar. Türü `key` TypeName *K*.  
...
## <a name="see-also"></a>Ayrıca Bkz.  
 ...
 [Koleksiyonları](../cppcx/collections-c-cx.md)   
