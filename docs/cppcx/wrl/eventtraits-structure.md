---
description: 'Daha fazla bilgi edinin: Eventnitelikler yapısı'
title: EventTraits Yapısı
ms.date: 11/04/2016
ms.topic: reference
f1_keywords:
- corewrappers/Microsoft::WRL::Wrappers::HandleTraits::EventTraits
helpviewer_keywords:
- EventTraits structure
ms.assetid: 2cf308a4-50e3-4321-bafc-4c7c0364e148
ms.openlocfilehash: 062438c945c3df2da88d6026febfaf2f91247a7d
ms.sourcegitcommit: d6af41e42699628c3e2e6063ec7b03931a49a098
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 12/11/2020
ms.locfileid: "97198562"
---
# <a name="eventtraits-structure"></a>EventTraits Yapısı

Bir `Event` Sınıf tanıtıcısının özelliklerini tanımlar.

## <a name="syntax"></a>Syntax

```cpp
struct EventTraits : HANDLENullTraits;
```

## <a name="members"></a>Üyeler

## <a name="inheritance-hierarchy"></a>Devralma Hiyerarşisi

`HANDLENullTraits`

`EventTraits`

## <a name="requirements"></a>Gereksinimler

**Üstbilgi:** corewrapper. h

**Ad alanı:** Microsoft:: WRL:: sarmalayıcılar:: Handlenitelikler

## <a name="see-also"></a>Ayrıca bkz.

[Microsoft:: WRL:: sarmalayıcılar:: Handlenitelikleri ad alanı](microsoft-wrl-wrappers-handletraits-namespace.md)
