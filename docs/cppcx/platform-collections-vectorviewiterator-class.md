...
**`true`** geçerli varsa `VectorViewIterator` , tersi durumda *other* **`false`** .
...
**`true`** geçerli bir değerden `VectorViewIterator` büyükse veya buna eşitse, aksi *other*takdirde **`false`** .
